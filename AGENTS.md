...
- Complete attached implementation plans end-to-end without editing the plan file; mark plan todos in progress and finish all before stopping.
- Prefer semantic module splits with passing imports over mechanical line-range file splits; discard incomplete split waves rather than ship broken packages.
- Reader-facing PDFs and manuscripts for multi-tier private projects should present the archival compendium only—no seed or demonstrator tier language in publication body.

## Learned Workspace Facts

- Private project source commits to the external `docxology/projects` repo (`$TEMPLATE_PRIVATE_PROJECTS_ROOT`), not the public `template/` git tree.
- Symlinked `projects/working/*` and `projects/active/*` are local-only but can be rendered from the template root with `uv run python scripts/03_render_pdf.py --project {name}` when project markers exist.
- Projects using `{{TOKEN}}` manuscript substitution must hydrate `output/manuscript/` via `infrastructure.rendering.manuscript_injection.write_resolved_manuscript_tree` before PDF render (pattern: `template_code_project`, `grateful_data` `scripts/z_generate_manuscript_variables.py`).
- `grateful_data` canonical tree lives at `docxology/projects/working/grateful_data`; default publication tier is `GRATEFUL_DATA_TIER=archival`.
