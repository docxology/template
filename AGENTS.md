...
- WIP resolution: `infrastructure.project.discovery.resolve_project_root` prefers `projects/<name>/` when that tree has `src/`, `tests/`, `scripts/`, and `manuscript/`; otherwise the same name under `projects_in_progress/` (nested WIP: pass e.g. `cognitive_integrity/cogsec_multiagent_1_theory`; bare `cogsec_multiagent_*` resolves only under `projects/`). Symlinked private projects under `projects/` carry their own operational canon in that tree's `AGENTS.md`; consult [`docs/_generated/active_projects.md`](docs/_generated/active_projects.md) instead of hard-coding rotating paths in root docs. Lifecycle moves between `/Users/4d/Documents/GitHub/projects/{active,passive,archive}/` can leave stale `.venv` interpreters and broken `projects_in_progress/` symlinks—confirm the canonical private path and run `uv sync` before long pytest gates.
...
- **`biology_textbook` (active private curriculum):** standalone repo at `/Users/4d/Documents/GitHub/projects/active/biology_textbook`, symlinked as `projects/biology_textbook` — commit project changes there, not in public `template/`. Unnumbered H1s: Pandoc `{#sec:… .unnumbered}` on the header line, not a standalone `\label` after `\section*`. Front matter, unit intros, labs, questions, appendices, and glossary use `\nameref{sec:…}` — not `\cref{sec:lab_…}` / `\cref{sec:q_…}` on unnumbered `\section*` (titlesec collapses those to one counter). In-chapter numbered sections may use `\cref`. Render prefers populated `output/manuscript/`; run `biology_analysis.py` after manuscript edits before `03_render_pdf.py`. Enrichment scripts default to dry-run; pass `--write` to persist. Publication audit: `REVIEW.md`, `scripts/audit_publication_readiness.py`. Operational canon lives in that tree's `AGENTS.md`.
- **PDF front matter / `\nameref` with titlesec:** `fix_starred_section_nameref_labels()` in `infrastructure/rendering/_pdf_combined_renderer.py` must run **after** `inject_latex_preamble()` — if it runs before titlesec loads, the fix never applies. It rewrites starred `\section*`/`\subsection*`/`\subsubsection*` plus `\label{…}` pairs (not only `sec:`) with `\phantomsection` and `\@currentlabelname` because titlesec prevents hyperref from storing unnumbered section titles. `check_pdf_log` may still flag front-matter `\nameref` ordering while the combined PDF renders correctly.
