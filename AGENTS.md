...
- Purge legacy / historical / outdated / deprecated narrative from docs, manuscripts, and code on every review pass; never leave superseded references behind — including fork residue (e.g. Crescent City / Del Norte / Tolowa prose, data, and bib keys after crescent_city → encinitas adaptation).
...
- When a delegated subagent's result is already visible to the user, do not re-summarize or restate it unless asked or multi-task synthesis is required; end with a brief third-person completion confirmation instead.
...
- After thermo-nuclear or publication remediation plans, when the user asks to proceed comprehensively or to do all P2, implement deferred modularizations and follow-ups in the same pass—not report-only or P0/P1-only closure. Honor explicit scope waivers (e.g. skipping a student edition) without reopening waived tracks.
...
- `docs/prompts/` hosts discoverable agent skills: hub [`docs/prompts/SKILL.md`](docs/prompts/SKILL.md) (`template-workflows`) plus `template-*` workflow subdirs; in `infrastructure.skills.discovery.DEFAULT_SKILL_SEARCH_ROOTS`; regenerate via `uv run python -m infrastructure.skills write` (+ `write-index`). Synthetic skill-eval harness at [`docs/prompts/_skill-eval/`](docs/prompts/_skill-eval/) — excluded from default docs lint via `SKILL_EVAL_DIR_NAME` in `infrastructure/validation/docs/scan_scope.py`; keyword grader, not live agent evals.
- **`biology_textbook` (active private curriculum):** standalone repo at `/Users/4d/Documents/GitHub/projects/active/biology_textbook`, symlinked as `projects/biology_textbook` — commit project changes there, not in public `template/`. Primary release artifact is the **Instructor Edition** (`export.include_solutions: true`; `watermark_instructor: false` — visible solutions without diagonal watermark); student edition is out of scope unless explicitly requested. **`manuscript/config.yaml` is canonical for unit/chapter display titles** — propagate via `sync_curriculum_materials.py` and `insert_chapter_metadata.py`; do not hand-edit generated front-matter navigation blocks. After title edits, run `replace_legacy_titles()` so back-matter pedagogy H2 suffixes match chapter titles. Zenodo DOI lives in config (`10.5281/zenodo.20286478`); canonical release gate is `scripts/audit_publication_readiness.py --check --full`. Matplotlib figures require `\caption{}` plus matching `<!-- alt: … -->`; front-matter TOC links use `\hyperref[sec:…]`. Unnumbered H1s: Pandoc `{#sec:… .unnumbered}` on the header line, not a standalone `\label` after `\section*`. Front matter, unit intros, labs, questions, appendices, and glossary use `\nameref{sec:…}` — not `\cref{sec:lab_…}` / `\cref{sec:q_…}` on unnumbered `\section*` (titlesec collapses those to one counter). In-chapter numbered sections may use `\cref`. Render prefers populated `output/manuscript/`; run `biology_analysis.py` after manuscript edits before `03_render_pdf.py`. Enrichment scripts default to dry-run; pass `--write` to persist. Claim-binding regression scaffold: `tests/regression/pinned_values/biology_textbook.json` (public template). Publication audit ledger: `REVIEW.md` plus `audit_publication_readiness.py`. Operational canon lives in that tree's `AGENTS.md`.
- **`entofile` (active private format research):** standalone repo at `/Users/4d/Documents/GitHub/projects/active/entofile`, symlinked as `projects/entofile` — commit project changes there, not in public `template/`. ENTO v0.1.0 modular stdlib-crypto container format mirroring `template_code_project` layout; typed models live in `src/models.py` (not `types.py` — stdlib shadowing). Security controls in `src/security.py` with threat-model docs under `docs/`; `validate_generated_outputs()` gates on tamper-detection rate `1.0`, figure registry `generated_by` provenance, and keyless `verify_container` on a benchmark ZIP. Zenodo DOI in config: `10.5281/zenodo.20396329`. Release gate: `scripts/audit_publication_readiness.py --check`. Operational canon lives in that tree's `AGENTS.md`.
- **`what_is_cogsec` (passive phrase-lineage review):** standalone repo at `/Users/4d/Documents/GitHub/projects/passive/what_is_cogsec`, symlinked as `projects_in_progress/what_is_cogsec` — commit project changes there, not in public `template/`. Not discovered by `./run.sh` until promoted; run via `resolve_project_root()` + `--project what_is_cogsec` from the template root. Corpus: 124 rows, 28 manifest-driven figures, three tracked phrases; project test gate 85% (`src/` measured ~92% post-refactor). Corpus figure renderers split across `src/analysis/corpus_*_figures.py`; methods figures split across `methods_pipeline_figures.py` / `methods_audit_figures.py`; reporting audits split across `reporting_prisma_audit.py` / `reporting_evidence_audit.py`. Phrase lineage uses contiguous `verify_phrase_in_text` matches with `phrase_lineage_negative_control_invariants()`. Claim-binding scaffold: `tests/regression/pinned_values/what_is_cogsec.json` (public template). Operational canon lives in that tree's `AGENTS.md`.
- `run.sh` and `secure_run.sh` source only [`scripts/shell_bootstrap.sh`](scripts/shell_bootstrap.sh) (shared `uv` bootstrap and sandbox env); menu and argparse live in `infrastructure.orchestration`. [`scripts/bash_utils.sh`](scripts/bash_utils.sh) serves backup/health scripts and tests, not pipeline entrypoints. Integration tests: `tests/integration/test_run_sh.py`, `tests/integration/test_secure_run_sh.py`. Exemplar doc/code drift: `scripts/check_template_drift.py` → `infrastructure.project.drift.run_drift_checks()` on `PUBLIC_PROJECT_NAMES` (`--project`, `--strict`). Layer 1 module size: `scripts/gates/module_line_count_check.py` and `uv run python -m infrastructure.core.health` (`module-line-count`). Opt-in gates under `scripts/gates/` report `status: "skipped"` under `skipped_tools` when tools are missing, not as clean scans.
- **`encinitas` (active private scholarly history):** standalone repo at `/Users/4d/Documents/GitHub/projects/active/encinitas`, symlinked as `projects/encinitas`, forked from archived `crescent_city` — commit project changes there, not in public `template/`. Co-authors Daniel Ari Friedman and Tucker Cahill Chambers (ORCID `0009-0008-3793-7872`); both affiliated with Atta Labs. Space/Time/People/Ideas architecture (~46 chapters, 26 figures); purge Crescent City / Del Norte / Tolowa residue from prose, data, bib, and tests on review passes. Release gate: `scripts/run_history_pipeline.py --strict`. Operational canon lives in that tree's `AGENTS.md`.
