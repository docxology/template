...
def publish_to_zenodo(
    ...
    Creates a new deposition on Zenodo, uploads the specified files,
    and publishes the deposition to obtain a DOI.

    Args:
        metadata: Publication metadata containing title, authors, abstract,
            keywords, and license information.
        file_paths: List of file paths to upload to the deposition.
            Non-existent files are silently skipped.
        access_token: Zenodo API access token for authentication.
        sandbox: If True, use Zenodo sandbox environment for testing.
            Defaults to True for safety.

    Returns:
        The DOI string assigned to the published deposition.

    Raises:
        PublishingError: If the Zenodo API request fails or returns an error.
    """
...
def prepare_arxiv_submission(output_dir: Path, metadata: PublicationMetadata) -> Path:
    ...
    Creates a compressed tarball containing LaTeX sources, bibliography files,
    and figures suitable for arXiv submission. The package includes .tex, .bib,
    .cls, .bst files and a figures/ directory if present.

    Args:
        output_dir: Directory containing build artifacts. The function expects
            a sibling 'manuscript' directory with LaTeX sources.
        metadata: Publication metadata used to locate the .bbl file by title.

    Returns:
        Path to the generated .tar.gz file, named with the current date
        (e.g., arxiv_submission_20240115.tar.gz).

    Note:
        If an arxiv_submission directory already exists in output_dir,
...
def create_github_release(
    ...
    Creates a new release on GitHub using the REST API, then uploads
    the specified asset files to the release.

    Args:
        tag_name: Git tag name for the release (e.g., 'v1.0.0').
        release_name: Human-readable title for the release.
        description: Markdown-formatted release notes or description.
        assets: List of file paths to attach to the release.
            Non-existent files are silently skipped.
    ...
    Returns:
        The HTML URL of the created release page.
