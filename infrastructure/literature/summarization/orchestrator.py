...
def get_library_analysis(library_entries: List[LibraryEntry]) -> Dict[str, int]:
    ...
    extracted_text_dir = Path("literature/extracted_text")
    ...
    for entry in library_entries:
        ...
        has_extracted_text = (Path("literature/extracted_text") / f"{citation_key}.txt").exists()
...
def find_papers_needing_processing(
    ...
    return {
        ...
        'need_summary': [e for e in library_entries if e.pdf_path and extractor.has_extracted_text(e.citation_key) and not Path(f"literature/summaries/{e.citation_key}_summary.md").exists()]
    }
...
def run_extract_text(workflow: "LiteratureWorkflow") -> int:
    ...
    for search_result, pdf_path in papers_needing_extraction:
        ...
        if success:
            ...
            failed += 1
            logger.error(f"[{citation_key}] Extraction failed: {error_msg}")
