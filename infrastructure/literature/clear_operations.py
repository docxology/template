...
from infrastructure.literature.library_index import LibraryIndex, LiteratureConfig

logger = get_logger(__name__)


def clear_pdfs(confirm: bool = True, interactive: bool = False) -> Dict[str, Any]:
    """Clear all PDFs from the literature/pdfs directory.
...
def clear_summaries(confirm: bool = True, interactive: bool = False) -> Dict[str, Any]:
    """Clear all summaries from the literature/summaries directory.
...
def clear_library(confirm: bool = True, interactive: bool = False) -> Dict[str, Any]:
    ...
    - Library index entries (literature/library.json)
    - All PDFs (literature/pdfs/)
    - All summaries (literature/summaries/)
    ...
    try:
        ...
        if entry_count == 0 and pdf_count == 0 and summary_count == 0 and not progress_file_exists:
            return {
                "success": True,
                "entries_removed": 0,
                "pdfs_removed": 0,
        ...
        # 4. Clear all library index entries
        entries_removed = 0
        for entry in entries:
            try:
