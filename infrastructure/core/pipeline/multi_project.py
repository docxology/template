...
def format_multi_project_detailed_report(
    ...
    *,
    ...
    STABILITY: this function is the canonical pipeline-completion reporting
    surface. Tests in ``tests/infra_tests/core/pipeline/`` (and the sibling
    ``tests/infra_tests/core/test_multi_project_detailed_report.py``) assert
    on substring presence (e.g. ``"MULTI-PROJECT EXECUTION SUMMARY"``,
    ``"PROJECT STATUS"``, ``"STAGE TIMING BREAKDOWN"``,
    ``"PERFORMANCE HIGHLIGHTS"``), not on layout. New sections may be
    appended; existing section headers MUST NOT change.

    Schema (top to bottom):
      - Banner: ``<bar>``, ``"MULTI-PROJECT EXECUTION SUMMARY"``, N/M counts, wall time
      - PROJECT STATUS: per-project row (icon, name, N/M stages, duration, size)
      - STAGE TIMING BREAKDOWN: per-stage avg/total/ok across all projects
      - PERFORMANCE HIGHLIGHTS: fastest + slowest project
      - Failure details (if any): per-failed-project stage + error excerpt
      - OUTPUT LOCATIONS: combined PDF path per project
      - NEXT STEPS: success or investigation hint

    Reference image: see docs/operational/logging/output-design.md.
...
def write_last_run_summary(summary_lines: list[str], repo_root: Path) -> Path | None:
    ...
    Called by the orchestrator at end-of-run, **once per multi-project pipeline
    invocation**. Kept out of :func:`format_multi_project_detailed_report` so
    that ~800 rendering tests that exercise the formatter do not pollute the
    docs tree on every test run.

    Args:
        summary_lines: Output of :func:`format_multi_project_detailed_report`.
        repo_root: Repository root containing ``docs/_generated/``.

    Returns:
        The artifact path on success, or None on failure (logged as warning).
    """
    try:
        ...
        artifact.write_text(
            ...
            "> Auto-generated by `infrastructure.core.pipeline.multi_project.write_last_run_summary`. "
        ...
        # Loud-but-non-fatal: docs-write failure must never abort the pipeline,
        # but the operator deserves to see that the regression-detection artifact
        # didn't refresh.
