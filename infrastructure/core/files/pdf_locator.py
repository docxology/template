...
def find_combined_pdf(output_dir: Path, project_name: str) -> tuple[Path, float] | None:
    ...
    3. ``projects/{qualified_name}/output/pdf/{project_name}_combined.pdf``
       (source, pre-copy — used by Stage 4 validation before Stage 5 runs).
       ``qualified_name`` carries any typed-subfolder/program prefix, e.g.
       ``projects/working/{name}/output/pdf/...``.

    Args:
        output_dir: The output directory to search. May be either the top-level
            ``output/{qualified_name}`` (post-copy) or a source
            ``projects/{qualified_name}/output`` directory.
