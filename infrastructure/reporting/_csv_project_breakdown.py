...
def _write_manuscript_rows(writer: RowWriter, project: "ProjectMetrics") -> None:
    ...
    rows = [
        ...
        (name, "Manuscript", "Figures", m.figures, "count", "Figure references in manuscript", "Low", "0-20 figures"),
        (name, "Manuscript", "References", m.references, "count", "Citation references", "Medium", "10-200 references"),
    ]
    for row in rows:
        writer.writerow(row)
...
def _write_codebase_rows(writer: RowWriter, project: "ProjectMetrics") -> None:
    ...
    c = project.codebase
    rows = [
        (name, "Codebase", "Source_Files", c.source_files, "files", "Python source files", "Medium", "1-20 files"),
        (name, "Codebase", "Source_Lines", c.source_lines, "lines", "Lines of source code", "Medium", "100-5000 lines"),
        (name, "Codebase", "Methods", c.methods, "count", "Function/method definitions", "Medium", "10-500 methods"),
        (name, "Codebase", "Classes", c.classes, "count", "Class definitions", "Low", "0-50 classes"),
        (name, "Codebase", "Scripts", c.scripts, "files", "Analysis scripts", "Low", "0-10 scripts"),
    ]
    for row in rows:
        writer.writerow(row)
...
def _write_test_rows(writer: RowWriter, project: "ProjectMetrics") -> None:
    ...
    t = project.tests
    rows = [
        (name, "Testing", "Test_Files", t.test_files, "files", "Test files discovered", "High", "1+ files"),
        (
            name,
            "Testing",
            "Total_Tests",
        ...
        (name, "Testing", "Execution_Time", t.execution_time, "seconds", "Test execution time", "Low", "1-300 seconds"),
    ]
    for row in rows:
        writer.writerow(row)
...
def _write_output_rows(writer: RowWriter, project: "ProjectMetrics") -> None:
    ...
    o = project.outputs
    rows = [
        (name, "Outputs", "PDF_Files", o.pdf_files, "files", "Generated PDF documents", "High", "1+ files"),
        (name, "Outputs", "PDF_Size_MB", f"{o.pdf_size_mb:.2f}", "MB", "Total PDF file size", "Low", "0.1-10 MB"),
        (name, "Outputs", "Figures", o.figures, "files", "Generated figure files", "Medium", "0-50 figures"),
        (name, "Outputs", "Slides", o.slides, "files", "Generated slide files", "Medium", "0-20 slide sets"),
        (name, "Outputs", "Web_Outputs", o.web_outputs, "files", "Generated web files", "Medium", "0-20 web files"),
    ]
    for row in rows:
        writer.writerow(row)
