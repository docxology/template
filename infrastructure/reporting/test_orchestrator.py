...
def report_results(
    ...
    if overall_success:
        if not infra_was_run:
            ...
            logger.info(f"Infrastructure: ✓ PASSED ({infra_passed}/{infra_total} tests, {infra_coverage:.1f}% coverage)")
        else:
            infra_failed = infra_results.get("failed", 0)
            logger.info(f"Infrastructure: ⚠ WARNING ({infra_failed} test(s) failed, but continuing)")

        logger.info(f"Project:       ✓ PASSED ({project_passed}/{project_total} tests, {project_coverage:.1f}% coverage)")
        logger.info("-" * 64)
