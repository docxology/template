...
class RepositoryScanner:
    ...
    def scan_all(self) -> ScanResults:
        """Run all scans."""
        print("=" * 70)
        print("REPOSITORY-WIDE ACCURACY AND COMPLETENESS SCAN")
        print("=" * 70)
        
        print("\n[1/6] Discovering repository structure...")
        self._discover_structure()
        
        print("\n[2/6] Checking code accuracy...")
        self._check_code_accuracy()
        
        print("\n[3/6] Checking completeness...")
        self._check_completeness()
        
        print("\n[4/6] Checking test coverage...")
        self._check_test_coverage()
        
        print("\n[5/6] Checking configuration accuracy...")
        self._check_configuration()
        
        print("\n[6/6] Checking thin orchestrator pattern compliance...")
    ...
    def _discover_structure(self):
        ...
        # Find scripts
        for script_dir in [self.repo_root / 'scripts', self.repo_root / 'repo_utilities']:
        ...
        print(f"  Found {len(self.src_modules)} src/ modules")
        print(f"  Found {len(self.script_files)} script files")
        print(f"  Found {len(self.test_files)} test files")
        print(f"  Found {len(self.documented_modules)} documented modules")
    
    def _find_documented_modules(self):
        """Find modules mentioned in documentation."""
        docs_dirs = [self.repo_root / 'docs', self.repo_root, 
                    self.repo_root / 'src', self.repo_root / 'scripts']
    ...
    def _check_code_accuracy(self):
        ...
        print(f"  Found {len(issues)} code accuracy issues")
    ...
    def _check_documented_commands(self):
        ...
        for md_file in [self.repo_root / 'README.md'] + list((self.repo_root / 'docs').glob('*.md')):
                ...
                for match in re.finditer(script_pattern, content):
                    ...
                    if not script_path.exists():
                        ...
                        for script_dir in ['scripts', 'repo_utilities']:
                            ...
                            if md_file.name == 'EXAMPLES.md' and 'Create' in content[max(0, match.start()-50):match.start()]:
                                continue
                            
                            if script_ref.endswith('.sh') or 'scripts/' in script_ref or 'repo_utilities/' in script_ref:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if 'FAILED' in result.stdout or 'ERROR' in result.stdout:
                    self.results.accuracy_issues.append(AccuracyIssue(
                        category='testing',
                        severity='error',
                        file='tests/',
                    ...
                    print("  ⚠️  Test suite has failures")
                else:
                    print("  ⚠️  Could not determine test status")
        except subprocess.TimeoutExpired:
            print("  ⚠️  Test suite timed out")
        except FileNotFoundError:
            print("  ⚠️  pytest not found (may need to install dependencies)")
        except Exception as e:
    ...
    def _check_thin_orchestrator_pattern(self):
        ...
        self.results.accuracy_issues.extend(issues)
        print(f"  Found {len(issues)} architecture pattern issues")
