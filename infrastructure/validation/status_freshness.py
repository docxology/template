"""Source-backed freshness validation for dated ``STATUS.md`` subsystem ledgers.

The ``scripts/gates/status_freshness.py`` gate is a thin CLI over this module.
It parses the nine-field subsystem verification table that ``STATUS.md`` keeps
and reports fail-closed findings when a row (or the ``**Last updated:**``
header) has gone stale, is dated in the future, or is missing required fields.
Line and column offsets are preserved so diagnostics map back to the source
document faithfully.
"""
