...
def decode_pdf_hex_strings(text: str) -> str:
    ...
    def decode_hex_match(match: re.Match) -> str:
        ...
        and converts it to the corresponding ASCII/Unicode character.

        Args:
            match: A regex Match object with group(1) containing a 2-character
                hex string (e.g., "45" for 'E', "78" for 'x').

        Returns:
            str: The decoded character if conversion succeeds, or the original
                matched string (e.g., "/x45") if conversion fails.
        """
...
def validate_pdf_rendering(pdf_path: Path, n_words: int = 200) -> Dict[str, Any]:
    ...
    report = {
        ...
        "summary": {"has_issues": issues["total_issues"] > 0, "word_count": word_count},
    }
