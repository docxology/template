...
- Per-exemplar `check_*` functions in `checks_exemplar.py` — function-name, coverage-floor, dead-link, test-class, `__all__`, and publication-metadata drift plus required-files/signpost/config-example-parity checks, and `check_metadata_export_current` (tracked `CITATION.cff` / `.zenodo.json` / `codemeta.json` must agree with `manuscript/config.yaml` on authorship names, ORCIDs, and concept DOI — expectations re-derived via `infrastructure.publishing.metadata_export`)
