"""Review generation functions for LLM manuscript review."""
...
from infrastructure.core.logging_utils import (
    get_logger, log_success, log_progress, log_with_spinner, StreamingProgress, Spinner
)
...
from infrastructure.llm.review_metrics import ReviewMetrics, ManuscriptMetrics, estimate_tokens
...
def extract_manuscript_text(pdf_path: Path) -> Tuple[Optional[str], ManuscriptMetrics]:
    ...
    """
    logger.info(f"Extracting text from manuscript: {pdf_path.name}")
    ...
    try:
        ...
        from infrastructure.core.logging_utils import format_error_with_suggestions
...
def generate_review_with_metrics(
    ...
    """
    logger.info(f"Generating {review_name}...")
...
def generate_translation(
    ...
    logger.info(f"Generating translation ({target_language})...")
