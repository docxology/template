    ...
    for line in suggestions.split("\n"):
        ...
        if in_high_priority and line.strip().startswith(
            ...
            if len(item) > 10 and item not in todos:
                todos.append(
                    f"[ ] {item[:100]}..." if len(item) > 100 else f"[ ] {item}"
                )
...
def calculate_format_compliance_summary(reviews: Dict[str, str]) -> str:
    ...
    compliance_rate = (
        ((total_reviews - conversational_count) / total_reviews) * 100
        if total_reviews > 0
        else 100
    )
...
"""
            ...
            if name.startswith("translation_"):
                ...
                logger.info(
                    f"  Saved translation ({lang_name}): {full_path} ({metrics.output_words:,} words)"
                )
            else:
...
"""
        ...
        for trans_key in translation_keys:
            ...
            lang_name = TRANSLATION_LANGUAGES.get(lang_code, lang_code)
            nav_rows.append(
                f"| [Translation ({lang_name})](#translation-{lang_code}) | Technical abstract in {lang_name} |"
            )
    ...
    try:
        ...
        for name, content in reviews.items():
            ...
            format_compliance_per_review[name] = {
                ...
                "conversational_phrases": len(
                    details.get("conversational_phrases", [])
                ),
            }

        # Calculate overall compliance rate
        total_reviews = len(reviews)
        compliant_reviews = sum(
            1 for r in format_compliance_per_review.values() if r["compliant"]
        )
        compliance_rate = (
            (compliant_reviews / total_reviews * 100) if total_reviews > 0 else 100
        )
        ...
        metadata = {
            ...
            "total_generation_time_seconds": round(
                session_metrics.total_generation_time, 2
            ),
            "config": {
...
"""
    ...
    try:
        ...
        if review_name.startswith("translation_"):
            ...
            lang_name = TRANSLATION_LANGUAGES.get(lang_code, lang_code)
            logger.info(
                f"✅ Saved: {full_path} ({metrics.output_words:,} words) - Translation ({lang_name})"
            )
        else:
            logger.info(f"✅ Saved: {full_path} ({metrics.output_words:,} words)")
...
def generate_review_summary(
    ...
    if other_files:
        ...
        for filepath in other_files:
            ...
            if word_count != "N/A":
                logger.info(
                    f"    • {full_path}: {size_kb:.1f} KB, {word_count:,} words"
                )
            else:
