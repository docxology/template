"""Symlink private lifecycle rules into the template checkout.
...
LIFECYCLE_LINK_DIRS: dict[str, str] = {
    WORKING_SUBDIR: "rules/working",
    ARCHIVE_SUBDIR: "rules/archive",
}
PROTECTED_NAMES: frozenset[str] = frozenset(Path(name).name for name in PUBLIC_RULE_NAMES)
CONFIG_FILENAME = ".rules_root"
ENV_VAR = "TEMPLATE_RULES_ROOT"
SKIP_ENV_VAR = "TEMPLATE_SKIP_RULE_LINK_SYNC"
IGNORED_LIFECYCLE_ENTRY_NAMES: frozenset[str] = frozenset({"output"})

RULES_LINK_CONFIG = SidecarLinkConfig(
...
def private_rules_root(repo_root: Path) -> Path | None:
    """Return the private rules root root path."""
    return resolve_private_root(repo_root, RULES_LINK_CONFIG)


def is_managed_symlink(path: Path, private_root: Path) -> bool:
    """Check whether managed symlink."""
    return _is_managed_symlink(path, private_root, RULES_LINK_CONFIG)


def sync_private_rule_links(
    repo_root: Path,
    ...
    *,
    ...
    """Synchronize private rule links."""
...
def sync_active_rule_links(
    ...
    *,
    ...
    """Synchronize active rule links."""
    return sync_private_rule_links(repo_root, private_root, prune=prune, dry_run=dry_run)


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m infrastructure.rules.linking",
        description="Symlink lifecycle rules from the private sidecar into the template checkout.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = sync_private_rule_links(
        repo_root,
        ns.private_root,
