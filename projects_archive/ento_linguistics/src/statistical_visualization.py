...
class StatisticalVisualizer(ConceptVisualizer):
    ...
    def visualize_statistical_significance(self, significance_results: Dict[str, Any],
        ...
        if 'p_value' in significance_results:
            ...
            # P-value bar chart
            ax1.bar(['P-Value', 'Threshold'], [p_val, threshold],
            ...
            ax1.text(0.5, max(p_val, threshold) * 0.8, status_text,
        ...
        if 'chi_square_statistic' in significance_results:
            ...
            ax2.bar(['Chi-Square', 'Effect Size'], [chi_sq, effect_size],
        ...
        if 'significant_patterns' in significance_results:
            ...
            if patterns:
                ...
                ax3.text(0.5, 0.5, 'No Significant Patterns Found',
        ...
        if 'effect_size' in significance_results:
            ...
            ax4.bar(['Effect Size'], [effect_size], color=category_colors[category_idx],
                   alpha=0.7, edgecolor='black', width=0.5)
            ax4.axhline(y=thresholds[category_idx], color=category_colors[category_idx],
                       linestyle='--', alpha=0.7, label=categories[category_idx])
            ...
            ax4.text(0, effect_size * 0.5, f'{categories[category_idx]}\n({effect_size:.3f})',
    ...
    def create_correlation_matrix_plot(self, correlation_data: Dict[str, Dict[str, float]],
                                     filepath: Optional[Path] = None,
    ...
    def visualize_distribution_comparison(self, distribution_data: Dict[str, List[float]],
                                        filepath: Optional[Path] = None,
        ...
        bp = ax1.boxplot(data_lists, labels=distributions, patch_artist=True)
        ...
        for i, (name, data) in enumerate(distribution_data.items()):
            ax3.hist(data, bins=20, alpha=0.7, label=name, color=colors[i], edgecolor='black')
    ...
    def visualize_effect_sizes(self, effect_size_data: Dict[str, Dict[str, float]],
                             filepath: Optional[Path] = None,
                             title: str = "Effect Size Analysis") -> plt.Figure:
        ...
        for i, (bar, es) in enumerate(zip(bars, effect_sizes)):
            ...
            ax.text(label_x, bar.get_y() + bar.get_height()/2,
                   '.2f', ha='left' if width >= 0 else 'right', va='center',
        ...
        # Add zone labels
        ax.text(-0.1, len(effect_sizes) * 0.9, 'Negligible',
               ha='center', va='center', fontsize=8, style='italic',
               bbox=dict(boxstyle='round,pad=0.2', facecolor='lightgray', alpha=0.7))
        ax.text(-0.35, len(effect_sizes) * 0.9, 'Small',
               ha='center', va='center', fontsize=8, style='italic',
               bbox=dict(boxstyle='round,pad=0.2', facecolor='lightblue', alpha=0.7))
        ax.text(-0.65, len(effect_sizes) * 0.9, 'Medium',
               ha='center', va='center', fontsize=8, style='italic',
               bbox=dict(boxstyle='round,pad=0.2', facecolor='orange', alpha=0.7))
        ax.text(-0.9, len(effect_sizes) * 0.8, 'Large',
               ha='center', va='center', fontsize=8, style='italic',
        ...
        ax.legend(handles=legend_elements, title='Effect Size Magnitude',
    ...
    def plot_confidence_intervals(self, ci_data: Dict[str, Dict[str, float]],
                                filepath: Optional[Path] = None,
                                title: str = "Confidence Intervals") -> plt.Figure:
        ...
        ax.errorbar(x_positions, estimates, yerr=yerr, fmt='o',
                   color='blue', ecolor='black', capsize=5, capthick=2,
        ...
        for i, (group, estimate) in enumerate(zip(groups, estimates)):
            ...
            if ci_lower > 0 or ci_upper < 0:
                # Significant result
                ax.plot(i, estimate, 'o', markersize=10, markerfacecolor='red',
                       markeredgecolor='black', markeredgewidth=2)
                ax.text(i, estimate + (ci_upper - estimate) * 0.1, '*',
        ...
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='blue', markersize=8,
                      label='Estimate', linestyle='None'),
            plt.Line2D([0], [0], marker='o', color='red', markersize=10,
                      markerfacecolor='red', markeredgecolor='black',
                      label='Statistically Significant', linestyle='None')
        ]
    ...
    def create_statistical_dashboard(self, dashboard_data: Dict[str, Any],
                                   filepath: Optional[Path] = None,
                                   title: str = "Statistical Analysis Dashboard") -> plt.Figure:
        ...
        if 'significance_results' in dashboard_data and plot_idx < len(axes):
            ...
            if 'p_value' in sig_data:
                ...
                axes[plot_idx].bar(['P-Value', 'Threshold'], [p_val, threshold],
        ...
        if 'distributions' in dashboard_data and plot_idx < len(axes):
            ...
            if len(dist_data) >= 2:
                ...
                axes[plot_idx].boxplot(data_lists, labels=dist_names)
        ...
        if 'confidence_intervals' in dashboard_data and plot_idx < len(axes):
            ...
            if ci_data:
                ...
                axes[plot_idx].errorbar(x_pos, estimates, yerr=[yerr_lower, yerr_upper],
                                      fmt='o', capsize=5)
    ...
    def visualize_hypothesis_testing(self, hypothesis_results: List[Dict[str, Any]],
                                   filepath: Optional[Path] = None,
                                   title: str = "Hypothesis Testing Results") -> plt.Figure:
        ...
        scatter = ax.scatter(p_values, effect_sizes, c=colors, s=sizes, alpha=0.7, edgecolors='black')

        # Add significance threshold line
        ax.axvline(x=0.05, color='red', linestyle='--', alpha=0.7, label='α = 0.05')

        # Add labels for significant results
        for i, (name, p_val, es, sig) in enumerate(zip(test_names, p_values, effect_sizes, significances)):
            if sig:
                ax.annotate(name, (p_val, es), xytext=(5, 5), textcoords='offset points',
        ...
        legend_elements = [
            ...
            plt.Line2D([0], [0], color='red', linestyle='--', label='Significance Threshold')
        ]
        ax.legend(handles=legend_elements, bbox_to_anchor=(1.05, 1), loc='upper left')
