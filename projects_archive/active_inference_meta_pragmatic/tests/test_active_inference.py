...
class TestActiveInferenceFramework:
    ...
    def test_policy_selection(self, framework):
        ...
        optimal_policy, selection_info = framework.select_optimal_policy(
            candidate_policies
        )
    ...
    def test_error_handling(self, framework):
        ...
        with pytest.raises(Exception):
            framework.calculate_expected_free_energy(
                np.array([-0.1, 1.1]), np.array([0])  # Invalid probabilities
            )
...
class TestTheoreticalValidation:
    ...
    def test_theoretical_correctness_validation(self, validator):
        ...
        implementation_result = {
            "efe_components": {"epistemic": 0.3, "pragmatic": 0.7, "total": 1.0},
            "posterior_beliefs": np.array([0.6, 0.4]),
        }

        theoretical_expectation = {
            "efe_expectation": {"epistemic_range": [0, 2], "pragmatic_range": [0, 2]},
            "inference_expectation": {"distribution_valid": True},
        }

        result = validator.validate_theoretical_correctness(
            "active_inference_test", implementation_result, theoretical_expectation
        )
...
class TestEdgeCases:
    ...
    def test_extreme_probabilities(self):
        ...
        efe_total, _ = framework.calculate_expected_free_energy(
            confident_posterior, policy
        )
        assert np.isfinite(efe_total)

        # Test with uniform posterior
        uniform_posterior = np.array([0.5, 0.5])
        efe_total2, _ = framework.calculate_expected_free_energy(
            uniform_posterior, policy
        )
    ...
    def test_policy_non_integer_dtype(self):
        """Test policy validation with non-integer dtype."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        posterior = np.array([0.5, 0.5])
        float_policy = np.array([0.5])  # Float instead of integer

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(posterior, float_policy)

        assert "integer" in str(exc_info.value).lower()

    def test_policy_out_of_range(self):
        """Test policy validation with out-of-range actions."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        posterior = np.array([0.5, 0.5])
        invalid_policy = np.array([10])  # Action index too high

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(posterior, invalid_policy)

        assert (
            "range" in str(exc_info.value).lower()
            or "actions" in str(exc_info.value).lower()
        )

    def test_posterior_nan_values(self):
        """Test posterior validation with NaN values."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        nan_posterior = np.array([0.5, np.nan])
        policy = np.array([0])

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(nan_posterior, policy)

        assert "NaN" in str(exc_info.value) or "infinite" in str(exc_info.value).lower()

    def test_posterior_inf_values(self):
        """Test posterior validation with infinite values."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        inf_posterior = np.array([0.5, np.inf])
        policy = np.array([0])

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(inf_posterior, policy)

        assert "infinite" in str(exc_info.value).lower() or "NaN" in str(exc_info.value)

    def test_posterior_not_normalized(self):
        """Test posterior validation with non-normalized distribution."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        non_normalized = np.array([0.5, 0.6])  # Sums to 1.1
        policy = np.array([0])

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(non_normalized, policy)

        assert "sum" in str(exc_info.value).lower()

    def test_missing_matrix_initialization(self):
        """Test initialization with missing required matrix."""
        incomplete_model = {
        ...
        with pytest.raises(ValidationError) as exc_info:
            ActiveInferenceFramework(incomplete_model)

        assert "Missing required matrix" in str(exc_info.value)

    def test_A_matrix_wrong_dimensions(self):
        """Test initialization with A matrix having wrong dimensions."""
        invalid_model = {
            "A": np.array([0.8, 0.2]),  # 1D instead of 2D
            "B": np.array([[[0.9, 0.1], [0.1, 0.9]]]),
            "C": np.array([1.0, -1.0]),
            "D": np.array([0.5, 0.5]),
        }
        ...
        with pytest.raises(ValidationError) as exc_info:
            ActiveInferenceFramework(invalid_model)

        assert (
            "2-dimensional" in str(exc_info.value)
            or "dimension" in str(exc_info.value).lower()
        )

    def test_negative_policy_actions(self):
        """Test policy validation with negative action indices."""
        model = create_simple_generative_model()
        framework = ActiveInferenceFramework(model)
        posterior = np.array([0.5, 0.5])
        negative_policy = np.array([-1])

        with pytest.raises(ValidationError) as exc_info:
            framework.calculate_expected_free_energy(posterior, negative_policy)
