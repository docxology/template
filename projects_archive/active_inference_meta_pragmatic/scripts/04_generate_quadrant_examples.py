...
def create_quadrant_1_example(
    ...
    fig.suptitle(
        "Quadrant 1: Data Processing (Cognitive) - EFE in Action", fontweight="bold"
    )
    ...
    bars = ax2.bar(
        policy_labels, efe_values, color=["lightblue", "lightcoral"], alpha=0.7
    )
...
def create_quadrant_2_example(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
    """Create Quadrant 2 example: Meta-data organization."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))
    fig.suptitle(
        "Quadrant 2: Meta-Data Organization (Cognitive) - Enhanced Processing",
        fontweight="bold",
    )
    ...
    ax1.fill_between(
        ...
        base_data,
        quality_weighted,
        where=(quality_weighted > base_data),
        alpha=0.3,
        color="red",
    )
...
def create_quadrant_3_example(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
    """Create Quadrant 3 example: Reflective processing (meta-cognitive)."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8))
    fig.suptitle(
        "Quadrant 3: Reflective Processing (Meta-Cognitive) - Self-Monitoring",
        fontweight="bold",
    )
...
def create_quadrant_4_example(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
    """Create Quadrant 4 example: Higher-order reasoning."""
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.set_title(
        "Quadrant 4: Higher-Order Reasoning (Meta-Cognitive) - Framework Adaptation",
        fontweight="bold",
        pad=20,
    )
    ax.set_xlim(0, 12)
    ...
    levels = [
        {
            "name": "Framework Level",
            "y": 6.5,
            "color": "red",
            "desc": "Meta-epistemic/meta-pragmatic",
        },
        {
            "name": "Strategy Level",
            "y": 4.5,
    ...
    ax.arrow(
        6, 6.2, 0, -1.2, head_width=0.1, head_length=0.2, fc="red", ec="red", alpha=0.7
    )
    ...
    for i, example in enumerate(examples):
        ...
        ax.text(
            6, y_pos, f"• {example}", ha="center", va="center", fontsize=10, color=color
        )
