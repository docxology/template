...
def create_system_boundary_diagram(
    ...
    ax.set_title(
        "Free Energy Principle: System Boundaries and Markov Blankets",
        fontweight="bold",
        pad=20,
    )
    ax.set_xlim(0, 12)
    ...
    ax.add_patch(
        plt.Rectangle(
            (3, 2.5), 6, 3, fill=True, alpha=0.2, edgecolor="red", linewidth=3
        )
    )
    ax.text(
        6,
    ...
    # Draw sensory states
    ax.add_patch(
        plt.Rectangle((2.5, 3.5), 1.5, 1, fill=True, alpha=0.4, color="orange")
    )
    ax.text(
        3.25,
        4,
    ...
    # Draw active states
    ax.add_patch(
        plt.Rectangle((8.0, 3.5), 1.5, 1, fill=True, alpha=0.4, color="purple")
    )
    ax.text(
        8.75,
        4,
    ...
    # Sensory to internal
    ax.arrow(
        4, 4, 1.8, 0, head_width=0.1, head_length=0.2, fc="blue", ec="blue", alpha=0.7
    )
    ...
    # Active to external
    ax.arrow(
        9.5, 4, 0.5, 0, head_width=0.1, head_length=0.2, fc="red", ec="red", alpha=0.7
    )
...
def create_free_energy_dynamics(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
...
def create_structure_preservation(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
    ...
    fig.suptitle(
        "Structure Preservation in Free Energy Minimization", fontweight="bold"
    )
    ...
    for i in range(internal_states.shape[1]):
        axes[0].plot(
            range(len(internal_states)),
            internal_states[:, i],
            label=f"Internal State {i+1}",
            linewidth=2,
        )
...
def create_physics_cognition_bridge(
    viz_engine: VisualizationEngine, figure_manager: FigureManager
) -> None:
    """Create physics-cognition bridge visualization."""
    fig, ax = plt.subplots(figsize=(14, 8))
    ax.set_title(
        "Free Energy Principle: Bridge Between Physics and Cognition",
        fontweight="bold",
        pad=20,
    )
    ax.set_xlim(0, 14)
    ...
    ax.text(
        7,
        1,
        "Active Inference applies FEP to cognitive systems",
        ha="center",
        va="center",
        fontsize=13,
        fontweight="bold",
