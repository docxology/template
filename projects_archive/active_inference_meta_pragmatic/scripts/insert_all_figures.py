...
def map_section_to_file(section: str) -> Path:
    ...
    filename = section_mapping.get(
        section, "03_quadrant_model.md"
    )  # Default to quadrant model
...
def get_section_insertion_points(manuscript_file: Path) -> Dict[str, str]:
    ...
    if "03_quadrant_model.md" in str(manuscript_file):
    ...
    elif "04_security_implications.md" in str(manuscript_file):
...
def insert_figures_into_manuscript() -> bool:
    ...
    for manuscript_file, figure_labels in figures_by_file.items():
        logger.info(
            f"Processing {len(figure_labels)} figures for {manuscript_file.name}"
        )
        ...
        for figure_label in figure_labels:
            ...
            try:
                ...
                success = insert_figure_directly(
                    manuscript_file, figure_label, section_name, registry[figure_label]
                )
    ...
    if successful_insertions == 0 and failed_insertions > 0:
        ...
        logger.warning(
            f"⚠️  {failed_insertions} figures failed to insert (may already exist in other files)"
        )
...
def insert_figure_directly(
    manuscript_file: Path, figure_label: str, section_name: str, figure_data: Dict
) -> bool:
    ...
    try:
        ...
        if figure_exists_in_manuscript(figure_label, manuscript_dir):
            logger.info(
                f"    ⏭️  Figure {figure_label} already exists in manuscript, skipping insertion"
            )
            return True
        ...
        if not match:
            ...
            flexible_pattern = flexible_pattern.replace(r"\\\)\|×\|\)", r"\\\)|×|)")
            flexible_pattern = flexible_pattern.replace(
                r"\(?:\\\\times\|×\)", r"(?:\\times|×)"
            )
        ...
        if not match:
            logger.debug(
                f"Section '{section_name}' not found in {manuscript_file.name}"
            )
            return False
...
def update_figure_references() -> bool:
    ...
    manuscript_files = [
        manuscript_dir / "02_background.md",
        manuscript_dir / "03_quadrant_model.md",
        manuscript_dir / "04_security_implications.md",
        manuscript_dir / "05_discussion.md",
    ]
...
def main() -> int:
    ...
    logger.info(
        f"Figure registry: {project_root / 'output' / 'figures' / 'figure_registry.json'}"
    )
    ...
    if validation_success:
        ...
        logger.info(
            "The manuscript now contains all registered figures as LaTeX blocks."
        )
        logger.info("PDF rendering should now include all figures.")
        return 0
    else:
        logger.warning(
            "Figure validation found some issues - but figures may exist in other files"
        )
