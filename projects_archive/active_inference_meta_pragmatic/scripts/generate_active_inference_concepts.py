...
def create_efe_decomposition_diagram(viz_engine: VisualizationEngine,
    ...
    y_pos = 0.8
    for eq in equations:
        ax2.text(0.05, y_pos, eq, fontsize=11, verticalalignment='top',
...
def create_perception_action_loop(viz_engine: VisualizationEngine,
                                figure_manager: FigureManager) -> None:
    ...
    ax.arrow(arrow_x, arrow_y, 0.3*np.cos(arrow_theta), 0.3*np.sin(arrow_theta),
    ...
    for label, angle, r in processes:
        ...
        ha = 'center' if abs(np.cos(angle)) < 0.5 else ('right' if np.cos(angle) > 0 else 'left')
        va = 'center' if abs(np.sin(angle)) < 0.5 else ('top' if np.sin(angle) > 0 else 'bottom')
        ax.text(x_pos, y_pos, label, ha=ha, va=va, fontsize=11, fontweight='bold')

    # Add internal components
    ax.text(center_x, center_y, 'Generative\nModel', ha='center', va='center',
            fontsize=12, fontweight='bold', bbox=dict(boxstyle="round,pad=0.3",
            facecolor='lightgreen', alpha=0.3))

    # Add EFE calculation
    ax.text(center_x, center_y - 1.5, 'EFE(π) = G(π) + H[Q(π)]',
            ha='center', va='center', fontsize=10,
            bbox=dict(boxstyle="round,pad=0.3", facecolor='lightyellow', alpha=0.5))

    # Add explanatory arrows
    ax.annotate('Minimize EFE', xy=(center_x, center_y + 0.5), xytext=(center_x + 1.5, center_y + 1.5),
...
def create_generative_model_structure(viz_engine: VisualizationEngine,
                                    figure_manager: FigureManager) -> None:
    """Create generative model structure visualization."""
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.set_title('Generative Model Structure in Active Inference', fontweight='bold', pad=20)
    ax.set_xlim(0, 12)
    ax.set_ylim(0, 8)
    ax.axis('off')
    ...
    for matrix in matrices:
        ...
        shape_str = f"shape: {shape}" if isinstance(shape, tuple) else f"shape: ({shape},)"
        ax.text(x, y - 0.8, shape_str, ha='center', va='center', fontsize=8, style='italic')
    ...
    for (x1, y1), (x2, y2), label in connections:
        ax.arrow(x1, y1, x2-x1, y2-y1, head_width=0.1, head_length=0.2,
    ...
    """
    ax.text(6, 2, explanation, ha='center', va='center', fontsize=9,
...
def create_meta_level_concepts(viz_engine: VisualizationEngine,
                             figure_manager: FigureManager) -> None:
    """Create meta-level concepts visualization."""
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Meta-Pragmatic and Meta-Epistemic Aspects of Active Inference',
    ...
    for concept in concepts:
        ...
        ax.add_patch(plt.Rectangle((0, 0), 1, 1, transform=ax.transAxes,
                                 facecolor=concept['color'], alpha=0.1))

        # Add title
        ax.text(0.5, 0.9, concept['title'], ha='center', va='top',
               fontsize=12, fontweight='bold', transform=ax.transAxes)

        # Add description
        ax.text(0.5, 0.7, concept['description'], ha='center', va='top',
               fontsize=10, wrap=True, transform=ax.transAxes)

        # Add examples
        examples_text = "Examples:\n" + "\n".join(f"• {ex}" for ex in concept['examples'])
        ax.text(0.5, 0.4, examples_text, ha='center', va='top',
