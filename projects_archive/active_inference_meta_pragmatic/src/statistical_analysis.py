...
class StatisticalAnalyzer:
    ...
    def calculate_descriptive_stats(self, data: NDArray) -> Dict[str, Union[float, int]]:
        ...
        try:
            ...
            if data.size < 2:
                raise ValidationError(f"Need at least 2 data points for statistical calculations, got {data.size}")
    ...
    def calculate_confidence_interval(self, data: NDArray,
                                    ...
                                    confidence: Optional[float] = None) -> Dict[str, Union[float, List[float]]]:
        ...
        try:
            ...
            # Validate input data
            if data.size == 0:
                raise ValidationError("Cannot calculate confidence interval for empty data")

            # Handle parameter name compatibility
            if confidence_level is not None and confidence is not None:
                raise ValidationError("Cannot specify both 'confidence_level' and 'confidence' parameters")
            ...
            if n == 1:
                ...
                logger.debug(f"Calculated {conf_level*100:.0f}% CI for single value: mean={mean:.3f}")
            ...
            logger.debug(f"Calculated {conf_level*100:.0f}% CI: mean={mean:.3f}, range=[{ci_lower:.3f}, {ci_upper:.3f}]")
            return confidence_interval
    ...
    def perform_t_test(self, group1: NDArray, group2: NDArray,
                      test_type: str = "independent") -> Dict[str, Union[float, bool, str]]:
        ...
        try:
            if test_type not in ["paired", "independent"]:
                raise ValidationError(f"test_type must be 'paired' or 'independent', got '{test_type}'")

            g1, g2 = np.asarray(group1), np.asarray(group2)

            # Validate minimum sample sizes to prevent degrees of freedom issues
            if len(g1) < 2:
                raise ValidationError(f"Group 1 must have at least 2 observations, got {len(g1)}")
            if len(g2) < 2:
                raise ValidationError(f"Group 2 must have at least 2 observations, got {len(g2)}")

            if test_type == "paired":
                if len(g1) != len(g2):
                    raise ValidationError(f"Paired t-test requires equal sample sizes, got {len(g1)} vs {len(g2)}")
            ...
            logger.debug(f"{test_name}: t={t_stat:.3f}, p={p_value:.3f}, d={cohens_d:.3f}")
