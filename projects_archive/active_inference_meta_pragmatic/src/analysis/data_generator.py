...
class DataGenerator:
    ...
    def generate_time_series(
        ...
        try:
            ...
            # Add noise
            if seed is not None:
                # Use provided seed for this specific generation
    ...
    def generate_categorical_observations(
        self, n_samples: int = 100, n_categories: int = 3, distribution: str = "uniform"
    ) -> NDArray:
        ...
        try:
            ...
            logger.debug(
                f"Generated categorical observations: {n_samples} samples, {n_categories} categories"
            )
            return observations
            ...
            raise ValidationError(
                f"Categorical observation generation failed: {e}"
            ) from e
    ...
    def generate_state_sequences(
        ...
        try:
            ...
            logger.debug(
                f"Generated state sequences: {n_sequences} sequences, length {sequence_length}"
            )
            return sequences
    ...
    def generate_observation_matrix(
        self, n_states: int = 3, n_observations: int = 4, noise_level: float = 0.1
    ) -> NDArray:
    ...
    def generate_transition_matrix(
        self, n_states: int = 3, n_actions: int = 2, transition_type: str = "controlled"
    ) -> NDArray:
        ...
        try:
            ...
            logger.debug(
                f"Generated transition matrix: {n_states}×{n_states}×{n_actions}"
            )
            return B
    ...
    def generate_preference_vector(
        self, n_observations: int = 4, preference_type: str = "simple"
    ) -> NDArray:
        ...
        try:
            if distribution == "gaussian":
                ...
                for c in range(n_classes):
                    ...
                    end_idx = (
                        (c + 1) * samples_per_class if c < n_classes - 1 else n_samples
                    )

                    # Class centers in a circle (in n_features dimensions)
                    angle = 2 * np.pi * c / n_classes
                    center = np.zeros(n_features)
                    center[0] = np.cos(angle) * 2  # x-coordinate
                    if n_features > 1:
                        center[1] = np.sin(angle) * 2  # y-coordinate
                    # Other dimensions remain 0

                    # Generate samples around center
                    n_class_samples = end_idx - start_idx
                    cov_matrix = (
                        np.eye(n_features) * 0.5
                    )  # n_features x n_features covariance matrix
                    class_features = self.rng.multivariate_normal(
                        center, cov_matrix, n_class_samples
                    )
            ...
            logger.debug(
                f"Created synthetic dataset: {n_samples} samples, {n_features} features, {n_classes} classes"
            )
...
def demonstrate_data_generation() -> Dict[str, Union[str, Dict]]:
    ...
    values = generator.generate_time_series(n_points=50, trend="sinusoidal")
    time = np.arange(len(values))

    # Generate categorical observations
    observations = generator.generate_categorical_observations(
        n_samples=30, n_categories=3
    )
