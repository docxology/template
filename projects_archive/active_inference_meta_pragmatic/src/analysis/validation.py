...
class ValidationFramework:
    ...
    def validate_probability_distribution(
        self, distribution: NDArray, name: str = "distribution"
        ...
        try:
            ...
            if distribution.ndim == 1:
                ...
                if not np.allclose(row_sums, 1.0, atol=self.tolerance):
                    bad_rows = np.where(
                        ~np.isclose(row_sums, 1.0, atol=self.tolerance)
                    )[0]
                    raise ValidationError(
                        f"{name} rows are not properly normalized: rows {bad_rows} sum to {row_sums[bad_rows]}"
                    )
                ...
                raise ValidationError(
                    f"{name} has unsupported dimensionality {distribution.ndim}"
                )

            # Check overall normalization
            if not np.isclose(total, expected_sum, atol=self.tolerance):
                raise ValidationError(
                    f"{name} is not properly normalized (sum = {total:.6f}, expected = {expected_sum:.6f})"
                )
            ...
            raise ValidationError(
                f"Probability distribution validation failed: {e}"
            ) from e

    def validate_generative_model(
        self, model: Dict[str, NDArray]
        ...
        try:
            ...
            if A.shape[1] != n_states:
                raise ValidationError(
                    f"A matrix columns ({A.shape[1]}) must match D length ({n_states})"
                )

            # C matrix compatibility
            if C.shape[0] != n_observations:
                raise ValidationError(
                    f"C matrix length ({C.shape[0]}) must match A rows ({n_observations})"
                )
            ...
            if len(B.shape) == 3:
                ...
                for action in range(B.shape[2]):
                    ...
                    self.validate_probability_distribution(
                        transition_matrix, f"B[:, :, {action}]"
                    )
            ...
            for state in range(n_states):
                ...
                self.validate_probability_distribution(obs_likelihood, f"A[:, {state}]")
            ...
            logger.info(
                f"Validated generative model: {n_states} states, {n_observations} observations"
            )
    ...
    def validate_theoretical_correctness(
        self,
        algorithm_name: str,
        implementation_result: Dict,
        theoretical_expectation: Dict,
    ) -> Dict[str, Union[bool, str, Dict]]:
        ...
        try:
            ...
            if "posterior_beliefs" in implementation_result:
                ...
                validation_results["theoretical_checks"][
                    "inference"
            ...
            stability_validation = self._validate_numerical_stability(
                implementation_result
            )
            validation_results["implementation_checks"][
                "numerical_stability"
            ...
            validation_results["message"] = (
                "Theoretical correctness validated"
                if validation_results["overall_correctness"]
                else f"Correctness issues found: {len(validation_results['issues'])} issues"
            )

            logger.info(
                f"Validated theoretical correctness for {algorithm_name}: "
            ...
            raise ValidationError(
                f"Theoretical correctness validation failed: {e}"
            ) from e

    def _validate_efe_correctness(
        self, efe_components: Dict, expectation: Dict
    ...
    def _validate_inference_correctness(
        self, posterior: NDArray, expectation: Dict
    ) -> Dict[str, Union[bool, List]]:
    ...
    def validate_algorithm_performance(
        self,
        performance_metrics: Dict[str, Union[float, List]],
        requirements: Dict[str, Union[float, Tuple[float, float]]],
    ) -> Dict[str, Union[bool, str, Dict]]:
        ...
        try:
            ...
            logger.info(
                f"Validated algorithm performance: "
            ...
            raise ValidationError(
                f"Algorithm performance validation failed: {e}"
            ) from e
    ...
    def create_validation_report(
        ...
        try:
            report = {
                "summary": {
                    ...
                    "passed": sum(
                        ...
                        if r.get("valid", r.get("overall_pass", False))
                    ),
                    "failed": sum(
                        1
                        for r in validation_results
                        if not r.get("valid", r.get("overall_pass", True))
                    ),
                    "overall_status": "PASS",
                },
            ...
            logger.info(
                f"Created validation report: {report['summary']['overall_status']} "
...
def demonstrate_validation_framework() -> Dict[str, Union[str, Dict]]:
    ...
    try:
        valid_result = framework.validate_probability_distribution(
            valid_dist, "test_valid"
        )
    except ValidationError:
        valid_result = {"valid": False, "error": "Validation failed"}

    try:
        invalid_result = framework.validate_probability_distribution(
            invalid_dist, "test_invalid"
        )
    except ValidationError as e:
