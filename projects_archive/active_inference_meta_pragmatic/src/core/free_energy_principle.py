...
class FreeEnergyPrinciple:
    ...
    def __init__(
        self, system_states: Dict[str, NDArray], precision: float = 1.0
    ) -> None:
        ...
        if len(state_shape_set) > 1:
            ...
            logger.debug(
                f"Normalized {len(corrected_states)} system states to shape {max_shape}"
            )
        else:
        ...
        logger.info(
            f"Initialized Free Energy Principle framework with precision {precision}"
        )
    ...
    def _calculate_energy(self, observations: NDArray, beliefs: NDArray) -> float:
        ...
        if np.any(observations < 0) or np.any(observations > 1):
            raise ValidationError(
                "Observations must be valid probabilities (0 ≤ p ≤ 1)"
            )
    ...
    def demonstrate_structure_preservation(
        ...
        try:
            ...
            logger.info(
                f"Demonstrated structure preservation over {time_steps} time steps"
            )
            return result
            ...
            raise ValidationError(
                f"Structure preservation demonstration failed: {e}"
            ) from e
    ...
    def _generate_observations(self, internal: NDArray, external: NDArray) -> NDArray:
        ...
        return np.clip(
            internal_trunc + external_trunc + np.random.normal(0, 0.1, min_len), 0, 1
        )
    ...
    def _update_internal_state(self, internal: NDArray, beliefs: NDArray) -> NDArray:
        ...
        if len(beliefs) < len(internal):
            ...
            beliefs_expanded = np.pad(
                beliefs, (0, len(internal) - len(beliefs)), "constant"
            )
        else:
...
def demonstrate_fep_concepts() -> Dict[str, Union[str, Dict]]:
    ...
    system_states = {
        ...
        "sensory": np.array(
            [0.4, 0.6, 0.0]
        ),  # Sensory inputs (match internal dimension)
    }
    ...
    demonstration = {
        ...
        "structure_preservation": {
            ...
            "converged": bool(
                np.std(preservation_demo["free_energy_history"][-10:]) < 0.01
            ),
        },
