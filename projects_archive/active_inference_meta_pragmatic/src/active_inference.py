...
class ActiveInferenceFramework:
    ...
    def __init__(self, generative_model: Union[Dict[str, NDArray], "GenerativeModel"], precision: float = 1.0) -> None:
        ...
        self.n_actions = self.generative_model['B'].shape[2] if len(self.generative_model['B'].shape) > 2 else 1
    ...
    def calculate_expected_free_energy(
        ...
        try:
            ...
            logger.debug(f"EFE calculation: epistemic={epistemic:.4f}, pragmatic={pragmatic:.4f}, total={total_efe:.4f}")
    ...
    def select_optimal_policy(self, candidate_policies: List[NDArray]) -> Tuple[NDArray, Dict]:
        ...
        try:
            ...
            optimal_policy_key = min(policy_scores.keys(),
                                   key=lambda k: policy_scores[k]['efe_total'])
