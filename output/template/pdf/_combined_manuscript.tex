...
separates 13 reusable infrastructure subpackages (\textasciitilde277
Python modules, validated by \textasciitilde6,028 tests) from
...
890 tests), and this self-referential architectural analysis
...
\item
  ...
  scripts, data). 13 infrastructure subpackages comprising
  \textasciitilde277 Python modules provide reusable services; projects
...
\item
  ...
  test doubles. \textasciitilde6,028 infrastructure tests and 326+
...
ten-stage DAG pipeline whose architecture is the subject of the
...
\item
  A detailed specification of the ten-stage DAG-based build pipeline
  (Stages 00--07), from environment sanitization through executive
  report generation.
\item
...
\item
  ...
  \texttt{cognitive\_case\_diagrams}, \texttt{template}), demonstrating
...
Module Reference} provides detailed documentation for all 13
subpackages.
...
\subsection{The Ten-Stage DAG
Pipeline}\label{the-ten-stage-dag-pipeline}
...
test suite alone contains \textasciitilde6,028 tests across 346+ test
files.
...
\texttt{run.sh}: it executes the standard ten-stage DAG pipeline and
then appends steganographic post-processing. For each rendered PDF, it
...
compose these MCP tools to reproduce the entire ten-stage DAG pipeline
...
aligned with these frameworks, the ten-stage DAG pipeline provides the
...
\tightlist
\item
  \textbf{Infrastructure tests} (\texttt{tests/}): \textasciitilde6,028
  tests validating the 13 infrastructure subpackages, covering logging,
...
\item
  ...
  (\texttt{code\_project}) to 890 tests
  (\texttt{cognitive\_case\_diagrams}).
\item
...
All three projects were executed through the full ten-stage DAG pipeline
...
\texttt{cognitive\_case\_diagrams} & 7 & \textasciitilde60s & 890/890 &
90\%+ \\
\texttt{template} & 7 & \textasciitilde25s & 66/66 & 90\%+ \\
\end{longtable}
}
...
The \texttt{cognitive\_case\_diagrams} project, with its 890 tests and
...
\endlastfoot
Test files & 346+ \\
Total tests & \textasciitilde6,028 \\
...
This test suite exercises all 13 infrastructure modules, including the
...
\bottomrule\noalign{}
\endlastfoot
\texttt{config} & 0 & ✓ & ✓ & Configuration schemas \\
\texttt{core} & 71 & ✓ & ✓ & \texttt{get\_logger},
\texttt{load\_config}, \texttt{TemplateError} \\
\texttt{docker} & 0 & ✓ & ✓ & Containerisation \\
\texttt{documentation} & 6 & ✓ & ✓ & \texttt{FigureManager},
\texttt{generate\_glossary} \\
\texttt{llm} & 53 & ✓ & ✓ & LLM review, literature search,
translation \\
\texttt{project} & 5 & ✓ & ✓ & \texttt{discover\_projects}, workspace
management \\
\texttt{publishing} & 14 & ✓ & ✓ & Citation generation (APA, BibTeX,
MLA), Zenodo \\
\texttt{rendering} & 27 & ✓ & ✓ & PDF rendering, Pandoc filters, HTML
reports \\
\texttt{reporting} & 45 & ✓ & ✓ & Coverage parsing, executive reports \\
\texttt{scientific} & 6 & ✓ & ✓ & \texttt{check\_numerical\_stability},
\texttt{benchmark\_function} \\
\texttt{skills} & 4 & ✓ & ✓ & --- \\
\texttt{steganography} & 10 & ✓ & ✓ & Metadata injection, QR overlays,
hashing \\
\texttt{validation} & 36 & ✓ & ✓ & PDF validation, Markdown checking,
CLI \\
\end{longtable}
}

All 13 modules have 100\% documentation coverage at Tiers 1--2
...
The three-tier
\href{03c_documentation.md\#agentic-skill-architecture}{skill protocol}
achieves complete coverage across all infrastructure modules:
...
Structural (\texttt{AGENTS.md}) & 13+ per-directory & 100\% \\
Skill (\texttt{SKILL.md}) & 13 modules & 100\% \\
PAI (\texttt{PAI.md}) & 1 (infrastructure-level) & --- \\
Human (\texttt{README.md}) & 13+ per-directory & 100\% \\
\end{longtable}
}

This four-layer coverage creates 13 fully described, MCP-mappable tool
endpoints that a sufficiently capable agent could invoke without any
source-code access. The aggregate documentation footprint (163+ files)
...
1.5s \\
\texttt{cognitive\_case\_diagrams} & \textasciitilde80 & \textless{}
0.3s & \textless{} 0.05s & \textless{} 2.0s & \textless{} 0.4s &
\textless{} 2.8s \\
...
time by the metric-placeholder system (\texttt{string.Template}--style
tokens such as \texttt{module\_count},
\texttt{infra\_test\_count\_approx}) described in the Methods, reading
live metrics from the repository's own structure. The figures below were
rendered by the same \texttt{architecture\_viz.py} module whose
font-size constraints are specified in the
\href{03e_quality.md\#quality-assurance}{Quality Assurance} section. In
this way, the manuscript does not merely \emph{describe} but
\emph{enacts} the pipeline it documents.
...
reusable infrastructure layer (13 subpackages, upper panel) from
domain-specific project workspaces (lower panel), connected by the
ten-stage DAG pipeline. Each module box displays its Python file count
and a documentation badge (A = \texttt{AGENTS.md}, R =
\texttt{README.md}, S = \texttt{SKILL.md}, P = \texttt{PAI.md}; a dot
\texttt{·} means absent). Project boxes show chapter and test counts.
All labels are drawn from live repository introspection at render time;
font sizes follow the 16 pt
\href{03e_quality.md\#visualization-standards}{accessibility floor}.

\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline Stage Flow}]{../figures/pipeline_stages.png}}
\textbf{Figure 2}: Sequential ten-stage DAG-based build pipeline (Stage
00--07, plus a pre-step clean stage). Viridis colour progression encodes
stage ordering. Each box includes a short description of the stage's
primary action. Stage names and descriptions are generated from
...
knowledge, the only open-source system that integrates all fourteen
...
duration: the full infrastructure test suite (\textasciitilde6,028
...
\item
  \textbf{\texttt{cognitive\_case\_diagrams}}: Compositional approaches
  to linguistic case for cognitive modeling, 890 tests, 90\%+ coverage,
  spanning enriched category theory, DisCoPy renderers, and categorical
  grammar diagrams. Demonstrates the template's capacity for
  computationally intensive, diagram-heavy research workflows with large
  test suites.
\item
  \textbf{\texttt{template}}: This self-referential architectural
  analysis, 66 tests, 90\%+ coverage. Demonstrates the system's ability
...
infrastructure layer---13 modules, \textasciitilde277 Python
...
\endlastfoot
Source modules & 1 &
\$\{project\_cognitive\_case\_diagrams\_src\_module\_count\} & 4 \\
Test files & 1 & 48 & 3 \\
Test count & 39 & 890 & 66 \\
Manuscript chapters & 8 &
\$\{project\_cognitive\_case\_diagrams\_chapter\_count\} & 21 \\
Analysis scripts & 3 &
\$\{project\_cognitive\_case\_diagrams\_script\_count\} & 2 \\
Figures (auto-generated) & 6 &
\$\{project\_cognitive\_case\_diagrams\_figure\_count\} & 4 \\
\end{longtable}
}

The infrastructure overhead per project is constant regardless of
project size: the same 13 modules, the same 8 pipeline stages, the same
...
signatures. All 13 infrastructure modules carry \texttt{AGENTS.md} and
...
\subsection{\texorpdfstring{\texttt{infrastructure.core} (71
modules)}{infrastructure.core (71 modules)}}\label{infrastructure.core-71-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.llm} (53
modules)}{infrastructure.llm (53 modules)}}\label{infrastructure.llm-53-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.project} (5
modules)}{infrastructure.project (5 modules)}}\label{infrastructure.project-5-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.publishing} (14
modules)}{infrastructure.publishing (14 modules)}}\label{infrastructure.publishing-14-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.rendering} (27
modules)}{infrastructure.rendering (27 modules)}}\label{infrastructure.rendering-27-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.reporting} (45
modules)}{infrastructure.reporting (45 modules)}}\label{infrastructure.reporting-45-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.steganography} (10
modules)}{infrastructure.steganography (10 modules)}}\label{infrastructure.steganography-10-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.validation} (36
modules)}{infrastructure.validation (36 modules)}}\label{infrastructure.validation-36-modules}
...
\subsection{\texorpdfstring{\texttt{infrastructure.skills} (4
modules)}{infrastructure.skills (4 modules)}}\label{infrastructure.skills-4-modules}
...
evaluator \citep{garijo2024fairsoft}.
...
\tightlist
\item
  Execute the standard ten-stage DAG pipeline for the target project.
\item
...
modeling & \$\{project\_cognitive\_case\_diagrams\_src\_module\_count\}
modules & 890 & 90\%+ & \textasciitilde60 \\
