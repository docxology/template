...
\title{Editorial Quality at Scale: A Reproducible Prose-Review Pipeline\\\normalsize Readability, structural, and citation-density gating for research manuscripts}
...
totalling 1802 words across 86 sentence(s) and 63 paragraph(s). Average
Flesch-Kincaid grade level is 16.58; average Gunning Fog index is 17.56;
...
configurable} exemplar (\breaktt{projects/template\_prose\_project/})
that wires it to the bibliography and the manuscript pipeline. Together
with \breaktt{template\_code\_project} (numerical research) and the
optional \breaktt{projects\_archive/template\_search\_project} add-on
(literature discovery), it covers the dominant shapes of academic
research projects in the template.
...
configurable, infrastructure-backed pipeline}. Like the permanent
sibling exemplar
\href{../../template_code_project}{\breaktt{template\_code\_project}}
(numerical research) and the optional search add-on
\href{../../../projects_archive/template_search_project}{\breaktt{template\_search\_project}}
(literature discovery), this project carries no novel research
contribution of its own; its purpose is to show how to compose existing
template infrastructure into a complete, reproducible research workflow
--- in this case, an editorial workflow.
...
\breaktt{src/pipeline.py::run\_prose\_pipeline} reads the manuscript
directory, calls
\href{../../../infrastructure/prose/SKILL.md}{\breaktt{infrastructure.prose.analyze\_manuscript}}
to produce a \texttt{ManuscriptReport}, cross-checks the cited keys
against
...
from doing the prose work alongside the code and literature exemplars in
the same template (sec.~\ref{sec:conclusion}).
...
The results are aggregated into a \texttt{ManuscriptReport} whose JSON
...
Each check produces a \texttt{CheckResult(passed,\ message,\ details)};
...
\item
  ...
  \texttt{ManuscriptReport}), \breaktt{output/checks.json} (the list of
...
\item
  \textbf{\texttt{every\_file\_has\_h1}} --- every prose file starts
  with an H1.
\item
...
\emph{editorial discipline} is just as expressible as a configurable
pipeline as \emph{numerical reproducibility}
(\breaktt{template\_code\_project}) and optional \emph{literature
curation} (\breaktt{projects\_archive/template\_search\_project}).

The permanent exemplars and optional search add-on share a single house
style:
...
\item
  \texttt{src/pipeline.py} is the only place the project touches
...
\item
  ...
  \breaktt{manuscript/references.bib} is the only artefact in the
  manuscript directory that may be auto-populated (only by the optional
  search add-on; this project validates but never writes).
\end{itemize}

The contribution of this exemplar is therefore architectural: a
\emph{generic, reusable} prose-quality module that any project in the
template can opt into, and a \emph{minimal, configurable} exemplar
wiring it to the bibliography and the manuscript pipeline. The
combination of \breaktt{template\_code\_project} (algorithm + numerical
experiments), \breaktt{template\_prose\_project} (editorial review), and
the optional \breaktt{projects\_archive/template\_search\_project} add-on
(literature discovery + LLM synthesis) covers the dominant shapes of
academic research projects in the template; new projects can copy
whichever exemplar most closely matches their shape and adjust from
there.
...
\item
  ...
  contract as the optional search add-on.
\end{enumerate}
...
\item
  ...
  \texttt{analyze\_structure}, and \texttt{analyze\_quality} are pure
...
\ExtensionTok{uv}\NormalTok{ run python projects/template\_prose\_project/scripts/run\_prose\_pipeline.py}
\FunctionTok{mv}\NormalTok{ projects/template\_prose\_project/output projects/template\_prose\_project/output\_first}
\ExtensionTok{uv}\NormalTok{ run python projects/template\_prose\_project/scripts/run\_prose\_pipeline.py}
\FunctionTok{diff} \AttributeTok{{-}ru} \DataTypeTok{\textbackslash{}}
\NormalTok{    projects/template\_prose\_project/output\_first/manuscript\_report.json }\DataTypeTok{\textbackslash{}}
\NormalTok{    projects/template\_prose\_project/output/manuscript\_report.json}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    projects/template\_prose\_project/manuscript/references.bib }\AttributeTok{{-}{-}strict}
\end{Highlighting}
\end{Shaded}
