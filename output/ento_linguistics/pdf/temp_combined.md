...
|--------|-------|
| Documents | **369** |
| Total processed tokens | **48,952** |
| Unique token types | **7,067** |
| Type–token ratio | **0.144** |
| Total characters | **539,570** |
| Average token length | **7.32 characters** |
...
Full text preprocessing — tokenization (`sent_tokenize`, `word_tokenize`), scientific-term merge, stop-word removal (NLTK English + `SCIENTIFIC_STOP_WORDS`), and lemmatization (`WordNetLemmatizer`) — is documented in Section S2 of Supplemental Methods \ref{sec:supplemental_methods}.
...
Domain-specific extraction from **369 publications** (48,952 tokens) identified **871 candidate terms** total, of which **223 receive domain assignments** spanning all six domains, with substantial variation in usage patterns:
...
This work establishes Ento-Linguistic analysis as a methodology for examining how scientific language constitutes—rather than merely represents—knowledge about ant biology. Through computational analysis of terminology networks across **362 entomological publications** (48,062 tokens; 871 extracted candidate terms, 223 domain-assigned) and six domains (Unit of Individuality, Behavior and Identity, Power \& Labor, Sex \& Reproduction, Kin \& Relatedness, and Economics), we demonstrate that entomological terminology carries systematic patterns of ambiguity, anthropomorphic framing, and conceptual structure that actively shape research practice. The accompanying open-source computational pipeline—implementing automated term extraction, co-occurrence network construction, and ambiguity scoring—provides a reproducible toolkit for extending this analysis to new corpora and domains.
