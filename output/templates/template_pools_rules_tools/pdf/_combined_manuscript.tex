...
\makeatother
\vfill
\includegraphics[width=0.98\textwidth,height=0.6\textheight,keepaspectratio]{\detokenize{../figures/cover_art.png}}
\vfill
\end{titlepage}
\thispagestyle{empty}
\tableofcontents
...
\textbf{Repository} &
\href{https://github.com/docxology/template}{Public template
repository} \\
\textbf{DOI} & 10.5281/zenodo.21298888 \\
...
in this paper are available in the
\href{https://github.com/docxology/template}{public template repository}
under the \breaktt{projects/templates/template\_pools\_rules\_tools/}
path. The integration pipeline is fully reproducible from source using
...
validated 2 rule sets, discovered 4 tools, and processed 8 bibliography
...
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.5\textheight,keepaspectratio,alt={Schema taxonomy comparison across the three fond types. Each fond declares a manifest and a type field; source-of-truth format, secondary mirror, and dedup key differ by category.}]{../figures/fond_taxonomy.png}
...
\AttributeTok{    }\FunctionTok{infrastructure}\KeywordTok{:}\AttributeTok{ }\DecValTok{60}
\AttributeTok{    }\FunctionTok{project\_src}\KeywordTok{:}\AttributeTok{ }\DecValTok{90}
\AttributeTok{    }\FunctionTok{public\_api}\KeywordTok{:}\AttributeTok{ }\DecValTok{95}
\end{Highlighting}
\end{Shaded}
...
rather than a style preference: coverage below the source-declared
threshold means tests are missing; a manuscript section without an
abstract means the document is incomplete.

\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.5\textheight,keepaspectratio,alt={Rule hierarchy: the two template rule sets, each split into a machine-enforceable strong/ branch and a guidance-only soft/ branch.}]{../figures/rule_hierarchy.png}
...
\bottomrule\noalign{}
\endlastfoot
\breaktt{strong/coverage-gate.yaml} & Minimum coverage: infrastructure
60\%, project \texttt{src/} 90\%, public API 95\% \\
...
\begin{figure}
\centering
\includegraphics[width=0.9\linewidth,height=0.5\textheight,keepaspectratio,alt={Invocation contract for the three template tools: stdin payload, tool behaviour, and stdout/exit-code shape.}]{../figures/tool_contract.png}
...
the current integration run, \textbf{4 tools} were discovered
...
\begin{figure}
\centering
\includegraphics[width=0.75\linewidth,height=0.5\textheight,keepaspectratio,alt={Runtime integration counts from manuscript\_variables.json. Bars show fonds loaded, rule sets validated, and tools discovered during a single pipeline run.}]{../figures/resource_counts.png}
...
declared manuscript-variable placeholders and inject them into
...
\begin{figure}
\centering
\includegraphics[width=0.95\linewidth,height=0.5\textheight,keepaspectratio,alt={The six-script pipeline from source validation through token hydration, ending at the combined-PDF render step.}]{../figures/pipeline_flow.png}
...
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.5\textheight,keepaspectratio,alt={Integration status dashboard showing per-resource validation results. Green indicates ok, amber indicates partial, red indicates missing.}]{../figures/status_dashboard.png}
...
what actually substitutes every declared placeholder and writes the
...
of computation of its own --- even \breaktt{01\_validate\_sources.py},
the largest entry point in this project, is entirely CLI plumbing
...
\texttt{src/} under the configured project coverage gate, while the
scripts themselves remain readable without a dedicated test suite of
their own.
...
\begin{figure}
\centering
\includegraphics[width=0.85\linewidth,height=0.5\textheight,keepaspectratio,alt={The integration layer's three-level resilience design: resource absence, schema malformation, and script absence, each with its own graceful-degradation response.}]{../figures/resilience_layers.png}
...
\item
  ...
  integration runtime statistics through doubled-brace, uppercase-key
  placeholders, ensuring that quantitative claims in the manuscript are
  always generated by the pipeline rather than authored manually. In the
  current run this covered 3 fonds, 2 rule sets, 4 tools, and 8
