...
\makeatother
\hypersetup{
  pdftitle={Exploratory Data Analysis: A Reproducible Notebook Template},
...
\title{Exploratory Data Analysis: A Reproducible Notebook Template}
\author{Daniel Ari Friedman\\\footnotesize{Active Inference Institute}\\\footnotesize{\texttt{daniel@activeinference.institute}}\\\footnotesize{\href{https://orcid.org/0000-0001-6232-9096}{ORCID: 0000-0001-6232-9096}} \\ \footnotesize{DOI: \href{https://doi.org/10.5281/zenodo.21086292}{10.5281/zenodo.21086292}}}
\date{\today}
...
Every figure and the summary table are produced by the
\href{https://github.com/docxology/template/blob/main/projects/templates/template_eda_notebook/scripts/eda_analysis.py}{EDA
analysis orchestrator} (\breaktt{scripts/eda\_analysis.py}), which calls
the tested figure-data preparers in \breaktt{src/eda/figures.py}. Running
the script regenerates the figures under \texttt{../figures/} and the
summary CSV under \texttt{output/data/}; the prose below describes what
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rows per group: sorted category labels and aligned counts from group\_count\_data(). The three groups are of comparable, though not identical, size; the counts sum to the complete-case row total.}]{../figures/group_counts.png}}
...
\ExtensionTok{uv}\NormalTok{ run python scripts/pipeline/stage\_03\_render.py }\AttributeTok{{-}{-}project}\NormalTok{ templates/template\_eda\_notebook}
\end{Highlighting}
\end{Shaded}
