...
\breaktt{projects/template\_code\_project/src/optimizer.py}; experiments
and figures are orchestrated by
\breaktt{projects/template\_code\_project/scripts/optimization\_analysis.py}
...
300\,dpi exports) are centralized in \breaktt{optimization\_analysis.py}.
...
\item
  ...
  (\breaktt{projects/template\_code\_project/tests/})}: An uncompromising
...
\item
  ...
  (\breaktt{projects/template\_code\_project/docs/})}: A structured
...
\item
  ...
  (\breaktt{markdown\_validator}, \texttt{output\_validator}) to ensure
...
\item
  ...
  \breaktt{optimization\_analysis.py} and uses \texttt{\{\{CONFIG\_*\}\}}
...
\breaktt{optimization\_analysis.py}. The active grid for this build is:
...
\breaktt{uv\ run\ pytest\ projects/template\_code\_project/tests/} during
...
\item
  \textbf{Project tests}:
  \breaktt{projects/template\_code\_project/tests/test\_optimizer.py}
  ...
  imports succeed, call into \breaktt{optimization\_analysis.py}
...
\item
  ...
  gate on \breaktt{projects/template\_code\_project/src/} prior to
...
\breaktt{optimization\_analysis.py}
(\breaktt{generate\_convergence\_plot},
...
\breaktt{generate\_benchmark\_visualization}). Captions in the markdown
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Sensitivity sweep produced by generate\_step\_size\_sensitivity\_plot() over an independent dense grid \textbackslash alpha \textbackslash in {[}0.005, 0.4{]} (15 points), distinct from the discrete experiment.step\_sizes used elsewhere in this section. Left: iterations to convergence on log--log axes --- the curve drops sharply from 501 iterations at the smallest \textbackslash alpha (the max\_iterations cap in this sub-experiment) to 38 iterations at \textbackslash alpha=0.4, illustrating the geometric speedup as \textbackslash rho(\textbackslash alpha)=|1-\textbackslash alpha|{} shrinks. Right: final f(x) versus \textbackslash alpha with horizontal reference lines at f(x\_0)=0 (initial) and f(x\^{}\textbackslash ast)=-0.5 (analytic optimum); every \textbackslash alpha in this stable window lands on the optimum.}]{../figures/step_size_sensitivity.png}}
...
tbl.~\ref{tbl:opt_results}. Rows follow \breaktt{experiment.step\_sizes}
...
\item
  ...
  \breaktt{projects/template\_code\_project/manuscript/config.yaml},
  automatically linked via the LaTeX configuration in
  \breaktt{projects/template\_code\_project/manuscript/preamble.md}.
\end{itemize}
...
\breaktt{projects/template\_code\_project/src/} with coverage gates
...
(\breaktt{projects/template\_code\_project/manuscript/04\_conclusion.md})
...
\textbf{3.1 \texorpdfstring{\ensuremath{\mu}}{mu}s} (recorded in
...
\item
  \textbf{Generated}: 2026-05-22T00:46:04Z
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
Config hash (SHA-256, truncated) & \texttt{cc2bb00f9a6b51d5} \\
Paper version & 2.0 \\
...
Analysis reports & 9 \\
\textbf{Total artifacts} & \textbf{19} \\
\end{longtable}
}
