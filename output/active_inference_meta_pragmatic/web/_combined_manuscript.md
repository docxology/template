...
```{=latex}
\[\{\lambda, \alpha, \beta, \gamma\} \rightarrow \{\lambda', \alpha', \beta', \gamma'\}\label{eq:metacog_hijack}\]
```
...
```{=latex}
\[A_{true} \rightarrow A_{corrupted}: \text{perception of reality distorted}\label{eq:epistemic_subversion}\]
```

**Pragmatic Landscape Alteration:** Modifying matrix $C$ changes what the agent values:
```{=latex}
\[C_{original} \rightarrow C_{corrupted}: \text{goal structure compromised}\label{eq:pragmatic_alteration}\]
```
...
```{=latex}
\[validation(c) = |accuracy_{predicted}(c) - accuracy_{actual}|\label{eq:confidence_validation}\]
```
...
```{=latex}
\[integrity(\Theta) = \|\Theta_t - \Theta_{baseline}\| < \epsilon\label{eq:framework_integrity}\]
```
...
```{=latex}
\[C_{safe} = \text{specification of safe preferences}\label{eq:safe_preferences}\]
```
