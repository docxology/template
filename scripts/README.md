...
- `generate_pipeline_reports.py` - reporting helper script
- `generate_test_summary.py` - test summary helper script
- `organize_executive_outputs.py` - reorganizes executive report outputs
...
- **`validation_report.{json,md}`** - validation report
...
flowchart TD
    ...
    COPY -->|✅ Files copied| SUCCESS([Pipeline<br/>Outputs in output/])
    WARN --> SUCCESS
