...
graph TB
    subgraph EntryPoints["Entry Points"]
        ...
        RUNALL[run_all.py<br/>Programmatic<br/>Core pipeline only]
    end
...
flowchart TD
    ...
    ENTRY -->|Programmatic| RUNALL[run_all.py<br/>Direct execution]
    ...
    VALIDATE -->|✅ Quality checks pass| COPY[Stage 05:<br/>Copy Outputs]
    ...
    LLM -->|✅ Ollama running| LLM_REVIEW[Stage 06:<br/>LLM Review]
    LLM -->|❌ No Ollama| SKIP_LLM[Skip LLM stages]

    LLM_REVIEW --> LLM_TRANSLATE[Stage 07:<br/>LLM Translations]
...
**Execution Flow:**
1. **Entry Points** → Choose interactive (`run.sh`) or programmatic (`run_all.py`)
