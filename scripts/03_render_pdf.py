...
def log_rendering_summary(summary: dict) -> None:
    ...
    logger.info(
        f"\n📦 Total Output Size: {summary['total_size_kb']:.1f} KB ({summary['total_size_kb'] / 1024:.2f} MB)"  # noqa: E501
    )
    logger.info("=" * 60 + "\n")
...
def verify_pdf_outputs(project_name: str = "project") -> bool:
    ...
    if pdf_files:
        ...
        if bib_file.exists():
            ...
            logger.warning(
                "\n⚠️  Bibliography file not found (citations detected in manuscript - bibliography processing will be skipped)"  # noqa: E501
            )
        else:
            logger.info(
                "\nℹ️  Bibliography file not found (no citations detected in manuscript - this is fine)"  # noqa: E501
            )
        ...
        if combined_pdf.exists():
            ...
            if size_mb > 0.01:
                if failed_compilations:
                    logger.warning(
                        f"\n⚠️  Combined manuscript PDF generated but {len(failed_compilations)} other PDF(s) failed to compile"  # noqa: E501
                    )
                else:
