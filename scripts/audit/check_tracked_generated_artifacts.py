...
def main(argv: list[str] | None = None) -> int:
    ...
    local_paths, secret_paths = tracked_public_output_leaks(repo_root)
    budget_findings = public_template_output_budget_findings(repo_root)
    if not offenders and not local_paths and not secret_paths and not budget_findings:
    ...
    if local_paths:
        ...
        for path in local_paths:
            print(f"  {path}")
    if secret_paths:
        print("Tracked public exemplar outputs contain credential-like secret material; remove and rotate it:")
