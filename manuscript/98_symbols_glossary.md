...
| `figure_manager` | `FigureManager` | class | Manages figures with automatic numbering and cross-referencing. |
| `figure_manager` | `FigureMetadata` | class | Metadata for a figure. |
...
| `image_manager` | `ImageManager` | class | Manages image insertion and cross-referencing in markdown files. |
...
| `markdown_integration` | `MarkdownIntegration` | class | Integrates figures and references into markdown files. |
...
| `pdf_validator` | `decode_pdf_hex_strings` | function | Decode PDF hex-encoded strings (e.g., /x45/x78 -> Ex) to readable text. |
