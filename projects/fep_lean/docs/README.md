...
- **Tests**: [`../tests/`](../tests/) — 30 modules, **338** collected tests (latest `pytest --collect-only`), coverage exceeds the **89%** gate (`pyproject.toml::fail_under = 89`; see [`tests/AGENTS.md`](../tests/AGENTS.md))
- **Lean workspace**: [`../lean/`](../lean/) (Lake + Mathlib v4.29.0, pinned in `lakefile.lean`)
- **Manuscript**: [`../manuscript/`](../manuscript/) — 24 numbered Markdown chapters + `AGENTS.md` + `README.md` + `preamble.md` + `config.yaml` + `manuscript_vars.yaml` + `references.bib` (27 files total under `manuscript/`); `09z_appendix_b_lean_catalogue.md` is regenerated by `_inject_manuscript_vars.py` (v0.7.0: fully expanded with real run data)
