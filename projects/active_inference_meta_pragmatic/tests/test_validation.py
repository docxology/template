...
from src.validation import ValidationFramework, demonstrate_validation_framework
...
class TestValidationFramework:
    ...
    def test_validate_probability_distribution_2d_valid(self):
        ...
        result = framework.validate_probability_distribution(transition_matrix, "transition")
    ...
    def test_validate_probability_distribution_2d_multiple_rows(self):
        ...
        result = framework.validate_probability_distribution(transition_matrix, "large_transition")
    ...
    def test_validate_probability_distribution_2d_invalid_rows(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            framework.validate_probability_distribution(transition_matrix, "invalid_transition")
    ...
    def test_validate_generative_model_incompatible_A_D(self):
        ...
        assert "A matrix columns" in str(exc_info.value) and "must match D length" in str(exc_info.value)
    ...
    def test_validate_generative_model_incompatible_C_A(self):
        ...
        assert "C matrix length" in str(exc_info.value) and "must match A rows" in str(exc_info.value)
    ...
    def test_validate_generative_model_incompatible_B(self):
        ...
        model = {
            'A': np.array([[0.8, 0.2], [0.2, 0.8]]),
            'B': np.array([[[0.9, 0.1, 0.0], [0.1, 0.9, 0.0]]]).transpose(1, 2, 0),  # Fixed shape
            'C': np.array([1.0, -1.0]),
            'D': np.array([0.5, 0.5])
        }
    ...
    def test_validate_generative_model_invalid_D(self):
        ...
        assert "prior beliefs" in str(exc_info.value) or "not properly normalized" in str(exc_info.value)
    ...
    def test_validate_generative_model_invalid_A_columns(self):
        ...
        B = np.array([[[0.9, 0.1], [0.1, 0.9]]]).transpose(1, 2, 0)
        model = {
            'A': np.array([[0.8, 0.3], [0.2, 0.8]]),  # First column sums to 1.0, second to 1.1
            'B': B,
            'C': np.array([1.0, -1.0]),
            'D': np.array([0.5, 0.5])
        }
        ...
        assert "A[:, 1]" in str(exc_info.value) or "not properly normalized" in str(exc_info.value)
    ...
    def test_validate_algorithm_performance_threshold(self):
        ...
        result = framework.validate_algorithm_performance(performance_metrics, requirements)
    ...
    def test_validate_algorithm_performance_range(self):
        ...
        framework = ValidationFramework()
        performance_metrics = {
            'latency': 0.5  # Should be between 0.1 and 1.0
        }
        requirements = {
            'latency': (0.1, 1.0)  # Range requirement
        }
    ...
    def test_create_validation_report_issue_aggregation(self):
        ...
        validation_results = [
            {'valid': False, 'issues': ['Issue 1', 'Issue 2']},
            {'overall_pass': False, 'issues': ['Issue 2', 'Issue 3']}  # Issue 2 is duplicate
        ]
...
class TestValidationFrameworkEdgeCases:
    ...
    def test_validate_algorithm_performance_list_value(self):
        ...
        requirements = {
            'scores': 0.85  # This will fail because list comparison
        }
    ...
    def test_validate_generative_model_A_column_validation_loop(self):
        ...
        model = {
            ...
            'B': np.array([[[0.9, 0.1, 0.0], [0.1, 0.8, 0.1], [0.0, 0.1, 0.9]]]).transpose(1, 2, 0),  # Fixed shape
