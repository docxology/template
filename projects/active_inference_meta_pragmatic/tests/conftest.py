...
# Add repo root so we can import infrastructure FIRST (before project src)
if REPO_ROOT not in sys.path:
    sys.path.insert(0, str(REPO_ROOT))

# Add project src/ to path so we can import src modules
# Insert after repo root to avoid shadowing infrastructure
PROJECT_SRC = PROJECT_ROOT / "src"
if PROJECT_SRC not in sys.path:
    # Insert after repo root but before any other paths that might conflict
    insert_pos = 1 if str(REPO_ROOT) in sys.path[:2] else 0
    sys.path.insert(insert_pos, str(PROJECT_SRC))
