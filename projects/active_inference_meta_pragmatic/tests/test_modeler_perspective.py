...
import numpy as np
import pytest
from framework.modeler_perspective import (ModelerPerspective,
...
class TestModelerPerspectiveDemonstration:
    ...
    def test_framework_storage(self):
        ...
        epistemic = modeler.specify_epistemic_framework(
            "test_epistemic", {"test": "epistemic_spec"}
        )

        pragmatic = modeler.specify_pragmatic_framework(
            "test_pragmatic", {"test": "pragmatic_spec"}
        )
        ...
        assert (
            modeler.specified_frameworks["test_epistemic"]["framework_type"]
            == "epistemic"
        )
        assert (
            modeler.specified_frameworks["test_pragmatic"]["framework_type"]
            == "pragmatic"
        )
...
class TestModelerIntegration:
    ...
    def test_modeler_with_quadrant_framework(self):
        ...
        assert (
            "meta-epistemic" in q4_info["description"].lower()
            or "meta-pragmatic" in q4_info["description"].lower()
        )
    ...
    def test_recursive_modeling_concept(self):
        ...
        assert (
            "modeling artificial agents"
            in self_understanding["cognitive_self_modeling"].lower()
        )
...
class TestModelerEdgeCases:
    ...
    def test_minimal_specifications(self):
        ...
        epistemic = modeler.specify_epistemic_framework(
            "minimal_epistemic", {"basic": "epistemic_spec"}
        )

        pragmatic = modeler.specify_pragmatic_framework(
            "minimal_pragmatic", {"basic": "pragmatic_spec"}
        )
    ...
    def test_specification_overwrite(self):
        ...
        # Specify framework
        modeler.specify_epistemic_framework("test_framework", {"version": "1"})

        # Overwrite with new specification
        updated = modeler.specify_epistemic_framework(
            "test_framework", {"version": "2"}
        )

        # Should be updated
        assert (
            modeler.specified_frameworks["test_framework"]["epistemic_boundaries"][
                "version"
            ]
            == "2"
        )
