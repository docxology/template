...
from src.meta_cognition import (
    MetaCognitiveSystem,
...
class TestMetaCognitiveSystem:
    ...
    def test_confidence_assessment(self, meta_system):
        ...
        assert assessment['assessment'] == "High confidence - inference appears reliable"
    ...
    def test_error_handling(self, meta_system):
        ...
        result = meta_system.assess_inference_confidence(np.array([-0.1, 1.1, 0.0]), 0.1)
        ...
        with pytest.raises(ValidationError):
            meta_system.adjust_attention_allocation(
...
class TestMetaCognitiveIntegration:
    ...
    def test_adaptive_attention_learning(self):
        ...
        scenarios = [
            {'beliefs': np.array([0.9, 0.05, 0.05]), 'uncertainty': 0.1},  # High confidence
            {'beliefs': np.array([0.4, 0.3, 0.3]), 'uncertainty': 0.7},   # Low confidence
            {'beliefs': np.array([0.6, 0.3, 0.1]), 'uncertainty': 0.3}    # Moderate confidence
        ]
...
class TestMetaCognitiveValidation:
    ...
    def test_meta_cognitive_numerical_stability(self, validator):
        ...
        for case in test_cases:
            assessment = system.assess_inference_confidence(case['beliefs'], case['uncertainty'])
