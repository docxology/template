...
\[B[:,:,a_1] = \begin{pmatrix} 0.8 & 0.2 \\ 0.0 & 1.0 \end{pmatrix} \quad B[:,:,a_2] = \begin{pmatrix} 1.0 & 0.0 \\ 0.2 & 0.8 \end{pmatrix}\]

**Matrix C (Preferences):**
\[C = \begin{pmatrix} 2.0 \\ -2.0 \end{pmatrix}\]
...
\[\mathcal{F}(\pi) = w_c(t) \cdot H[Q(\pi)] + w_r(t) \cdot G(\pi) + w_t(t) \cdot T(\pi)\]
...
\[q(s|t) = \frac{c(t) \cdot A[:,o_t] \odot q(s|t-1)}{Z}\]
...
\[\Theta^* = \arg\max_{\Theta} \mathbb{E}[U(c, e, \kappa | \Theta)]\]
...
**Figure [2](#fig:efe_decomposition):** Shows how EFE combines epistemic and pragmatic components
**Figure [3](#fig:perception_action_loop):** Illustrates the complete Active Inference cycle
**Figure [4](#fig:generative_model_structure):** Displays the A, B, C, D matrix relationships
**Figure [5](#fig:meta_level_concepts):** Demonstrates meta-epistemic and meta-pragmatic aspects
**Figure [6](#fig:fep_system_boundaries):** Shows Free Energy Principle system structure
**Figure [7](#fig:free_energy_dynamics):** Illustrates minimization over time
**Figure [8](#fig:structure_preservation):** Demonstrates system organization maintenance
