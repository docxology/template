...
class VisualizationEngine:
    ...
    def __init__(self, output_dir: Union[str, Path] = "output/figures",
    ...
    def create_figure(self, figsize: Tuple[float, float] = (8, 6)) -> Tuple[plt.Figure, plt.Axes]:
    ...
    def apply_publication_style(self, ax: plt.Axes, title: str, xlabel: str, ylabel: str,
                              grid: bool = True, legend: bool = False) -> None:
    ...
    def save_figure(self, fig: plt.Figure, filename: str, formats: Optional[List[str]] = None) -> Dict[str, Path]:
    ...
    def create_quadrant_matrix_plot(self, matrix_data: Dict) -> plt.Figure:
        ...
        if 'quadrants' in matrix_data:
            ...
            y_axis_labels = matrix_data.get('y_axis', {}).get('categories', ['Low', 'High'])
            x_axis_labels = matrix_data.get('x_axis', {}).get('categories', ['Low', 'High'])
            ...
            raise ValidationError("matrix_data must contain either 'quadrants' or 'matrix' and 'labels' keys")

        # Bottom-left (Q1)
        ax.add_patch(Rectangle((0, 0), 1, 1, fill=True, alpha=0.1,
                             color=quadrants['bottom_left']['color']))
        ax.text(0.5, 0.5, f"{quadrants['bottom_left']['id']}\n{quadrants['bottom_left']['name']}",
               ha='center', va='center', fontsize=12, fontweight='bold')

        # Bottom-right (Q2)
        ax.add_patch(Rectangle((1, 0), 1, 1, fill=True, alpha=0.1,
                             color=quadrants['bottom_right']['color']))
        ax.text(1.5, 0.5, f"{quadrants['bottom_right']['id']}\n{quadrants['bottom_right']['name']}",
               ha='center', va='center', fontsize=12, fontweight='bold')

        # Top-left (Q3)
        ax.add_patch(Rectangle((0, 1), 1, 1, fill=True, alpha=0.1,
                             color=quadrants['top_left']['color']))
        ax.text(0.5, 1.5, f"{quadrants['top_left']['id']}\n{quadrants['top_left']['name']}",
               ha='center', va='center', fontsize=12, fontweight='bold')

        # Top-right (Q4)
        ax.add_patch(Rectangle((1, 1), 1, 1, fill=True, alpha=0.1,
                             color=quadrants['top_right']['color']))
        ax.text(1.5, 1.5, f"{quadrants['top_right']['id']}\n{quadrants['top_right']['name']}",
               ha='center', va='center', fontsize=12, fontweight='bold')

        # Add axis labels
        ax.text(-0.1, 1, y_axis_labels[1],
               ha='center', va='center', rotation=90, fontsize=14, fontweight='bold')
        ax.text(-0.1, 0, y_axis_labels[0],
               ha='center', va='center', rotation=90, fontsize=14, fontweight='bold')

        ax.text(0.5, -0.1, x_axis_labels[0],
               ha='center', va='center', fontsize=14, fontweight='bold')
        ax.text(1.5, -0.1, x_axis_labels[1],
               ha='center', va='center', fontsize=14, fontweight='bold')
    ...
    def create_generative_model_diagram(self, model_structure: Dict) -> plt.Figure:
        ...
        matrices = [
            {'name': 'A\nObservation\nLikelihoods', 'pos': (0.5, 2), 'color': '#1f77b4'},
        ...
        for matrix in matrices:
            ...
            ax.add_patch(FancyBboxPatch(
                (matrix['pos'][0]-0.3, matrix['pos'][1]-0.3), 0.6, 0.6,
                boxstyle="round,pad=0.1", facecolor=matrix['color'], alpha=0.7
            ))

            # Add matrix label
            ax.text(matrix['pos'][0], matrix['pos'][1], matrix['name'],
        ...
        ax.set_title('Generative Model Structure (A, B, C, D Matrices)',
        ...
        """
        ax.text(2, 0.5, explanation, ha='center', va='center',
               fontsize=9, style='italic', bbox=dict(boxstyle="round,pad=0.3", facecolor='lightgray', alpha=0.5))
    ...
    def create_meta_cognitive_diagram(self, meta_cog_data: Dict) -> plt.Figure:
        ...
        processes = [
            ...
            {'name': 'Meta-Cognitive\nEvaluation', 'pos': (0.7, 0.8), 'color': '#d62728'},
        ...
        for process in processes:
            ax.add_patch(FancyBboxPatch(
                (process['pos'][0]-0.08, process['pos'][1]-0.08), 0.16, 0.16,
                boxstyle="round,pad=0.05", facecolor=process['color'], alpha=0.7
            ))

            ax.text(process['pos'][0], process['pos'][1], process['name'],
        ...
        for i in range(len(processes)-1):
            ...
            ax.arrow(start[0]+0.08, start[1], end[0]-start[0]-0.16, 0,
                    head_width=0.02, head_length=0.03, fc='black', ec='black',
                    alpha=0.7, length_includes_head=True)

        # Add feedback loop
        ax.arrow(0.82, 0.72, -0.6, -0.4, head_width=0.02, head_length=0.03,
                fc='red', ec='red', alpha=0.7, length_includes_head=True)
        ax.text(0.5, 0.3, 'Feedback Loop:\nAdaptive Control', ha='center', va='center',
               fontsize=9, color='red', fontweight='bold')

        # Add title
        ax.set_title('Meta-Cognitive Process Flow', fontsize=14, fontweight='bold', pad=20)
    ...
    def create_fep_visualization(self, fep_data: Dict) -> plt.Figure:
        ...
        # Internal states
        ax.add_patch(FancyBboxPatch((1.5, 1.5), 1, 1, boxstyle="round,pad=0.1",
                                   facecolor='#1f77b4', alpha=0.3))
        ax.text(2, 2, 'Internal\nStates', ha='center', va='center',
               fontsize=12, fontweight='bold')

        # Sensory states
        ax.add_patch(FancyBboxPatch((0.5, 1), 1, 0.8, boxstyle="round,pad=0.1",
                                   facecolor='#ff7f0e', alpha=0.3))
        ax.text(1, 1.4, 'Sensory\nStates', ha='center', va='center',
               fontsize=10, fontweight='bold')

        # Active states
        ax.add_patch(FancyBboxPatch((2.5, 1), 1, 0.8, boxstyle="round,pad=0.1",
                                   facecolor='#2ca02c', alpha=0.3))
        ax.text(3, 1.4, 'Active\nStates', ha='center', va='center',
               fontsize=10, fontweight='bold')

        # External states
        ax.add_patch(FancyBboxPatch((1.5, 0.2), 1, 0.6, boxstyle="round,pad=0.1",
                                   facecolor='#d62728', alpha=0.3))
        ax.text(2, 0.5, 'External\nStates', ha='center', va='center',
               fontsize=10, fontweight='bold')

        # Draw Markov blanket boundary
        ax.add_patch(FancyBboxPatch((0.3, 0.8), 3.4, 1.4, boxstyle="round,pad=0.2",
                                   facecolor='none', edgecolor='black', linewidth=2))
        ax.text(2, 2.3, 'Markov Blanket', ha='center', va='center',
               fontsize=12, fontweight='bold', color='black')

        # Add flow arrows
        # Internal to active
        ax.arrow(2.5, 1.9, 0.5, 0, head_width=0.05, head_length=0.1,
                fc='blue', ec='blue', alpha=0.7)
        # External to sensory
        ax.arrow(2, 0.8, -0.5, 0.2, head_width=0.05, head_length=0.1,
                fc='red', ec='red', alpha=0.7)
        # Sensory to internal
        ax.arrow(1.5, 1.4, 0.5, 0.4, head_width=0.05, head_length=0.1,
                fc='orange', ec='orange', alpha=0.7)

        # Add title and description
        ax.set_title('Free Energy Principle: System Boundaries and Markov Blanket',
                    fontsize=14, fontweight='bold', pad=20)
        ...
        """
        ax.text(2, 0.1, explanation, ha='center', va='center',
