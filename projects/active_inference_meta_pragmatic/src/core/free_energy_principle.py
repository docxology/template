...
class FreeEnergyPrinciple:
        ...
        if len(state_shape_set) > 1:
            ...
            for key, arr in system_states.items():
                ...
                if current_shape < max_shape:
                    # Pad with zeros to match max shape - more efficient than creating zeros then copying  # noqa: E501
    ...
    def _validate_system_states(self) -> None:
        ...
        if internal_shape != sensory_shape:
            logger.warning(
                f"Internal state shape {internal_shape} differs from sensory state shape {sensory_shape}. "  # noqa: E501
                "This may affect free energy calculations."
            )

    def calculate_free_energy(
        self, observations: NDArray, beliefs: NDArray
        ...
        try:
            ...
            logger.debug(
                f"Free energy calculation: F={free_energy:.4f}, E={energy:.4f}, H={entropy:.4f}"
            )
    ...
    def demonstrate_structure_preservation(
        self, time_steps: int = 100, seed: Optional[int] = 42
    ) -> Dict[str, NDArray]:
        ...
        try:
            ...
            for t in range(time_steps):
                ...
                observations = self._generate_observations(internal_state, external_state, rng)
                ...
                external_state = self._update_external_state(external_state, rng)
    ...
    def _generate_observations(
        self, internal: NDArray, external: NDArray, rng: Optional[np.random.Generator] = None
    ) -> NDArray:
        ...
        return np.clip(internal_trunc + external_trunc + rng.normal(0, 0.1, min_len), 0, 1)
    ...
    def _update_external_state(
        self, external: NDArray, rng: Optional[np.random.Generator] = None
    ) -> NDArray:
        """Update external state (environmental dynamics)."""
        if rng is None:
            rng = np.random.default_rng()
        # Simplified environmental dynamics
        return external + 0.05 * rng.normal(0, 1, external.shape)
