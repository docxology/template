...
def create_enhanced_quadrant_diagram(
    ...
    for i, (quadrant_id, (row, col)) in enumerate(zip(quadrant_names, positions)):
        ...
        # Clean and wrap description text (strip leading whitespace from triple-quoted strings)
        raw_desc = quadrant_info["description"]
        clean_desc = " ".join(raw_desc.split())  # Normalize whitespace
        wrapped_desc = textwrap.fill(clean_desc, width=50)
        ax.text(
            0.5,
            0.82,
            wrapped_desc,
        ...
        # Add examples with proper wrapping
        if "examples" in quadrant_info:
            examples_lines = ["Examples:"]
            for _key, example in quadrant_info["examples"].items():
                clean_ex = " ".join(example.split())
                wrapped_ex = textwrap.fill(clean_ex, width=55)
                examples_lines.append(f"• {wrapped_ex}")
