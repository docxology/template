...
This directory contains thin orchestrator scripts that implement the analysis pipeline for the Active Inference meta-pragmatic framework. Following the Research Project Template's thin orchestrator pattern, these scripts contain **no business logic** - all computational work is performed by modules in the `src/` directory.
...
5. **Report Generation**: Create reports using infrastructure reporting tools
...
- `FigureManager` from infrastructure
...
repo_root = project_root.parent             # project/ -> template/
