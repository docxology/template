...
# Try to find infrastructure module by walking up directories
current = script_dir
...
from generative_models import create_simple_generative_model, demonstrate_generative_model_concepts
    ...
    try:
        ...
        for stage_num in stages_to_run:
            ...
            if args.dry_run:
                ...
                pipeline_results['stages_completed'].append(f"stage_{stage_num}_dry_run")
                continue

            # Execute stage
            stage_result = execute_stage(
                stage_num, output_dir, figures_dir, data_dir,
                reports_dir, pipeline_results, figure_manager, error_aggregator
            )
            ...
            # Progress update
            completed = len([s for s in pipeline_results['stages_completed'] if not s.endswith('_dry_run')])
            logger.info(f"✅ Stage {stage_num} completed ({completed}/{len(stages_to_run)} stages done)")

        # Generate final pipeline report
        if not args.dry_run:
            logger.info("\n📊 Generating Pipeline Report...")
            generate_final_report(pipeline_results, perf_monitor, reports_dir)
        ...
        if args.dry_run:
            ...
            completed_stages = len([s for s in pipeline_results['stages_completed']
        ...
        if not args.dry_run:
            ...
            error_aggregator.add_error("pipeline", str(e), "critical", suggestions=[
...
def execute_stage(stage_num: int, output_dir: Path, figures_dir: Path,
                 data_dir: Path, reports_dir: Path, pipeline_results: Dict,
                 figure_manager: FigureManager, error_aggregator) -> Dict:
...
def execute_visualization_generation(pipeline_results: Dict, figures_dir: Path,
                                   figure_manager: FigureManager, error_aggregator) -> Dict:
    ...
    try:
        ...
        fig = viz_engine.create_generative_model_diagram({
            'matrices': ['A (Observations)', 'B (Transitions)', 'C (Preferences)', 'D (Priors)']
        })
...
def execute_statistical_analysis(pipeline_results: Dict, data_dir: Path, reports_dir: Path) -> Dict:
    ...
    if 'theoretical_demonstrations' in pipeline_results:
        ...
        if 'generative_models' in demos:
            ...
            if 'inference_demo' in gm_demo:
                ...
                analysis_results['generative_model_inference'] = calculate_descriptive_stats(
                    np.array(posterior)
                )
        ...
        if 'meta_cognition' in demos:
            ...
            if 'scenarios' in mc_demo:
                ...
                analysis_results['meta_cognitive_confidence'] = calculate_descriptive_stats(
    ...
    if 'theoretical_demonstrations' in pipeline_results:
        ...
        if 'generative_models' in demos:
            ...
            if 'model_structure' in gm_demo:
                ...
                try:
                    test_model = {
                        'A': np.array(gm_demo.get('A_sample', [[0.8, 0.2], [0.2, 0.8]])),
                        'B': np.random.rand(2, 2, 2),  # Simplified
                        'C': np.array(gm_demo.get('C_values', [1.0, -1.0])),
                        'D': np.array([0.5, 0.5])
                    }
                    validation_results['generative_model'] = validation_framework.validate_generative_model(test_model)
                except Exception as e:
                    validation_results['generative_model'] = {'valid': False, 'error': str(e)}

    # Validate visualizations
    try:
        from infrastructure.validation import validate_figure_registry
        validation_results['figures'] = validate_figure_registry()
    except Exception as e:
...
def execute_report_generation(pipeline_results: Dict, reports_dir: Path) -> Dict:
    ...
    summary_report = {
        'pipeline_execution': {
            ...
            'theoretical_demonstrations': len(pipeline_results.get('theoretical_demonstrations', {})),
            'visualizations_generated': len(pipeline_results.get('visualizations_generated', [])),
            'validation_passed': pipeline_results.get('validation_results', {}).get('overall_valid', False)
        },
        'key_findings': {
...
def execute_data_export(pipeline_results: Dict, data_dir: Path) -> Dict:
    ...
    return {
        'exports_created': len(exports),
        'export_files': exports
    }


def generate_final_report(pipeline_results: Dict, perf_monitor: StagePerformanceTracker,
                         reports_dir: Path) -> None:
    """Generate final comprehensive pipeline report."""
    # Construct stage_results list from pipeline_results
    stage_results = []
    perf_summary = perf_monitor.get_summary()

    for stage_key, stage_result in pipeline_results.items():
        if stage_key.startswith('stage_') and stage_key.endswith('_result'):
            stage_num = int(stage_key.split('_')[1])
            stage_name = f"stage_{stage_num}"

            # Get performance metrics for this stage
            stage_metrics = None
            if perf_summary and 'stages' in perf_summary:
                for stage_perf in perf_summary['stages']:
                    if stage_perf.get('stage_name') == stage_name:
                        stage_metrics = stage_perf
                        break

            # Construct stage result dict
            stage_dict = {
                'name': stage_name,
                'exit_code': 0,  # Assume success if we got here
                'duration': stage_metrics.get('duration', 0.0) if stage_metrics else 0.0,
            }
            stage_results.append(stage_dict)

    report = generate_pipeline_report(
        stage_results=stage_results,
        total_duration=perf_summary.get('total_duration', 0.0) if perf_summary else 0.0,
        repo_root=reports_dir.parent.parent,  # Go up to repo root
        test_results=None,  # Could be populated from pipeline_results if available
        validation_results=pipeline_results.get('validation_results'),
        performance_metrics=perf_summary,
    )

    final_report_file = reports_dir / "final_pipeline_report.json"
    save_pipeline_report(report, final_report_file)
...
def execute_final_integration(pipeline_results: Dict, output_dir: Path) -> Dict:
    ...
    try:
        ...
        integration_summary = {
            ...
            'total_figures_generated': len(pipeline_results.get('figures_registered', [])),
