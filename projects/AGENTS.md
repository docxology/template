...
| [`templates/template_pools_rules_tools/`](templates/template_pools_rules_tools/) | Fonds/rules/tools resource-pool integration exemplar (passive data pools + governance rule sets + executable tool entry points) | yes (`src/fonds_reader.py`, `src/rules_applier.py`, `src/tools_invoker.py`) | curated read-only | n/a | see canonical facts | see canonical facts |
