    ...
    *,
    ...
    for section in manifest.sections:
        ...
        if not section.should_compose():
            continue
        content = compose_section(section, root, registry=registry, options=opts, issues=issues)
        target = out_dir / section.output_name
        target.write_text(content, encoding="utf-8")
        written.append(target)
    emit_coverage_artifacts(root)
    from orchestration.coverage_pipeline import run_coverage_figures_and_page

    run_coverage_figures_and_page(root)
