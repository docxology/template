...
| `full_sheaf_appendix_tracks` | `16_appendix_full_sheaf.md` contains every `sheaf-track:{id}` marker bound in the `appendix_full_sheaf` manifest section (nine tracks; registry has ten types with optional `layers` methods-only) |
