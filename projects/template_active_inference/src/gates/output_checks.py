...
def _pymdp_logging_expected(root: Path) -> bool:
    ...
    return cfg.logging.enabled


def validate_outputs(project_root: Path) -> dict[str, bool]:
    root = project_root.resolve()
    required = [
        root / "output" / "data" / "parameter_sweep.csv",
        root / "output" / "data" / "si_tmaze_summary.json",
        root / "output" / "data" / "si_tmaze_trace.json",
        root / "output" / "data" / "analysis_statistics.json",
        root / "output" / "figures" / "ising_mi_curve.png",
        root / "output" / "figures" / "free_energy_curve.png",
        root / "output" / "figures" / "si_tmaze_actions.png",
        root / "output" / "figures" / "si_belief_entropy_curve.png",
        root / "output" / "figures" / "si_obs_action_trace.png",
        root / "output" / "figures" / "sheaf_layers_overview.png",
        root / "output" / "figures" / "sheaf_coverage_heatmap.png",
        root / "output" / "data" / "sheaf_coverage_matrix.json",
        root / "output" / "reports" / "invariants.json",
        root / "output" / "reports" / "si_invariants.json",
        root / "output" / "reports" / "si_tmaze_run_report.json",
    ]
