...
Fourth public exemplar: **sheaf-composed** Active Inference manuscript with configurable multi-track sections (analytical, pymdp, GNN, ontology, Lean, visualizations).
...
The first manuscript page ([`manuscript/00_00_sheaf_coverage.md`](manuscript/00_00_sheaf_coverage.md)) shows a **B/W/G heatmap** of section × track coverage (black = present, white = absent, gray = missing binding). Compose writes `output/data/sheaf_coverage_matrix.json` and `output/figures/sheaf_coverage_heatmap.png`.

Section [`16_appendix_full_sheaf.md`](manuscript/16_appendix_full_sheaf.md) binds {{appendix_sheaf_track_count}} track types as a composability proof (registry defines {{sheaf_track_count}} types; optional `layers` is methods-only; `animation` fragment is bound in the appendix row).
...
See [`tracks.yaml`](tracks.yaml). **Pipeline:** seven required tracks (analytical, pymdp, Lean, GNN, ontology, visualizations, manuscript). **Sheaf registry:** ten fragment types; appendix binds nine (`layers` methods-only). **Extension tracks** (opt-in): `render_animation.py` (placeholder GIF), `simulate_si_graph_world.py` (stub JSON).
