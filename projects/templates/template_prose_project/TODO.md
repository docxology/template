...
- Threshold-name or report-key migrations require the existing check-name and schema-shape assertions in `tests/test_pipeline.py` plus a scoped row when the public contract changes; renaming a threshold without moving its assertion is the known-wrong migration path this contract forecloses.
