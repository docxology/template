...
\makeatother
\hypersetup{
  pdftitle={Editorial Quality at Scale: A Reproducible Prose-Review Pipeline},
...
totalling 1745 words across 86 sentence(s) and 64 paragraph(s). Average
Flesch-Kincaid grade level is 15.97; average Gunning Fog index is 16.73;
the manuscript references 6 unique citation key(s); the longest section
is 416 words and the shortest is 17. These numbers are auto-substituted
...
\item
  ...
  review report), \breaktt{output/evidence\_summary.json} (versioned
  diagnostic-only evidence), and \breaktt{output/run\_summary.json}
  (one-line metadata).
\item
