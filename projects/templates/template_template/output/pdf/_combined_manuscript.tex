...
packages, about 604 modules, validated by about 7,780
...
(\breaktt{template\_code\_project}, 231 tests), prose
...
(127 tests) as a public exemplar in the same discovered/rendered tree,
...
\item
  ...
  test doubles. about 7,780 infrastructure tests and 2902+
...
\breaktt{template\_autoresearch\_project} runs readiness validation; the
archived \breaktt{template\_search\_project} merges remote literature
JSON, generates scripted figures
(\breaktt{y\_generate\_search\_figures.py}), and writes manifests;
\breaktt{template\_code\_project} emits optimization plots; and
\breaktt{template\_prose\_project} triggers structural validation
scaffolding. The pipeline shape is identical across all four---only the
Stage 02 payload differs---which is exactly what lets one orchestrator
serve heterogeneous research domains.
...
\tightlist
\item
  \textbf{Infrastructure tests} (\texttt{tests/}): about 7,780
...
\item
  ...
  example 296 tests in \breaktt{template\_autoresearch\_project} and 231
...
\bottomrule\noalign{}
\endlastfoot
\breaktt{template\_code\_project} & 8 & about 40 s & 231/231 &
90\%+ \\
...
\texttt{publishing} & 71 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & Zenodo, executable bundle, archival
targets \\
...
duration: the full infrastructure test suite (about 7,780
...
We have validated scaling with 12 canonical exemplars under
\breaktt{projects/templates/}---always present for onboarding and
tooling---and with this manuscript from
\breaktt{projects/templates/template\_template} (127 tests) as a
...
\item
  ...
  example with gradient-descent narration, 231 tests, 90\%+ coverage.
...
totals use \texttt{604} Python sources across infra + \texttt{7,780}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.publishing} (71
files)}{infrastructure.publishing (71 files)}}\label{infrastructure.publishing-71-files}
...
\NormalTok{├── tests/                       \# Infra suites (447+ files)}
...
figures, scripted tables & 231 @ 90\%+ gate & Controlled matplotlib
exports \\
...
(20 AI prompt templates), \texttt{security/} (steganography, hashing),
...
first-level subdirectories, and \texttt{20} counts the workflow
