...
\makeatother
\hypersetup{
  pdftitle={A template/ approach to Reproducible Generative Research},
...
\ifXeTeX
\special{pdf:trailerid [ <06fef8e8fe4aef54e270286ef226805c> <06fef8e8fe4aef54e270286ef226805c> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <06fef8e8fe4aef54e270286ef226805c> <06fef8e8fe4aef54e270286ef226805c> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <06fef8e8fe4aef54e270286ef226805c> <06fef8e8fe4aef54e270286ef226805c> ]}
\fi
...
about 662 modules, validated by about 8,583 tests)
...
and verified. When these layers are fragmented, manuscripts, data, code,
and figures can drift out of alignment, creating gaps that are
structural rather than incidental. Survey and economic evidence
documents the practical cost of failed reproduction attempts
\citep{baker2016reproducibility, freedman2015economics}, but
reproducibility is only the most visible face of a broader problem.
Research software engineering, epistemic integrity, and the coordination
between human researchers and AI collaborators all depend on whether the
tools that produce research artifacts can themselves be transparent,
testable, and self-documenting. Nosek et al.
\citep{nosek2018preregistration} argue that committing to analytical
plans before data collection is a necessary structural reform; we extend
this logic to the research build pipeline.
...
desynchronization. The version of a figure embedded in a PDF may not
match the code that ostensibly generated it, while tests may exercise
analysis code without exercising the rendering path. Pimentel et al.
\citep{pimentel2019jupyter} studied a corpus described as 1.4 million
Jupyter notebooks; in the valid execution subset, 24.11\% ran without
errors and 4.03\% reproduced the recorded results. Those distinctions
matter: execution success is not equivalent to result reproduction. Peng
...
underlying a published result be available for independent verification.
Barba \citep{barba2018terminologies} further documents how
``reproducibility,'' ``replicability,'' and ``repeatability'' carry
conflicting definitions across disciplines, complicating cross-field
standards.
...
execution and an extension architecture for execution and storage
backends, while its central scope remains computational pipeline
orchestration rather than this repository's combined manuscript, test,
and document-provenance contract.
...
\bottomrule\noalign{}
\endlastfoot
Pipeline orchestration & Snakemake, Nextflow, CWL & Quarto, Jupyter
Book, R Markdown, Typst, Overleaf, Prism \\
Manuscript rendering & Quarto, Jupyter Book, R Markdown, Typst,
Overleaf, Prism & Snakemake, Nextflow, CWL, DVC \\
...
\item
  ...
  scripts, data). 28 infrastructure subdirectories (25 importable
  packages) comprising about 662 Python modules provide
  reusable services; projects consume them without modification.
\item
  \textbf{Execution Integrity}: Pipeline advancement is contingent on
  test passage. The configured source-coverage floors are 60\% for
  infrastructure and 90\% for each project. The static gate rejects
  prohibited mock-framework imports, while the suites exercise real
  filesystem and subprocess behavior and use local protocol servers or
  explicitly enabled live services for network boundaries.
  about 8,583 infrastructure tests and 4907+ project tests are
  discovered by the introspection build.
\item
...
\item
  A detailed specification of the 16-stage DAG in
...
Module Reference} provides detailed documentation for all 28
subdirectories.
...
patterns, the YAML-declared pipeline (16 stages; default full 10)
...
\textbf{Infrastructure Layer} (\texttt{infrastructure/}): 28
infrastructure subdirectories---25 of them independently-importable
Python packages---comprising about 662 modules and providing
...
\breaktt{scripts/runner/execute\_pipeline.py} expands the declarative
DAG, applies tag filters (\texttt{-\/-core-only} skips \texttt{llm}
stages), checkpoints between nodes, then dispatches numbered scripts
...
\item
  ...
  with the configured \texorpdfstring{\textgreater{}=}{>=}60\% source-coverage floor.
\item
  \textbf{Project Tests} (\breaktt{01\_run\_tests.py\ -\/-project-only})
  --- isolated per-project suites with each project's declared floor
  (\texorpdfstring{\textgreater{}=}{>=}90\% for this exemplar).
\item
...
(\breaktt{scripts/pipeline/stage\_07\_executive\_report.py}) is
\textbf{not} a YAML node inside the single-project executor.
\breaktt{-\/-all-projects} / \breaktt{execute\_multi\_project.py} invokes
it once after iterating projects, consolidating cross-project KPIs
dashboards.
...
layer, the declarative DAG of \texttt{16} \texttt{pipeline.yaml} stages
...
The repository policy prohibits mock frameworks such as
\texttt{unittest.mock}, \texttt{MagicMock}, and \texttt{patch}
decorators \citep{martin2008clean, meszaros2007xunit}. A static gate
rejects those imports, and a separate inventory surfaces semantic
dependency-replacement patterns for review. Tests that require external
services (Ollama or public networks) use explicit \texttt{pytest.mark}
markers for conditional execution; deterministic network tests use real
local HTTP servers. The philosophical motivation---analogizing excessive
interaction mocking to Simmons et al.'s \emph{researcher degrees of
freedom} \citep{simmons2011falsepositive} and the preregistration remedy
...
\NormalTok{    modules }\OperatorTok{=}\NormalTok{ discover\_infrastructure\_modules(REPO\_ROOT)}
    \ControlFlowTok{assert}\NormalTok{ modules                     }\CommentTok{\# actual subpackages on disk}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1666}}
...
\bottomrule\noalign{}
\endlastfoot
Project & \texttt{projects/*/src/} & 90\% & Reported by the current test
artifact & Domain code must be thoroughly validated \\
Infrastructure & \texttt{infrastructure/} & 60\% & Reported by the
current test artifact & Broader shared surface \\
\end{longtable}
}

These thresholds are enforced at Stage 01 of the pipeline. A project
test run below its declared 90\% floor fails before downstream
publication stages. This manuscript deliberately does not quote an
achieved coverage percentage unless a fresh coverage artifact supplies
it.
...
\tightlist
\item
  \textbf{Infrastructure tests} (\texttt{tests/}): about 8,583
  tests validating the 28 infrastructure subdirectories, covering
...
\item
  ...
  example 300 tests in \breaktt{template\_autoresearch\_project} and 236
...
\tightlist
\item
  Shared 16pt font constant for primary text elements, with compact
  annotations derived from that constant.
\item
  Colorblind-safe palettes (IBM Design / Wong palette) with
  high-contrast fallbacks.
\item
  200 DPI PNG rendering, sourced from the same constant used by every
  figure writer.
\item
...
The \breaktt{test\_architecture\_viz.py} suite verifies that each real
figure generator writes a non-empty PNG and that the comparative matrix
has its declared shape and value domain. The font and render-resolution
values above are injected from \texttt{viz\_palette.py}, preventing
prose and implementation from drifting even though visual accessibility
still requires human inspection of rendered figures.

\newpage

\section{Results}\label{results}

This section reports repository facts produced by the meta-project's
live introspection pass. Test counts, module inventories, pipeline-stage
counts, and figure data are injected from generated artifacts. Runtime
and achieved-coverage claims are intentionally omitted when no versioned
benchmark or coverage artifact backs them.

\subsection{Multi-Project Pipeline
Execution}\label{multi-project-pipeline-execution}

The \texttt{./run.sh} interactive orchestrator can execute the public
roster through the shared DAG. The table below is a structural snapshot,
not a timing benchmark: counts come from repository introspection, and
the coverage column states each exemplar's declared policy floor rather
than an unversioned observed percentage.
...
Discovered tests
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Declared project floor
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
\breaktt{template\_code\_project} & 8 & 236 & 90\% \\
\breaktt{template\_prose\_project} & 8 & 120 & 90\% \\
\breaktt{template\_autoresearch\_project} & 8 & 300 & 90\% \\
\end{longtable}
}

\texorpdfstring{\textsuperscript{1}}{1}``Core-only'' excludes LLM-tagged and other opt-in stages according to
the YAML stage tags. A fresh run must be used to establish completion
status or wall-clock performance for a particular machine and dependency
set.
...
\endlastfoot
Test files & 504+ \\
Total tests & about 8,583 \\
Infrastructure coverage gate & \texorpdfstring{\textgreater{}=}{>=}60\% configured floor \\
Prohibited mock-framework imports & Checked by the static no-mocks
gate \\
\end{longtable}
}
...
templates \\
\texttt{core} & 112 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texttt{get\_logger},
...
\texttt{documentation} & 13 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texttt{FigureManager},
\breaktt{generate\_glossary} \\
\texttt{fonds} & 6 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & --- \\
...
validation \\
\texttt{orchestration} & 9 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texttt{PipelineRunner}, entry
...
\texttt{provenance} & 7 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & --- \\
...
\texttt{rendering} & 60 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & PDF/HTML/slide rendering, Pandoc
filters \\
...
executive artefacts \\
\texttt{research} & 3 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & --- \\
\texttt{rules} & 6 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & --- \\
...
\texttt{search} & 62 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \breaktt{infrastructure.search.literature}
clients + cache \\
...
regeneration \\
\texttt{steganography} & 13 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & Watermark overlays + hash
manifests \\
\texttt{tools} & 6 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & --- \\
\texttt{validation} & 75 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & PDF + Markdown + integrity CLIs \\
\end{longtable}
}

All 28 enumerated subdirectories carry Tier‑1/\texttt{README.md} and
...
\texttt{390+} Markdown shards under \texttt{docs/} capture operational
...
\subsection{Steganographic Performance
Boundary}\label{steganographic-performance-boundary}

The steganography path exposes metadata injection, SHA-256 hashing,
overlay generation, and optional QR-code insertion as separately
testable operations. This revision makes no latency claim because the
project does not currently generate a versioned benchmark artifact
recording hardware, input PDF, warm-up policy, repetitions, and
dispersion. A future performance table should be compiled from such an
artifact rather than copied from an ad-hoc workstation run.
...
\texttt{28}). Architecture figures stem from
...
\texttt{28} reusable subpackages, each annotated with its Python file
...
\textbf{Figure 2.} Pipeline DAG with 16 YAML-declared stages (core, LLM,
...
\texorpdfstring{\textsuperscript{1}}{1} Nextflow lineage records exist at workflow scope, not per rendered PDF
citation graph.
...
duration: the full infrastructure test suite (about 8,583
...
We have validated scaling with 23 canonical exemplars under
\breaktt{projects/templates/}---always present for onboarding and
tooling---and with this manuscript from
\breaktt{projects/templates/template\_template} (138 tests) as a
...
\item
  ...
  example with gradient-descent narration and 236 discovered tests. Its
  \texttt{pyproject.toml} declares the 90\% project-source floor.
  Minimal footprint: compact \texttt{src/}, scripted analysis, short
  manuscript sections.
\item
  \textbf{\breaktt{template\_prose\_project}}: Prose-heavy manuscript
  emphasizing narrative structure and bibliography discipline, with 120
  discovered tests and the same declared project-source floor; tests
  exercise rendering and Markdown integrity without heavyweight
  numerics.
\item
...
These workspaces share no project-level code---only Layer~1 (28
infrastructure subdirectories, about 662 Python
...
project size: the same 28 modules, the same 16 pipeline stages, the same
...
co-locates---testing enforcement, coverage thresholds, steganographic
...
peers). Their value here lies in a shared pipeline contract: required
test and documentation gates run together, while selected provenance
stages carry their own validation. The FAIR4RS principles
...
compliance observationally; \texttt{template/} operationalizes part of
this agenda by failing project tests below the declared 90\% floor and
validating provenance artifacts when those stages are enabled. Cohen et
al.'s four pillars of research software engineering
\citep{cohen2021four}---sustainability, quality, community, and
policy---are addressed primarily through the first two pillars via
...
\citep{anthropic2024mcp}, provides a bridge to the agentic software
paradigm. Lu et al.'s AI Scientist \citep{lu2024aiscientist}
demonstrates end-to-end autonomous research, while OpenHands is
evaluated on SWE-Bench Verified
\citep{wang2024opendevin, jimenez2024swebench}. These systems motivate
structured, protocol-aligned tool inventories for unfamiliar codebases.
An agent navigating \texttt{template/} reads \texttt{CLAUDE.md} for
global constraints, scans \texttt{AGENTS.md} for local contracts, and
can invoke skill-enabled capabilities through \texttt{SKILL.md}
descriptors. The live module inventory verifies \texttt{AGENTS.md} and
\texttt{README.md} coverage; \texttt{SKILL.md} remains a
capability-specific layer rather than a universal claim about every
...
substantial: \texttt{390} Markdown files under \texttt{docs/} alone,
...
\item
  ...
  directory provides a progressive sequence of beginner, intermediate,
  advanced, and expert guides alongside a comprehensive new-project
  setup checklist.
\end{enumerate}
...
\texttt{28} \breaktt{discover\_infrastructure\_modules(repo\_root)}. File
totals use \texttt{662} Python sources across infra + \texttt{8,583}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.core} (112
files)}{infrastructure.core (112 files)}}\label{infrastructure.core-112-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.documentation} (13
files)}{infrastructure.documentation (13 files)}}\label{infrastructure.documentation-13-files}

Figure registries plus glossary tooling feeding manuscript automation.

\subsubsection{\texorpdfstring{\breaktt{infrastructure.fonds} (6
files)}{infrastructure.fonds (6 files)}}\label{infrastructure.fonds-6-files}

Resource pool management for curated fonds (tracked reference datasets,
bibliographic collections, and evidence corpora). Fonds mirror
\breaktt{projects/templates/} with git-tracked \texttt{templates/*}
exemplars and sidecar-linked private lifecycle folders.
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.orchestration} (9
files)}{infrastructure.orchestration (9 files)}}\label{infrastructure.orchestration-9-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.provenance} (7
files)}{infrastructure.provenance (7 files)}}\label{infrastructure.provenance-7-files}

Content-addressed provenance DAG. Records artifact lineage (which run
produced which file, from which inputs) as a verifiable graph of
artifact/run/source/claim nodes connected by
produced/consumed/derived-from/supports/refutes edges. Includes a
structured Review system with severity (blocking/major/minor/info) and
verdict (refutes/supports). Features a CLI and pipeline integration
hooks for automatic lineage recording after every stage.
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.rendering} (60
files)}{infrastructure.rendering (60 files)}}\label{infrastructure.rendering-60-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.research} (3
files)}{infrastructure.research (3 files)}}\label{infrastructure.research-3-files}

Seven-stage research workflow
(SCOPE\texorpdfstring{\ensuremath{\to}}{->}LITERATURE\texorpdfstring{\ensuremath{\to}}{->}REASON\texorpdfstring{\ensuremath{\to}}{->}DESIGN\texorpdfstring{\ensuremath{\to}}{->}COMPUTE\texorpdfstring{\ensuremath{\to}}{->}SYNTHESIZE\texorpdfstring{\ensuremath{\to}}{->}WRITE) defined as
typed ResearchStage data classes with explicit outputs, skills, and
template commands per stage. Includes a full PRISMA-adapted literature
review prompt and a research workflow prompt referencing template/
infrastructure commands.

\subsubsection{\texorpdfstring{\breaktt{infrastructure.rules} (6
files)}{infrastructure.rules (6 files)}}\label{infrastructure.rules-6-files}

Governance rules layer for validating project lifecycle transitions,
sidecar sync policies, and pipeline gate orchestration. Rules mirror
\breaktt{projects/templates/} with git-tracked \texttt{templates/*}
exemplars.
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.search} (62
files)}{infrastructure.search (62 files)}}\label{infrastructure.search-62-files}

Two-tier search architecture: the \texttt{literature/} client stack
(client.py, backends, caches) powers literature search with arXiv,
Crossref, local, and Paperclip backends. \texttt{connectors/} exposes
the built-in scientific database adapters through a uniform
\breaktt{ConnectorRegistry}; OpenAlex, UniProt, PDB, Semantic Scholar,
European PMC, bioRxiv, and other registered adapters share list/search
CLI commands with HTTP timeout, retry, and TTL caching. The live
registry, not this prose, is authoritative for connector count.
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.steganography} (13
files)}{infrastructure.steganography (13 files)}}\label{infrastructure.steganography-13-files}

Watermark overlays, hashing companions triggered by secure pipeline
path.

\subsubsection{\texorpdfstring{\breaktt{infrastructure.tools} (6
files)}{infrastructure.tools (6 files)}}\label{infrastructure.tools-6-files}

Invocable tool definitions registered by resource-pool governance; tools
mirror \breaktt{projects/templates/} with git-tracked
\texttt{templates/*} exemplars.

\subsubsection{\texorpdfstring{\breaktt{infrastructure.validation} (75
files)}{infrastructure.validation (75 files)}}\label{infrastructure.validation-75-files}
...
\item
  \texttt{/CreationDate}: UTC timestamp in \texttt{ISO\ 8601} format
\item
...
complete build provenance chain: the system identifier,
\texttt{ISO\ 8601} build timestamp, short Git commit hash, pipeline
version, and project name. Together these fields allow a verifier to
reconstruct---from the watermark alone---which version of the code, at
which moment in time, produced the document.
...
\tightlist
\item
  Execute the standard YAML-declared pipeline (16 stages; default full
...
Connector Search & \breaktt{08\_connector\_search.py} &
\breaktt{manuscript/config.yaml} &
\breaktt{output/data/connector\_search/} & opt-in (\texttt{science} tag);
skipped if not configured \\
Provenance Record &
\breaktt{09\_provenance\_record.py\ -\/-stage\ "Connector\ Search"} &
prior stage outputs & \breaktt{.provenance/graph.json} & opt-in
(\texttt{provenance} tag); skipped if not configured \\
...
Ebook Generation & \breaktt{11\_ebook\_generation.py} & rendered combined
markdown & \texttt{output/ebook/} (EPUB/MOBI/DOCX) & opt-in
(\texttt{ebook} tag); soft fail \\
Metadata Package & \breaktt{12\_metadata\_package.py} &
\breaktt{manuscript/config.yaml} & \breaktt{output/metadata/}
(ONIX/JSON/OPF) & opt-in (\texttt{metadata} tag); soft fail \\
...
\breaktt{scripts/pipeline/stage\_07\_executive\_report.py} is invoked
...
\NormalTok{├── docs/ (18 top{-}level areas, 390+ markdown files per live counter)}
\NormalTok{├── tests/                       \# Infra suites (504+ files)}
...
(\breaktt{uv\ run\ python\ scripts/docgen/active\_projects.py}).
...
\begin{table}[h]
\caption{Three representative workspaces under \breaktt{projects/templates/} illustrating heterogeneous domains while sharing Layer~1. This is a hand-picked sample, not the full roster: the complete public exemplar set is enumerated dynamically from \breaktt{PUBLIC\_PROJECT\_NAMES} and listed below.}
...
figures, scripted tables & 236 discovered tests; 90\% declared gate &
...
Planner + validation CLI & 300 & Readiness reports from Stage 02 \\
\end{longtable}
}
...
agent context \\
\texttt{docs/} directory & 390 files across 18 subdirectories & User
...
docs), \texttt{guides/} (progressive skill-level guides),
\texttt{architecture/} (system design), \texttt{usage/} (content
authoring), \texttt{operational/} (build, config, logging,
troubleshooting), \texttt{reference/} (API, FAQ, glossary),
\texttt{modules/} (28 infrastructure modules), \texttt{development/}
(contributing, testing), \texttt{best-practices/} (version control,
migration), \texttt{prompts/} (21 AI prompt templates),
\texttt{security/} (steganography, hashing), and \texttt{audit/} (review
reports).

Every count in this appendix is injected from live repository
introspection rather than hand-maintained: \texttt{390} counts every
Markdown file beneath \texttt{docs/} recursively, \texttt{18} counts its
...
documentation total silently rots as the tree grows, whereas a token
re-resolves on every render. A reader onboarding to the repository
should start at \texttt{docs/core/}, follow the graduated
...
\texorpdfstring{\textsuperscript{1}}{1} Nextflow lineage records operate at workflow scope rather than as a
rendered-document citation graph. \texorpdfstring{\textsuperscript{2}}{2} DVC provides content-addressed
...
distributed compute. This matrix is a versioned manuscript snapshot, not
a continuously updated product survey. A future refresh should re-check
every external capability against current primary documentation before
changing a cell.
