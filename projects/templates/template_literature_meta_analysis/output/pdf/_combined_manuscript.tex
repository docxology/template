...
\textbf{Modafinil}. The pipeline dispatches across 10 literature engines
(arXiv, OpenAlex, Semantic Scholar, Crossref, PubMed, SovietRxiv,
ChinaRxiv, Europe PMC, bioRxiv/medRxiv, and medrxiv), each degrading
gracefully to a skipped source when an API key or the network is
unavailable, then merges and de-duplicates records by a canonical
identifier hierarchy (DOI \(>\) arXiv ID \(>\) Semantic Scholar ID \(>\)
OpenAlex ID \(>\) title digest) into a corpus of \(N = 2334\) records
spanning 2000--2026 (26 years). Records are classified into a
configurable 6-bucket subfield taxonomy (Clinical Sleep, Cognition,
Pharmacology, Psychiatry, Safety, and Neuroscience); the largest
subfield is \textbf{Clinical Sleep} (63.0\% of the classified corpus).
The corpus grows at a compound annual rate of 5.48\% (mean
year-over-year growth 7.8\%, doubling time 9.2 years), peaking in 2025
with 147 records.
...
\item
  ...
  are gathered from 10 independent engines (arXiv, OpenAlex, Semantic
  Scholar, Crossref, PubMed, SovietRxiv, ChinaRxiv, Europe PMC,
  bioRxiv/medRxiv, and medrxiv). An engine with no API key or no network
  reports a \emph{skipped} status; the run completes from whatever
  engines remain plus a committed offline corpus. Each new retrieval
  persists per-engine outcome and count provenance in
...
\item
  ...
  the configured query across 10 engines (arXiv, OpenAlex, Semantic
  Scholar, Crossref, PubMed, SovietRxiv, ChinaRxiv, Europe PMC,
  bioRxiv/medRxiv, and medrxiv), merge, and de-duplicate into
  \texttt{corpus.jsonl}. Each engine is an isolated adapter exposing a
  uniform \texttt{search(query)\ -\textgreater{}\ list{[}Paper{]}}
  interface; engines that are keyless need no credentials, while
  Semantic Scholar uses a key when present. SovietRxiv and ChinaRxiv
  share a unified API with an optional \texttt{X-API-Email} header for
  the polite rate-limit pool (300/min vs 30/min anonymous).
\item
...
\item
  ...
  PubMed, SovietRxiv, ChinaRxiv, Europe PMC, bioRxiv, and medRxiv (each
...
Retrieval dispatches the configured query across 10 independent
literature engines (arXiv, OpenAlex, Semantic Scholar, Crossref, PubMed,
SovietRxiv, ChinaRxiv, Europe PMC, bioRxiv/medRxiv, and medrxiv). Each
engine is an isolated adapter exposing a uniform
...
de-duplicated 2334 records across 10 engines (arXiv, OpenAlex, Semantic
Scholar, Crossref, PubMed, SovietRxiv, ChinaRxiv, Europe PMC,
bioRxiv/medRxiv, and medrxiv), classified them into 6 configurable
subfields (with \textbf{Clinical Sleep} dominant at 63.0\%), extracted 5
topics over a 500-feature vocabulary, computed reproducible document
embeddings, mapped the citation network (2234 nodes, 8,623 edges, 1416
communities), and framed 6 hypotheses for optional evidence scoring.
...
This manuscript was generated from a live retrieval run using 10
engines. Every number, table, and figure in this document is injected
from a committed artifact (\breaktt{output/data/*.json},
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Live search (all 10 engines, max 1000 per engine)}
...
\breaktt{-\/-skip-biorxiv}, \breaktt{-\/-skip-medrxiv}.
...
\item
  ...
  hierarchy)
\item
  All 10 engine paths (arXiv, Semantic Scholar, OpenAlex, Crossref,
...
metadata. This run drew on 10 engines, a 6-bucket taxonomy, and 6
hypotheses.
...
Europe PMC, bioRxiv/medRxiv, and medrxiv) with a uniform search
