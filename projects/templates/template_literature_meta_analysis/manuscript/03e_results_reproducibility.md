...
This stage is entirely optional and never runs in the offline default: with no fulltext
available and no language model configured it is skipped, and the reproducibility
aggregates read *pending* — the same graceful-degradation convention used by the
knowledge-graph assertion-extraction stage (see
[`02d_methods_knowledge_graph.md`](02d_methods_knowledge_graph.md)). A live run with
fulltext and a language model available populates the mean score, low-score count, and
per-paper table from extracted workflow graphs.
