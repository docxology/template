...
**Why this is achievable here**: the modules in `src/` that touch external systems do so through a small, well-defined surface — `infrastructure.search.literature.LiteratureClient`, `AbstractFetcher`, `FulltextFetcher`, and a duck-typed `llm: Callable[[str], str]` argument in `src/pipeline/synthesis.py`. Tests substitute the search backend with `infrastructure.search.literature.LocalBackend` against a temp-dir corpus, and substitute the LLM with a deterministic local function.

**LLM corollary**: `src/pipeline/synthesis.py::synthesise_per_paper` and `synthesise_corpus` accept `llm` as a positional callable. Tests pass a function; never a mock.
...
**Correct pattern** (from `tests/pipeline/test_synthesis.py`):
```python
...
|---|---|---|
| `src/publish/config.py` | `dataclasses`, `pathlib`, `yaml`, stdlib `typing` | Anything from `infrastructure.*` |
| `src/pipeline/pipeline.py` | `infrastructure.search.literature`, `infrastructure.reference.citation` | `infrastructure.llm.*`, `infrastructure.rendering.*`, `infrastructure.reporting.*` |
| `src/search/deep_search.py` | `infrastructure.search.literature`, `infrastructure.reference.citation` | Same as above |
| `src/pipeline/synthesis.py` | `infrastructure.search.literature` (for `Paper`) | `infrastructure.llm.*` directly — receives a callable |
| `src/pipeline/llm_runtime.py` | `infrastructure.llm` | `infrastructure.search.*`, `infrastructure.reference.*` |
| `src/publish/figures.py` | `matplotlib`, `numpy` | `infrastructure.*` |
| `src/publish/manuscript_variables.py` | stdlib only | `infrastructure.*` |
| `src/analysis/report.py` | `infrastructure.search.literature` (for `Paper`) | Anything that writes files itself outside the function's `output_path` arg |
| `src/analysis/analysis.py` | stdlib only (`re`, `pathlib`, `subprocess`) | `infrastructure.*` |
| `src/search/search_invariants.py` | stdlib only | `infrastructure.*` |
| `src/pipeline/dotenv.py` | stdlib only | Anything else |
...
```python
# BAD — pagination + dedup belongs in src/pipeline/pipeline.py
...
```markdown
`src/pipeline/pipeline.py::run_literature_pipeline` queries the backends listed in
...
| "An LLM summarises each paper." | "`src/pipeline/synthesis.py::synthesise_per_paper` runs each enriched paper through the callable returned by `src/pipeline/llm_runtime.py::build_llm_callable`, writing the response to `output/llm/per_paper/<safe_id>.md`." |
...
| typed config | `projects/templates/template_search_project/src/publish/config.py` |
| standard pipeline | `projects/templates/template_search_project/src/pipeline/pipeline.py` |
| deep search | `projects/templates/template_search_project/src/search/deep_search.py` |
| synthesis | `projects/templates/template_search_project/src/pipeline/synthesis.py` |
| LLM runtime | `projects/templates/template_search_project/src/pipeline/llm_runtime.py` |
| variables | `projects/templates/template_search_project/src/publish/manuscript_variables.py` |
| review helpers | `projects/templates/template_search_project/src/analysis/analysis.py` |
...
Follow the patterns established in `src/publish/config.py` and `src/publish/manuscript_variables.py`:
...
- Aggregate run outputs in a single dataclass returned by the orchestration entry point. The conventional names are:
  - `LiteratureRunArtifacts` — returned by `src/pipeline/pipeline.py::run_literature_pipeline`
  - `DeepSearchArtifacts` — returned by `src/search/deep_search.py::run_deep_search`
  - `SearchCache` (from infrastructure) — keyed on canonical query identity; re-runs hit the cache without code changes

**Example** (following `ManuscriptVariables` in `src/publish/manuscript_variables.py`):
```python
...
**Correct** (following the patterns in `src/publish/config.py` and `src/pipeline/pipeline.py`):
```python
