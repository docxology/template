...
\ifXeTeX
\special{pdf:trailerid [ <2605eb076e86ea379702d145e73a2137> <2605eb076e86ea379702d145e73a2137> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <2605eb076e86ea379702d145e73a2137> <2605eb076e86ea379702d145e73a2137> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <2605eb076e86ea379702d145e73a2137> <2605eb076e86ea379702d145e73a2137> ]}
\fi
...
keyword from
\breaktt{project\_config.deep\_search.max\_results\_per\_keyword} in
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Per-source paper counts read from SearchResult.per\_source\_counts (pre-deduplication contribution per backend). The numeric label above each bar reports the raw count; the y-axis spans {[}0, max + headroom{]}. Bar order follows project\_config.search.sources. Empty runs render (no results) centred. Generated by src/figures.py::plot\_papers\_per\_source.}]{../figures/papers_per_source.png}}
...
Bar order follows \protect\breaktt{project\_config.search.sources}. Empty
runs render \protect\texttt{(no\ results)} centred. Generated by
...
\protect\texttt{Paper.score}; long y-tick titles are truncated with an
ellipsis. Backends without scoring (notably
\protect\texttt{LocalBackend}) report
...
\item
  ...
  \breaktt{project\_config.search.sources}.
\item
...
\item
  \textbf{Vector recall on \texttt{LocalBackend}} for sufficiently large
  curated corpora, gated behind an optional dependency and an explicit
  benchmark-derived crossover threshold.
\end{enumerate}
...
\tightlist
\item
  \texttt{project\_config.search.sources:\ {[}local{]}} consumes
...
\tightlist
\item
  \textbf{Live search drift.} When
  \breaktt{project\_config.search.sources} includes \texttt{arxiv} or
  \texttt{crossref}, the first cache-miss invocation hits the live API;
  the cached JSON freezes that response, but two cold-start clones
  running on different days will see different paper sets. Pin a
  \texttt{LocalBackend} corpus or commit \breaktt{output/search/cache/}
...
\item
  ...
  opt-in and the production endpoint may reject the adapter's request
  method; the run records the response in
...
The deep-search pipeline reads \breaktt{project\_config.deep\_search}
from \breaktt{manuscript/config.yaml}, runs one \texttt{SearchQuery} per
...
\item
  ...
  (\texttt{output/cache/pdf/\textless{}safe\_id\textgreater{}.\{pdf,txt\}}).
\item
  LLM seed (\breaktt{project\_config.deep\_search.llm\_seed:\ 42}) +
...
\item
  Including \texttt{paperclip} in
  \breaktt{project\_config.deep\_search.sources}.
\end{enumerate}
...
Failure-isolation: any per-backend error (including a method-rejection
response the production endpoint may return during migration) is
captured into \texttt{SearchResult.errors{[}paperclip{]}} and the run
continues. See
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Run with everything from config.yaml (assumes project\_config.deep\_search.enabled: true)}
...
\breaktt{project\_config.deep\_search.llm\_per\_paper:\ true} and ensure
Ollama is reachable to populate this section).}
