...
|------|------|--------------|----------------|
| `output/figures/convergence_plot.png` | Gradient descent trajectories | `src/template_code_project/figures/convergence.py::generate_convergence_plot()` (via `src/template_code_project/analysis/` / `scripts/optimization_analysis.py`) | 4 — Run Analysis |
| `output/figures/step_size_sensitivity.png` | Step-size vs final objective | `src/template_code_project/figures/sensitivity.py::generate_step_size_sensitivity_plot()` | 4 — Run Analysis |
| `output/figures/convergence_rate_comparison.png` | Convergence-rate comparison across α | `src/template_code_project/figures/convergence.py::generate_convergence_rate_plot()` | 4 — Run Analysis |
| `output/figures/algorithm_complexity.png` | O(·) complexity visual | `src/template_code_project/figures/scientific.py::generate_complexity_visualization()` | 4 — Run Analysis |
| `output/figures/performance_benchmark.png` | Benchmark dimensions vs deterministic work proxy and iterations | `src/template_code_project/figures/scientific.py::generate_benchmark_visualization()` | 4 — Run Analysis |
| `output/figures/stability_analysis.png` | Stability heat map | `src/template_code_project/figures/scientific.py::generate_stability_visualization()` | 4 — Run Analysis |
| `output/figures/benchmark_timings.png` | Deterministic benchmark input size vs objective value | `scripts/04_benchmark_stage.py` via `src/template_code_project/core/benchmark_support.py` | 4 — Run Analysis |
| `output/data/optimization_results.csv` | Per-step-size results table including vector solution and termination reason | `save_optimization_results()` in `src/template_code_project/analysis/` | 4 — Run Analysis |
| `output/data/convergence_plot.json` | Source values for the convergence figure | `src/template_code_project/figures/convergence.py::generate_convergence_plot()` | 4 — Run Analysis |
| `output/data/dashboard_payload.json` | Numerical payload for the interactive dashboard | `src/template_code_project/dashboard/dashboard.py::build_dashboard_html()` | 4 — Run Analysis |
| `output/data/manuscript_variables.json` | `{TOKEN: value}` substitution map (hydration-self output, not an `ARTIFACT_DATA_FILES` input) | `scripts/z_generate_manuscript_variables.py` (calls `src/template_code_project/core/manuscript_variables.py::generate_variables`) | 4 — Run Analysis (post-stage) |
| `output/reports/stability_analysis.json` | Stability assessment numbers | `run_stability_analysis()` in `src/template_code_project/analysis/` | 4 — Run Analysis |
| `output/reports/performance_benchmark.json` | Byte-stable benchmark inputs, objective outputs, checks, and timing policy | `run_performance_benchmarking()` in `src/template_code_project/analysis/` | 4 — Run Analysis |
| `output/reports/benchmark_report.json` | Deterministic rubric results for the exemplar benchmark | `scripts/04_benchmark_stage.py` via `src/template_code_project/core/benchmark_support.py` | 4 — Run Analysis |
| `output/reports/dashboard_invariants.txt` | Machine-readable dashboard invariant results | `src/template_code_project/dashboard/dashboard.py::build_dashboard_html()` | 4 — Run Analysis |
| `output/reports/dashboard_summary.txt` | Human-readable dashboard summary | `src/template_code_project/dashboard/dashboard.py::build_dashboard_html()` | 4 — Run Analysis |
| `output/reports/output_validation.json` | Integrity summary for analysis outputs | `save_validation_report()` in `src/template_code_project/analysis/` | 4 — Run Analysis |
| `output/web/dashboard.html` | Plotly interactive dashboard | `build_dashboard.py` / `src/template_code_project/dashboard/dashboard.py` | 4 — Run Analysis |
...
| `output/citations/*.{bib,apa,mla}` | Citation metadata in 3 formats | `generate_citations_from_metadata()` in `src/template_code_project/analysis/` | 4 — Run Analysis |
...
live in `src/template_code_project/core/manuscript_variables.py` as `_ANALYSIS_FIGURE_FILENAMES`,
...
1. Add the generator in the appropriate module:
   - **Figures** → `src/template_code_project/figures/` (invoked from `src/template_code_project/analysis/`; script entry: `scripts/optimization_analysis.py`)
   - **Data** → `src/template_code_project/analysis/`
   - **Reports** → `src/template_code_project/dashboard/dashboard.py` or `infrastructure.reporting.*`
2. Append a row to the table above naming the file, role, generator, and pipeline stage.
3. If the new artifact is consumed by the manuscript, add a `{{TOKEN}}` to `src/template_code_project/core/manuscript_variables.py::generate_variables()` and reference it in the appropriate `manuscript/*.md` section.
