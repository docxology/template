"""Tests for ``template_code_project.core.manuscript_variables``.
...
from template_code_project.core.manuscript_variables import (
...
def test_load_config_missing_file(tmp_path):
    root = tmp_path / "empty"
    root.mkdir()
    from template_code_project.core.manuscript_variables import _load_config

    assert _load_config(root) == {}


def test_load_optimization_results_missing_csv(tmp_path):
    from template_code_project.core.manuscript_variables import _load_optimization_results
...
# ---------------------------------------------------------------------------
# generate_variables — missing-config fallback (covers the missing-file branch
# of `_load_config` and the N/A fallbacks in `generate_variables`)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# generate_variables — rho ≥ 1 convergence factor (the ``elif rho >= 1``
# divergent branch of the RESULT_CONVERGENCE_FACTORS bullet)
# ---------------------------------------------------------------------------
...
spec = importlib.util.spec_from_file_location(
    ...
    project_root / "src" / "template_code_project" / "core" / "manuscript_variables.py",
)
...
def test_step_sensitivity_caption_numbers_match_sweep():
    ...
    from template_code_project.core.sweeps import sensitivity_sweep
    ...
    caption = (Path(__file__).resolve().parents[2] / "manuscript" / "03_results.md").read_text(encoding="utf-8")
