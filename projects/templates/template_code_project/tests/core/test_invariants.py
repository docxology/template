"""Tests for the numerical-invariants module
(``src/template_code_project/core/invariants.py``).
...
``src/template_code_project/core/invariants.py`` is exercised against the
gradient-descent implementation in
``src/template_code_project/core/optimizer.py`` using real numerical
computations (no mocks, per the project's Zero-Mock policy).
...
import numpy as np
import pytest
from template_code_project.core.invariants import (
    InvariantResult,
