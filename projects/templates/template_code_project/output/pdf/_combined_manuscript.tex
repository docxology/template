...
\item
  ...
  \breaktt{infrastructure.scientific.benchmarking} to check numerical
  boundaries and exercise runtime performance diagnostics without
  pinning host-dependent timings.
\item
...
figure reports deterministic convergence iterations and the proxy
\(d \times \text{iterations}\); neither surface claims that one host run
proves an asymptotic bound.
...
below \breaktt{experiment.tolerance}, when \(k\) reaches
\breaktt{experiment.max\_iterations}, or when an objective, gradient, or
update becomes non-finite. The boolean \texttt{converged} plus
\breaktt{termination\_reason} in exported CSV rows distinguish these
outcomes. A non-finite candidate is rejected before it enters
\breaktt{objective\_history}, preserving the last finite state for
...
termination, iteration count capped by \(N_{\max} = 1000\), the
\protect\texttt{converged} flag from
\protect\breaktt{gradient\_descent()} using \(\|\nabla f\| < 10^{-8}\),
and its structured termination reason. A \protect\texttt{non\_finite}
row stops before an overflowing update is serialized and retains the
last finite iterate.}\label{tbl:opt_results}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
0.01 & 1.0000 & -0.5000 & 1000 & No & max\_iterations \\
...
2.50 &
-18027808360396904929014313133756094602021490593623455934143098782085898358268968910321926429082931340564236187769460640748166785144621899661251320903368704.0000
&
.0000
& 876 & No & non\_finite \\
\end{longtable}
...
fig.~\ref{fig:benchmark} shows deterministic computational work and
convergence iterations for \breaktt{gradient\_descent()} on
identity-Hessian quadratics of dimension
\(d \in \{1, 2, 5, 10, 20, 50\}\).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Deterministic dimensional benchmark from generate\_benchmark\_visualization(). Dimensions d \textbackslash in \textbackslash\{1, 2, 5, 10, 20, 50\textbackslash\} run on identity-Hessian quadratics with \textbackslash alpha=0.1 and gradient tolerance 10\^{}\{-10\}. Left: the implementation-independent work proxy d \textbackslash times \textbackslash text\{iterations\}, proportional to scalar elements processed by the dense matrix-vector update. Right: exact iterations to convergence. Wall-clock microseconds are intentionally excluded because they vary with hardware and host load. The similar iteration counts are expected because \textbackslash kappa(I\_d)=1 and the contraction factor \textbackslash rho=|1-\textbackslash alpha|=0.9 is dimension-independent.}]{../figures/performance_benchmark.png}}
\caption{Deterministic dimensional benchmark from
\protect\breaktt{generate\_benchmark\_visualization()}. Dimensions
\(d \in \{1, 2, 5, 10, 20, 50\}\) run on identity-Hessian quadratics
with \(\alpha=0.1\) and gradient tolerance \(10^{-10}\). \textbf{Left}:
the implementation-independent work proxy
\(d \times \text{iterations}\), proportional to scalar elements
processed by the dense matrix-vector update. \textbf{Right}: exact
iterations to convergence. Wall-clock microseconds are intentionally
excluded because they vary with hardware and host load. The similar
iteration counts are expected because \(\kappa(I_d)=1\) and the
contraction factor \(\rho=|1-\alpha|=0.9\) is
dimension-independent.}\label{fig:benchmark}
\end{figure}
...
\item
  Mean iterations across rows in tbl.~\ref{tbl:opt_results}: 351
\end{itemize}
...
to isolate algorithmic scaling from problem conditioning effects. The
tracked \breaktt{output/reports/performance\_benchmark.json} records
fixed inputs, exact objective values, and validation checks. Wall-clock
timing is retained only as a runtime diagnostic because host load and
hardware make microsecond values unsuitable as pinned reproducibility
claims.
...
\tightlist
\item
  \textbf{Python}: 3.13.11
\item
  \textbf{NumPy}: 2.4.2
\item
  \textbf{Platform}: Darwin arm64
\item
  \textbf{Generated}: 2026-07-20T15:59:12Z
\end{itemize}
...
\bottomrule\noalign{}
\endlastfoot
Config hash (SHA-256, truncated) & \breaktt{292643985e5af3f2} \\
Paper version & 2.5.2 \\
...
\breaktt{src/benchmark\_support.py}, which evaluates the pure
\breaktt{quadratic\_function} across fixed seeded inputs and turns
...
\texttt{RubricSet}, then rendered with \breaktt{scores\_to\_markdown}
into the byte-stable \breaktt{output/reports/benchmark\_report.json} with
a deterministic objective-value figure. Real timing still executes as a
runtime diagnostic, but it is not serialized into tracked evidence.
