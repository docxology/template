...
| `test_figures.py` | Plot-ready data preparers (schema rows, inventory rows, provenance steps, severity counts), truthful alternate text, and the demonstration perturbation. |
| `test_figure_pipeline.py` | Typed source producer over real temporary descriptors: prepared snapshot binding, real PNG rendering, complete publication, alternate inputs, and fail-closed incomplete runs. |
