...
Configuration lives in `manuscript/config.yaml`; the machine-readable descriptor lives in `data/example_descriptor.json`; public synthetic fixture bytes live in `data/fixtures/`; reusable validation, verification, figure-data, and figure-production logic lives in `src/data_descriptor/` (`descriptor.py`, `verification.py`, `figures.py`, `figure_pipeline.py`, `registry.py`). Manuscript figures under `manuscript/figures/`, their provenance registry under `output/figures/figure_registry.json`, and release artifacts under `output/reports/` are regenerated by `scripts/generate_figures.py` and `scripts/generate_release_artifacts.py` and must not be hand-edited.
