...
![Total, effective, and reserved-only product size for the grammar at seed={{GRAMMAR_SEED}}, generated by `fig_product_space_annotation` (`src/template_autopoiesis/manuscript/manuscript_figures.py`) directly from `Grammar.product_size` / `Grammar.effective_product_size` — not hand-typed.](../output/figures/fig_product_space.png){#fig:product_space width=80% fig-alt="Bar chart comparing total, effective, and reserved-only grammar product space."}
