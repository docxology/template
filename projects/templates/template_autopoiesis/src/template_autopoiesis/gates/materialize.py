...
from ..core.expand import Spec
from ..manuscript.emit_templates import emit_all
...
def _rewrite_kernel_imports(code: str) -> str:
    ...
    return (
        code.replace("from src.primitives", "from primitives")
        .replace("from .primitives", "from primitives")
        .replace("from template_autopoiesis.primitives", "from primitives")
    )
...
def _vendor_kernel_sources(spec: Spec, template_root: Path) -> dict[str, str]:
    ...
    prim_dir = template_root / "src" / "template_autopoiesis" / "primitives"
    files: dict[str, str] = {}
