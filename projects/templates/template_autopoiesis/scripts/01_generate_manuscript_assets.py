...
from src.grammar import load_grammar, KNOWN_DOMAINS
from src.project_paths import project_output_dirs


def _teal_slate_palette(n: int) -> list[str]:
    """Return n teal/slate brand colors."""
    base = ["#0d9488", "#1e293b", "#0f766e", "#334155", "#14b8a6",
            "#475569", "#0f172a", "#94a3b8"]
    return [base[i % len(base)] for i in range(n)]


def fig_stacked_product(out_dir: Path) -> Path:
    ...
    for i, (name, size) in enumerate(zip(names, sizes)):
        ...
        ax.text(0.32, bottom + size / 2, f"{name}\n({size})",
...
def fig_domain_coverage(out_dir: Path) -> Path:
    ...
    for bar, d in zip(bars, domains):
        ax.text(
            ...
            bar.get_height() + 0.02,
            d, ha="center", va="bottom", fontsize=9,
...
def fig_product_space_annotation(out_dir: Path) -> Path:
    ...
    for bar in bars:
        ...
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            h + 1,
            str(h), ha="center", va="bottom", fontsize=11, fontweight="bold",
        )

    # Zero-bar arrow for reserved-only if 0
    if reserved == 0:
        ax.annotate(
...
def main():
    ...
    fig_stacked_product(fig_dir)
    fig_domain_coverage(fig_dir)
    fig_product_space_annotation(fig_dir)
