...
from ..tools.integration import run_integration_demo
from ..tools.type_defs import IntegrationResult
...
def _filesystem_counts(project_root: Path) -> tuple[int, int, int]:
    ...
    test_files = sum(path.is_file() for path in (project_root / "tests").rglob("test_*.py"))
