...
- **`src/figures/figure_support.py::INTEGRATION_FIGURE_SPECS` (re-exported from `src/figures/figures.py`) is the figure-provenance source.** It does not auto-insert images — every figure still needs a real `![caption](figures/x.png){#fig:label}` block in the manuscript body, or its `@fig:` cross-reference renders as a literal `??`.
