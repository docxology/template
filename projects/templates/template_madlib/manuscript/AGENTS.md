...
- Keep long prose in generated body variables from `src/template_madlib/composition.py`.
...
- Expand Methods by editing `madlib.method_protocol`, `madlib.pipeline_phases`, `madlib.quality_probes`, `madlib.failure_modes`, `madlib.audit_rules`, `madlib.contribution_claims`, and `src/template_madlib/composition.py`, not by editing hydrated output.
...
- Add new placeholders only after adding corresponding keys in `src/template_madlib/manuscript_variables.py` and tests in `tests/test_manuscript_variables.py`.
