...
\ifXeTeX
\special{pdf:trailerid [ <36e5f47b75a672f31ff110f192b2fb39> <36e5f47b75a672f31ff110f192b2fb39> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <36e5f47b75a672f31ff110f192b2fb39> <36e5f47b75a672f31ff110f192b2fb39> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <36e5f47b75a672f31ff110f192b2fb39> <36e5f47b75a672f31ff110f192b2fb39> ]}
\fi
...
\breaktt{scripts/20\_render\_decks.py} resolves tokens once and calls
both renderers ---
\breaktt{infrastructure.rendering.slide\_deck.render\_pdf} and
\breaktt{infrastructure.rendering.pptx\_deck.render\_pptx} --- against
the identical resolved content, so PDF and PPTX cannot drift from each
other.
...
\breaktt{uv\ run\ python\ projects/templates/template\_pitch\_deck/scripts/20\_render\_decks.py}
...
\item
  ...
  of \texttt{Slide}.
\item
  \texttt{SlideBudget} --- the three published maximum lengths
  (\texttt{SHORT} \texorpdfstring{\ensuremath{\leq}}{<=} 11 slides, \texttt{MEDIUM} \texorpdfstring{\ensuremath{\leq}}{<=} 38, \texttt{LONG} \texorpdfstring{\ensuremath{\leq}}{<=} 58)
  and \breaktt{filter\_deck\_for\_budget}, a pure function that truncates
  a deck to a budget without mutating slide order or content.
\end{itemize}
...
--- never layout/drawing code. \breaktt{scripts/20\_render\_decks.py} is
the thin orchestrator that ties content loading, token resolution,
cliché linting, and the two infra renderers together into the six
published artifacts under \texttt{output/\{pdf,pptx\}/}.
...
\breaktt{scripts/20\_render\_decks.py} is deterministic: given the same
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{uv}\NormalTok{ run python scripts/20\_render\_decks.py}
