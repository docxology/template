...
| Token `{{METRIC}}` appears raw | Variable not injected in `src/template_autoresearch_project/manuscript/manuscript_tokens_core.py` or `manuscript_tokens_ml.py` (`manuscript/manuscript_variables.py` is only the re-export facade) | Add the variable binding there and re-run `z_generate_manuscript_variables.py` |
...
| Figure registry missing entries | New figure added without updating `src/template_autoresearch_project/manuscript/manuscript_tokens_figures.py` | Add figure variable, update tests, re-run analysis |
