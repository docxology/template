...
\makeatother
\hypersetup{
  pdftitle={A Domain Language for Specifying Controlled Methods},
...
\ifXeTeX
\special{pdf:trailerid [ <f90fda0ba0a77d543ba41d45db8f51f2> <f90fda0ba0a77d543ba41d45db8f51f2> ]}
\fi
\ifPDFTeX
\pdftrailerid{}
\pdftrailer{/ID [ <f90fda0ba0a77d543ba41d45db8f51f2> <f90fda0ba0a77d543ba41d45db8f51f2> ]}
\fi
\ifLuaTeX
\pdfvariable trailerid {[ <f90fda0ba0a77d543ba41d45db8f51f2> <f90fda0ba0a77d543ba41d45db8f51f2> ]}
\fi
...
that resolves to one of 18 controlled units across seven dimensions, and
...
template exemplar. On the architecture side, the DSL is exercised by a
zero-mock test suite under the repository's configured project coverage
gate, generates 14 artifacts (1 figures, 7 data files, 6 reports) per
pipeline run, and injects reproducibility metadata (configuration hash
\breaktt{a0f000565bef6a79}, build timestamp
\breaktt{2026-07-12T02:08:51Z}) into sec.~\ref{sec:reproducibility}.
...
All tests pass under the configured project coverage gate, with no
mocks.
...
\ExtensionTok{uv}\NormalTok{ run python scripts/pipeline/stage\_03\_render.py }\AttributeTok{{-}{-}project}\NormalTok{ templates/template\_methods\_paper}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{uv}\NormalTok{ run python scripts/runner/execute\_pipeline.py }\AttributeTok{{-}{-}project}\NormalTok{ templates/template\_methods\_paper }\AttributeTok{{-}{-}core{-}only}
\end{Highlighting}
\end{Shaded}
...
injected: \breaktt{a0f000565bef6a79} is the SHA-256 of
\breaktt{manuscript/config.yaml} at build time, and
\breaktt{2026-07-12T02:08:51Z} records when the variables were generated
...
goal is not to compete with BPL's substantially larger full compiler
pipeline \citep{bpl2026} --- with its Lark grammar, robot backend, and
...
generalized from wet-lab protocols to any controlled procedure.
