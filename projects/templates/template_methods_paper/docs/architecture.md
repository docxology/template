...
`src/template_methods_paper/methods_dsl/`, and mocks appearing in tests.

## Layer Reference

| Layer | Primary Files | Public API | Invariants | Testability |
|---|---|---|---|---|
| **`src/template_methods_paper/methods_dsl/` — DSL Library** | `vocabulary.py`, `units.py`, `model.py`, `validation.py`, `compiler.py`, `export.py`, `trust.py`, `examples_methods.py` | Re-exported from `src/template_methods_paper/__init__.py` (`compile_method`, `run_all_gates`, …) | Pure dataclasses and functions; no plotting, no file I/O, no `infrastructure.*` imports except the one declared exception in `_logging.py` | Direct unit tests against real `Method` fixtures |
...
src/template_methods_paper/methods_dsl/ ──→ [stdlib only, + one declared infrastructure.core.logging exception]
...
graph TD
    ...
    INIT[src/template_methods_paper/__init__.py] --> VOC
    INIT --> UNI
    INIT --> MDL
    INIT --> VAL
    INIT --> CMP
...
|---|---|---|
| Gate or scheduling logic inside `scripts/` | Cannot be unit-tested at the same granularity; drifts from the library | Move to `src/template_methods_paper/methods_dsl/validation.py` or `compiler.py`, add a test, call it from the script |
| `import matplotlib` inside `src/template_methods_paper/methods_dsl/` | Breaks library purity; needs a display backend | Return text/JSON from exporters; plot in `scripts/` |
...
1. **Construct the `Method`** in `src/template_methods_paper/methods_dsl/examples_methods.py` (or a
...
4. **Update the manuscript** — describe it in `manuscript/05_experimental_setup.md`
   ...
   concrete paths (e.g. `src/template_methods_paper/methods_dsl/compiler.py::compile_method()`).
