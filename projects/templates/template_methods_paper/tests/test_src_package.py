"""Tests for src/__init__.py — the top-level `from template_methods_paper import ...` re-export surface.

src/methods_dsl/AGENTS.md's re-export rule: "Re-export the public surface
from src/__init__.py so callers write `from template_methods_paper import compile_method`
...
top-level re-export contract — and the exact `from template_methods_paper import (...)` usage
...
`src/AGENTS.md` API table, so `from template_methods_paper import export_receipt` raised
ImportError. The first test below fails loudly if the surface drifts again.
"""

from __future__ import annotations

import template_methods_paper
from template_methods_paper.methods_dsl import __all__ as methods_dsl_all


def test_src_all_reexports_every_methods_dsl_public_symbol():
    missing = set(methods_dsl_all) - set(template_methods_paper.__all__)
...
def test_export_receipt_end_to_end_from_top_level_package():
    from template_methods_paper import all_example_methods, compile_method, export_receipt
