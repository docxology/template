...
def _write_visual_pdf(
    ...
    *,
    ...
    doc = canvas.Canvas(str(output_pdf), pagesize=letter, pageCompression=1)
    doc.setFillColor(color(background.fill_rgb))
    doc.rect(0, 0, page_width, page_height, stroke=0, fill=1)

    y = page_height - 22
    doc.setFillColor(color(background.text_rgb))
    doc.setFont("Helvetica-Bold", 11)
    doc.drawString(left_margin, y, title)
    doc.setFont("Helvetica", 7)
    doc.drawRightString(page_width - right_margin, y, f"{style.label} / {background.label}")
    y -= 22
    ...
    for segment in segments:
        ...
        doc.setFillColor(color(background.subdued_text_rgb))
        doc.setFont("Helvetica-Bold", 7)
        doc.drawString(left_margin, y, f"{segment.id} | {segment.classification}")
        y -= 14
        y = _draw_segment(
            doc,
            segment,
            decision_map.get(segment.id, ()),
            x=left_margin,
            y=y,
    ...
    doc.drawString(left_margin, 12, "Dev proof only: invented fixture text, source-safe masks, no operational content.")
    doc.save()
