...
Do not reuse the historical 153-test baseline below. The current explicit
full-suite evidence lives in `TODO.md`; the authoritative status is always the
fresh `uv run pytest ...` output plus the live coverage floor in
`pyproject.toml`.
...
| ISC-36 | suite | full baseline | live collected-test count passes | `uv run pytest tests/` |
