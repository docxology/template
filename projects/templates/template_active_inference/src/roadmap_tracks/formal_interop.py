...
def validate_formal_interop_artifacts(project_root: Path) -> list[str]:
    ...
    profile_rows_ok = all_rows(
        ontology_profile,
        lambda row: row.get("profile_kind") in {"gnn_variable", "graph_world_model", "toy_benchmark_model"}
        and row.get("model")
    ...
    profile_kinds = {str(row.get("profile_kind")) for row in ontology_profile.get("rows") or [] if row.get("profile_kind")}
    if (
