...
def validate_runtime_diagnostics(project_root: Path) -> list[str]:
    ...
    required_phases = set(payload.get("required_phases") or ["construction", "inference", "backend", "warning", "fallback"])
    phase_types = {str(row.get("phase")) for row in phase_rows if row.get("phase")}
    phase_rows_ok = bool(phase_rows) and required_phases.issubset(phase_types) and all(
