...
def validate_animation_frame_deltas(project_root: Path) -> list[str]:
    ...
    frame_hashes_present = bool(frames) and all(
        row.get("perceptual_hash") and row.get("sha256") and row.get("width") and row.get("height")
        for row in frames
    )
    if payload.get("all_frame_hashes_present") is not True or payload.get(
        "all_frame_hashes_present"
    ) != frame_hashes_present:
    ...
    if payload.get("all_adjacent_hashes_distinct") is not True or payload.get(
        "all_adjacent_hashes_distinct"
    ) != adjacent_hashes_distinct:
