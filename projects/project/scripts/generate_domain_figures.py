...
class DomainFigureGenerator:
    ...
    def generate_domain_figures(self, domain_name: str,
        ...
        analyses = analyzer.analyze_all_domains(list(all_terms.items()), texts)
    ...
    def generate_all_domain_figures(self, corpus_file: Optional[Path] = None) -> Dict[str, Any]:
        ...
        for domain in domains:
            try:
                ...
                logger.error(f"Failed to generate figures for {domain}: {e}")
    ...
    def _generate_term_frequency_plot(self, domain_name: str,
        ...
        plt.tight_layout()
        filename = f"{domain_name}_term_frequency.png"
    ...
    def _generate_ambiguity_plot(self, domain_name: str,
        ...
        plt.tight_layout()
        filename = f"{domain_name}_ambiguity.png"
        filepath = self.figures_dir / filename
        fig.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close(fig)
...
def main():
    ...
    if args.domain == 'all':
        ...
        print(f"Generated figures for {results['domains_processed']} domains")
        print(f"Total figures: {results['total_figures_generated']}")
    else:
        results = generator.generate_domain_figures(args.domain, args.corpus_file)

        if 'error' not in results:
            print(f"Figures generated for {args.domain}")
            print(f"Terms visualized: {results.get('terms_visualized', 0)}")
            print(f"Figures created: {results.get('figures_generated', 0)}")
        else:
            print(f"Figure generation failed for {args.domain}: {results['error']}")
