...
class TestTerminologyExtractor:
    ...
    def test_extract_terms_frequency_filtering(self, extractor: TerminologyExtractor) -> None:
        ...
        texts = [
            "eusocial colony eusocial",  # "eusocial" appears 3 times
            "colony behavior",          # "colony" appears 2 times
            "foraging"                  # "foraging" appears 1 time
        ]

        # With min_frequency=2
        terms = extractor.extract_terms(texts, min_frequency=2)

        # Should include "eusocial" and "colony" but not "foraging"
        term_texts = [term.text for term in terms.values()]
        assert "eusocial" in term_texts
        assert "colony" in term_texts
        assert "foraging" not in term_texts
...
class TestTerminologyExtractionIntegration:
    ...
    def test_term_evolution_tracking(self) -> None:
        ...
        texts2 = ["foraging behavior"] * 2  # Lower frequency
        ...
        # Should find "foraging" and "behavior" in second set
        term_texts2 = [term.text for term in terms2.values()]
        assert "foraging" in term_texts2
        assert "behavior" in term_texts2
