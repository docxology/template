...
\title{A \texttt{template/} approach to Reproducible Generative Research\\\normalsize Architecture and Ergonomics from Configuration through Publication}
\author{Daniel Ari Friedman\\\footnotesize{Active Inference Institute}\\\footnotesize{\texttt{daniel@activeinference.institute}}\\\footnotesize{\href{https://orcid.org/0000-0001-6232-9096}{ORCID: 0000-0001-6232-9096}} \\ \footnotesize{\href{https://doi.org/10.5281/zenodo.19139090}{DOI: 10.5281/zenodo.19139090}} \\ \footnotesize{2026-03-20}}
\date{2026-03-20}
...
separates 12 reusable infrastructure subpackages (\textasciitilde150
Python modules, validated by \textasciitilde3,083 tests) from
self-contained project workspaces, connected by an eight-stage build
pipeline progressing from environment sanitization through test
...
meta-analysis pipeline (\texttt{act\_inf\_metaanalysis}, 505 tests), and
this self-referential architectural analysis (\texttt{template}, 65
tests)---achieving 100\% pipeline success with zero mock violations. The
fact that these words, these metrics, and the figures accompanying them
were generated by the very pipeline they describe is itself a
demonstration of the system's self-productive capacity: the manuscript
is not merely \emph{about} \texttt{template/} but \emph{of} it, rendered
through the same eight-stage pipeline, validated by the same test suite,
and watermarked by the same steganographic layer documented herein. A
comparative feature analysis against nine peer tools across fourteen
dimensions confirms that \texttt{template/} uniquely integrates all
...
multi-project management, AI-agent documentation, agentic skill
protocol, interactive TUI, Zero-Mock policy, manuscript rendering, and
pipeline orchestration---within a single enforced pipeline.
\texttt{template/} is open source under the Apache 2.0 License at
...
is developed in the \hyperref[comparison-to-existing-tools]{Comparison
to Existing Tools} section; the summary table below captures the gap
landscape.
...
\item
  ...
  scripts, data). 12 infrastructure subpackages comprising
  \textasciitilde150 Python modules provide reusable services; projects
...
\item
  ...
  test doubles. \textasciitilde3,083 infrastructure tests and 708+
...
\item
  ...
  \texttt{infrastructure.rendering} does not need to read source code:
  it reads the \texttt{rendering/SKILL.md}, which declares the module's
...
documented in the \hyperref[methods]{Methods}; the figures were rendered
by the visualization code validated by the test suite described in
\hyperref[quality-assurance]{Quality Assurance}; the PDF carrying these
words was assembled by the same eight-stage pipeline whose architecture
is the subject of the \hyperref[results]{Results}. This self-productive
...
\item
  A detailed specification of the eight-stage build pipeline (Stages
  00--07), from environment sanitization through executive report
  generation.
\item
  A comparative analysis positioning \texttt{template/} against ten peer
  tools---Snakemake, Nextflow, CWL, Quarto, Jupyter Book, R Markdown,
  DVC, Typst, Overleaf, OpenAI Prism---across fourteen feature
  dimensions, demonstrating that \texttt{template/} uniquely integrates
  all eleven distinctive capabilities.
\item
  An empirical evaluation of the system across three heterogeneous
  exemplar projects (\texttt{code\_project},
  \texttt{act\_inf\_metaanalysis}, \texttt{template}), demonstrating
  scalability, coverage metrics, and pipeline timing.
\item
...
The \hyperref[methods]{Methods} describe the Two-Layer Architecture,
Thin Orchestrator pattern, pipeline stages, and AI collaboration model.
\hyperref[results]{Results} present quantitative metrics from
multi-project execution, coverage analysis, and steganographic
benchmarks. The \hyperref[the-zero-mock-tradeoff]{Discussion} addresses
the Zero-Mock tradeoff, scalability implications, a detailed tool
comparison, and future directions. The
\hyperref[infrastructure-module-reference]{Infrastructure Module
Reference} provides detailed documentation for all twelve subpackages.
\hyperref[security-and-provenance]{Security and Provenance} describes
the steganographic and cryptographic integrity layer. The
\hyperref[appendices]{Appendices} provide pipeline, configuration, and
...
\textbf{Infrastructure Layer} (\texttt{infrastructure/}): 12 Python
subpackages comprising \textasciitilde150 modules and providing reusable
...
\texttt{../figures/} and data in \texttt{output/data/}. Scripts follow
the Thin Orchestrator pattern, importing logic from \texttt{src/}
...
The
\hyperref[documentation-duality-and-the-agentic-skill-architecture]{Documentation
...
\texttt{infrastructure/rendering/SKILL.md}:
...
All ten active infrastructure modules carry \texttt{SKILL.md} files. A
companion \texttt{PAI.md} (Personal AI Infrastructure) file at the top
of the \texttt{infrastructure/} directory documents the collection's
role within a researcher's broader AI-assisted ecosystem---capturing
import rules, testing obligations, and cross-module dependencies in the
format used by Codomyrmex-style PAI frameworks.

This coverage is a structural invariant enforced by the Documentation
Duality standard---every new module added to \texttt{infrastructure/}
must carry \texttt{AGENTS.md}, \texttt{README.md}, and \texttt{SKILL.md}
before it is accepted by the pipeline.
...
\tightlist
\item
  \texttt{infrastructure.llm} → MCP \textbf{Tool} (\texttt{query},
  \texttt{apply\_template}) + MCP \textbf{Prompt} (research prompt
  templates)
\item
  \texttt{infrastructure.rendering} → MCP \textbf{Tool}
  (\texttt{render\_pdf}, \texttt{render\_html}) + MCP \textbf{Resource}
  (rendered PDFs as URI-addressable resources)
\item
  \texttt{infrastructure.validation} → MCP \textbf{Tool}
  (\texttt{validate\_pdf\_rendering}, \texttt{validate\_markdown})
\item
  \texttt{infrastructure.publishing} → MCP \textbf{Tool}
  (\texttt{publish\_to\_zenodo}, \texttt{generate\_citation\_bibtex}) +
  MCP \textbf{Resource} (DOI registry)
\item
  \texttt{infrastructure.steganography} → MCP \textbf{Tool}
  (\texttt{SteganographyProcessor.process}) + MCP \textbf{Resource}
  (hash manifests)
\end{itemize}

An agent orchestrating a full research pipeline could, in principle,
compose these MCP tools to reproduce the entire eight-stage pipeline
programmatically---discovering available tools via \texttt{SKILL.md}
...
aligned with these frameworks, the eight-stage pipeline provides the
planning scaffold, and the checkpoint system provides the memory layer.
...
\texttt{./run.sh\ -\/-pipeline}, and produce a byte-for-byte identical
...
DOI registration (supported by \texttt{infrastructure.publishing}), this
...
\hyperref[the-zero-mock-tradeoff]{Zero-Mock Tradeoff} discussion. To our
...
\texttt{template/} was evaluated through a multi-project pipeline
...
\bottomrule\noalign{}
\endlastfoot
\texttt{code\_project} & 7 & \textasciitilde40s & 39/39 & 90\%+ \\
\texttt{act\_inf\_metaanalysis} & 7 & \textasciitilde60s & 505/505 &
90\%+ \\
\texttt{template} & 7 & \textasciitilde25s & 65/65 & 90\%+ \\
\end{longtable}
}

\textbf{Overall success rate}: 100.0\% (3/3 projects) \textbf{Total
pipeline duration}: \textasciitilde125s \textbf{Average per project}:
\textasciitilde42s

\emph{Timing measured on Apple M-series hardware with SSD; analysis
scripts use fixed random seeds. Figures are representative; actual
duration scales with system load, test suite size, and manuscript
complexity.}

The \texttt{act\_inf\_metaanalysis} project, with its 505 tests and
programmatically generated figures, represents the most computationally
intensive exemplar---yet completes in under one minute, confirming that
the Zero-Mock policy's real-method overhead remains tractable at this
scale.
...
\endlastfoot
Test files & 163+ \\
Total tests & \textasciitilde3,083 \\
...
This test suite exercises all twelve infrastructure modules, including
the rendering pipeline (Pandoc/XeLaTeX integration), steganographic
...
\bottomrule\noalign{}
\endlastfoot
\texttt{core} & 28 & \texttt{get\_logger}, \texttt{load\_config},
\texttt{TemplateError} \\
\texttt{documentation} & 6 & \texttt{FigureManager},
\texttt{generate\_glossary} \\
\texttt{llm} & 30 & LLM review, literature search, translation \\
\texttt{project} & 2 & \texttt{discover\_projects}, workspace
management \\
\texttt{publishing} & 9 & Citation generation (APA, BibTeX, MLA),
Zenodo \\
\texttt{rendering} & 12 & PDF rendering, Pandoc filters, HTML reports \\
...
\texttt{steganography} & 8 & Metadata injection, QR overlays, hashing \\
\texttt{validation} & 22 & PDF validation, Markdown checking, CLI \\
(+ config, docker) & --- & Configuration, containerization \\
\textbf{Total} & \textbf{\textasciitilde150} & \\
\end{longtable}
}

\emph{The \textasciitilde150 figure includes approximately 13 additional
modules in the \texttt{config/} and \texttt{docker/} subpackages
(configuration schemas and containerization utilities) not enumerated
individually above.}

All 12 modules have 100\% documentation coverage at Tiers 1--2
(\texttt{AGENTS.md}, \texttt{README.md}); the 10 active subpackages
additionally carry \texttt{SKILL.md} for Tier-3 agentic skill discovery.
This places \texttt{template/} among the first research software
frameworks to implement an MCP-aligned skill layer
\citep{anthropic2024mcp} across its infrastructure stack.

\subsection{Agentic Skill Documentation
Coverage}\label{agentic-skill-documentation-coverage}

The three-tier
\hyperref[documentation-duality-and-the-agentic-skill-architecture]{skill
protocol} achieves complete coverage across all infrastructure modules:
...
\bottomrule\noalign{}
\endlastfoot
System (\texttt{CLAUDE.md}) & 1 & 100\% \\
Structural (\texttt{AGENTS.md}) & 12+ per-directory & 100\% \\
Skill (\texttt{SKILL.md}) & 12 modules & 100\% \\
PAI (\texttt{PAI.md}) & 1 (infrastructure-level) & --- \\
Human (\texttt{README.md}) & 12+ per-directory & 100\% \\
\end{longtable}
}

This four-layer coverage creates 12 fully described, MCP-mappable tool
endpoints that a sufficiently capable agent could invoke without any
source-code access. The aggregate documentation footprint (148+ files)
represents a deliberate engineering investment: each documentation file
is not commentary but a specification, enforcing architectural
constraints through structured natural language \citep{lau2025aicoding}.
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0682}}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{code\_project} & \textasciitilde20 & \textless{} 0.3s &
\textless{} 0.05s & \textless{} 0.8s & \textless{} 0.4s & \textless{}
1.5s \\
\texttt{act\_inf\_metaanalysis} & \textasciitilde80 & \textless{} 0.3s &
\textless{} 0.05s & \textless{} 2.0s & \textless{} 0.4s & \textless{}
2.8s \\
\texttt{template} & \textasciitilde30 & \textless{} 0.3s & \textless{}
0.05s & \textless{} 0.9s & \textless{} 0.4s & \textless{} 1.6s \\
\end{longtable}
}

\emph{Performance measured on Apple M-series hardware with SSD,
single-threaded execution. Values represent wall-clock time; actual
performance scales with PDF page count and system I/O.}
...
This manuscript is itself a product of the \texttt{template/} pipeline,
demonstrating its self-productive capability. The \texttt{template}
project's \texttt{src/template/introspection.py} module programmatically
analyzes the repository and generates four architecture figures, all
presented below. The numeric values in the tables above---module counts,
test counts, file totals---were not typed by hand but injected at build
time by the \texttt{\$\{variable\}} substitution system described in the
Methods, reading live metrics from the repository's own structure. The
figures below were rendered by the same \texttt{architecture\_viz.py}
module whose font-size constraints are specified in the
\hyperref[quality-assurance]{Quality Assurance} section. In this way,
the manuscript does not merely \emph{describe} but \emph{enacts} the
...
\textbf{Figure 1}: Two-Layer Architecture separating the generic,
reusable infrastructure layer (12 subpackages, upper panel) from
domain-specific project workspaces (lower panel), connected by the
eight-stage pipeline. Each module box displays its Python file count and
a documentation badge (A = \texttt{AGENTS.md}, R = \texttt{README.md}, S
= \texttt{SKILL.md}, P = \texttt{PAI.md}; a dot \texttt{·} means
absent). Project boxes show chapter and test counts. All labels are
drawn from live repository introspection at render time; font sizes
follow the 16 pt \hyperref[visualization-standards]{accessibility
floor}.

\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline Stage Flow}]{../figures/pipeline_stages.png}}
\textbf{Figure 2}: Sequential eight-stage build pipeline (Stage 00--07,
plus a pre-step clean stage). Viridis colour progression encodes stage
ordering. Each box includes a short description of the stage's primary
action. Stage names and descriptions are generated from
\texttt{PipelineStage} objects returned by
\texttt{report.pipeline\_stages}, ensuring the figure reflects the
actual pipeline.

\pandocbounded{\includegraphics[keepaspectratio,alt={Infrastructure Module Inventory}]{../figures/module_inventory.png}}
\textbf{Figure 3}: Python source-file count per infrastructure
subpackage, sorted descending. Bar colour intensity scales with file
count. Documentation badges \texttt{{[}ARSP{]}} appear to the right of
each count (A = \texttt{AGENTS.md}, R = \texttt{README.md}, S =
\texttt{SKILL.md}, P = \texttt{PAI.md}; a dot \texttt{·} means absent).
Total file count is annotated at chart bottom.

\subsection{Comparative Feature
Analysis}\label{comparative-feature-analysis}

To contextualize \texttt{template/}'s contributions, we compare its
feature set against nine established tools. The full capability matrix
(14 capabilities × 10 tools) is rendered as a colour-coded heatmap in
Figure 4 and reproduced as a text table in Appendix F. Rows are grouped
into three categories --- \emph{Core Pipeline}, \emph{Quality \&
Security}, and \emph{Ecosystem} --- separated by horizontal dividers.
...
\textbf{Figure 4}: Comparative feature matrix (14 capabilities × 10
tools). Colour scale: green \textbf{✓} = full native support; yellow
\textbf{◐} = partial / plugin-based; red \textbf{---} = absent. The
\texttt{template/} column is outlined in blue. Capabilities are grouped
into \emph{Core Pipeline} (rows 1--2), \emph{Quality \& Security} (rows
3--8), and \emph{Ecosystem} (rows 9--14). \texttt{template/} is, to our
knowledge, the only open-source system that integrates all twelve unique
capabilities (pipeline orchestration through zero-mock policy, plus
optional containerization) within a single cohesive architecture.
Snakemake, Nextflow, and CWL provide superior distributed execution
support not yet in \texttt{template/}.

¹ Nextflow 25.04.0: data-lineage provenance tracking at build level, not
document level. ² DVC: content-addressed artifact versioning via object
store. ³ DVC: remote storage integration (S3, GCS, Azure) without native
...
The \hyperref[zero-mock-testing-policy]{Zero-Mock testing policy} is
\texttt{template/}'s most distinctive design decision. By prohibiting
all mock objects, we gain confidence that tests exercise real code
paths---a pytest run against the template genuinely invokes
\texttt{pandoc}, writes to disk, and parses real YAML. The cost is test
duration: the full infrastructure test suite (\textasciitilde3,083
...
\item
  ...
  \texttt{grep\ -rn\ "MagicMock\textbackslash{}\mid{}unittest.mock\textbackslash{}\mid{}@patch"\ tests/}
...
The \hyperref[the-gap]{gap analysis} established that no single tool
integrates all six cross-cutting concerns. Here we synthesize the
\hyperref[appendix-matrix]{fourteen-dimension comparison} into three
structural insights. First, the landscape bifurcates: workflow managers
(Snakemake \citep{koster2012snakemake}, Nextflow
\citep{ditommaso2017nextflow}, CWL \citep{amstutz2016cwl}) provide
distributed execution but no manuscript support; publication tools
(Quarto \citep{allaire2024quarto}, Jupyter Book, R Markdown
\citep{xie2018dynamic}, Typst \citep{madje2023typst}, Overleaf
\citep{overleaf2025}, Prism \citep{openai2026prism}) author documents
but embed no integrity guarantees; and DVC \citep{iterative2024dvc}
versions artifacts without orchestrating pipelines. \texttt{template/}
occupies the intersection, sacrificing distributed execution for unified
...
(\hyperref[documentation-duality-and-the-agentic-skill-architecture]{Documentation
Duality}, \hyperref[the-ai-collaboration-model]{AI Collaboration}).
...
that tests \hyperref[the-zero-mock-tradeoff]{reflect reality}. The
steganographic provenance layer ensures that published artifacts carry
their own \hyperref[security-and-provenance]{authentication}. The
\hyperref[appendix-matrix]{comparative analysis} confirms that no
existing tool integrates all eleven distinctive capabilities---testing
enforcement, coverage thresholds, cryptographic provenance,
steganographic watermarking, multi-project management, AI-agent
documentation, the agentic skill protocol, interactive TUI, Zero-Mock
policy, manuscript rendering, and pipeline orchestration---within a
single enforced pipeline.
...
This section provides a detailed reference for all 12 infrastructure
...
comprises \textasciitilde150 Python modules validated by 3,083 tests.
...
\subsection{Infrastructure Maturity
Summary}\label{infrastructure-maturity-summary}

The twelve-module architecture achieves 100\% Tier 1--2 documentation
...
the Model Context Protocol \citep{anthropic2024mcp}, making the
...
\hyperref[documentation-duality-and-the-agentic-skill-architecture]{Documentation
Duality} and \hyperref[the-ai-collaboration-model]{AI Collaboration}
sections. The combination of high coverage, complete documentation, and
protocol-aligned discoverability positions \texttt{template/}'s
infrastructure as deployment-ready research software rather than a
prototype, satisfying the executability and metadata quality indicators
defined by Garijo et al.'s FAIRsoft evaluator
\citep{garijo2024fairsoft}.
...
\tightlist
\item
  Execute the standard eight-stage pipeline for the target project.
\item
  The \texttt{secure\_run.sh} script invokes
  \texttt{SteganographyProcessor}.
\item
...
\item
  ...
  future \texttt{infrastructure.provenance} module would close both gaps
  automatically.
\end{itemize}
...
\begin{Highlighting}[]
\NormalTok{template/}
\NormalTok{├── infrastructure/           \# Layer 1: Shared services (\textasciitilde{}150 .py files)}
\NormalTok{│   ├── core/                 \# 28 files — logging, config, exceptions}
\NormalTok{│   ├── documentation/        \# 6 files — figure management, glossary}
\NormalTok{│   ├── llm/                  \# 30 files — Ollama integration, literature}
\NormalTok{│   ├── project/              \# 2 files — project discovery}
\NormalTok{│   ├── publishing/           \# 9 files — citation generation, Zenodo}
\NormalTok{│   ├── rendering/            \# 12 files — Pandoc + XeLaTeX + reports}
\NormalTok{│   ├── reporting/            \# 14 files — coverage parsing, reports}
\NormalTok{│   ├── scientific/           \# 6 files — stability, benchmarking}
\NormalTok{│   ├── steganography/        \# 8 files — watermarking, hashing}
\NormalTok{│   ├── validation/           \# 22 files — PDF + Markdown validation}
\NormalTok{│   ├── config/               \# Configuration}
...
\NormalTok{│   ├── act\_inf\_metaanalysis/ \# Meta{-}analysis pipeline}
\NormalTok{│   └── biology\_textbook/     \# Domain content exemplar (when populated)}
\NormalTok{├── projects\_in\_progress/     \# Work{-}in{-}progress (template, etc.)}
\NormalTok{├── tests/                    \# Infrastructure test suite (160+ files, \textasciitilde{}3,050 tests)}
\NormalTok{├── docs/                     \# Repository documentation (90+ files, 12 subdirectories)}
...
\texttt{act\_inf\_metaanalysis} & Active & Active inference
meta-analysis & Multiple modules & 505 & 90\%+ & \textasciitilde80 \\
\texttt{template} & In-progress & Meta-architecture &
\texttt{introspection.py} & 65 & 90\%+ & \textasciitilde30 \\
\end{longtable}
}
...
\texttt{reference/} (API, FAQ, glossary), \texttt{modules/} (12
...
\hyperref[comparative-feature-analysis]{Figure 4} for a colour-coded
heatmap rendering of this table.
