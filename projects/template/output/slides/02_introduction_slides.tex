...
\begin{frame}{Research Tools as Epistemic Infrastructure}
...
{[}@baker2016reproducibility{]}. Freedman et al.~estimate that the
biomedical industry alone loses \$28 billion annually to irreproducible
preclinical research {[}@freedman2015economics{]}. But reproducibility
...
al. {[}@nosek2018preregistration{]} have argued that the preregistration
...
disconnected tools: LaTeX editors {[}@lamport1994latex{]} for prose,
...
rendering pipeline. Pimentel et al. {[}@pimentel2019jupyter{]} analyzed
...
workflows. Peng {[}@peng2011reproducible{]} argues that reproducibility
...
{[}@barba2018terminologies{]} documents how ``reproducibility,''
...
introduces execution-order fragility {[}@pimentel2019jupyter{]} and does
not naturally support the separation of logic from orchestration that
characterizes maintainable research software.

\textbf{R Markdown} {[}@xie2018dynamic{]} pioneered knitr-based dynamic
...
\textbf{Typst} {[}@madje2023typst{]} is an emerging markup-based
...
{[}@piccolo2016tools{]} systematically survey tools for computational
...
all three. Stodden et al. {[}@stodden2016enhancing{]} advocate for
enhanced computational method transparency. The FAIR principles
{[}@wilkinson2016fair{]}---Findable, Accessible, Interoperable,
...
{[}@lamprecht2020towards{]} formalize ``Towards FAIR Principles for
Research Software,'' providing the conceptual scaffolding that Barker et
al. {[}@barker2022fair4rs{]} would soon operationalize as the FAIR4RS
...
address. Cohen et al. {[}@cohen2021four{]} characterize the four pillars
...
framework. Garijo et al. {[}@garijo2024fairsoft{]} operationalize
...
completeness. Goble et al. {[}@goble2020fair{]} extend FAIR to
...
Nüst et al. {[}@nust2017containerization{]} introduce the
...
and SLSA {[}@openssf2023slsa{]} (Supply-chain Levels for Software
...
Despite advances in FAIR4RS principles {[}@barker2022fair4rs;
@lamprecht2020towards{]}, automated FAIR software assessment
{[}@garijo2024fairsoft{]}, FAIR computational workflow standards
{[}@goble2020fair{]}, supply-chain attestation frameworks
{[}@torresarias2019intoto; @openssf2023slsa{]}, and the preregistration
revolution {[}@nosek2018preregistration{]}, no existing system
...
is developed in the \hyperlink{comparison-to-existing-tools}{Comparison
...
\begin{frame}[fragile]{\texttt{template/}: An Integrated Solution}
...
{[}@boettiger2015docker{]}---the template enforces integrity at the
architectural level. It realizes Gentleman and Temple Lang's research
compendium vision {[}@gentleman2007research{]} at repository scale,
...
{[}@ram2013git{]}. It stands on four primary pillars:
...
\item
  ...
  {[}@anthropic2024mcp{]}---define each infrastructure module as a
  ...
  agent literature: Yao et al.'s ReAct framework {[}@yao2023react{]}
  ...
  {[}@schick2023toolformer{]} showed that self-supervised tool use can
  be bootstrapped from natural language; Wang et al.'s Voyager
  {[}@wang2023voyager{]} proved that growing skill libraries enable
...
documented in the \hyperlink{methods}{Methods}; the figures were
rendered by the visualization code validated by the test suite described
in \hyperlink{quality-assurance}{Quality Assurance}; the PDF carrying
these words was assembled by the same eight-stage pipeline whose
architecture is the subject of the \hyperlink{results}{Results}. This
self-productive loop---where the system that is described is also the
system that produces the description---is not incidental but structural,
a concrete demonstration that \texttt{template/} can sustain the full
lifecycle from source code to published artifact within a single,
...
\item
  ...
  the W3C PROV data model {[}@moreau2013provdm{]} and SLSA
  {[}@openssf2023slsa{]}.
\item
...
The \hyperlink{methods}{Methods} describe the Two-Layer Architecture,
Thin Orchestrator pattern, pipeline stages, and AI collaboration model.
\hyperlink{results}{Results} present quantitative metrics from
multi-project execution, coverage analysis, and steganographic
benchmarks. The \hyperlink{the-zero-mock-tradeoff}{Discussion} addresses
the Zero-Mock tradeoff, scalability implications, a detailed tool
comparison, and future directions. The
\hyperlink{infrastructure-module-reference}{Infrastructure Module
Reference} provides detailed documentation for all twelve subpackages.
\hyperlink{security-and-provenance}{Security and Provenance} describes
the steganographic and cryptographic integrity layer. The
\hyperlink{appendices}{Appendices} provide pipeline, configuration, and
