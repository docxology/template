...
def calculate_coherence(values: np.ndarray) -> float:
    """Calculate coherence score (stub).
    ...
    if len(values) < 2:
        return 1.0
    return float(1.0 - np.std(values) / (np.mean(np.abs(values)) + 1e-10))


def calculate_consistency(values1: np.ndarray, values2: np.ndarray) -> float:
    """Calculate consistency between two sequences (stub).
    ...
    return float((correlation + 1) / 2)  # Map -1..1 to 0..1
