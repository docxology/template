...
We have validated scaling with three canonical exemplars under `projects/`—always present for onboarding and tooling—and with this manuscript from `projects_in_progress/template` (${project_template_template_test_count} tests) before promotion adds the meta-analysis tree to automated discovery menus.
