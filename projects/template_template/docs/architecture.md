...
| **Project** (Layer 2) | `projects_in_progress/template/` | `src/template/`, manuscript, scripts, tests |
...
- **`discover_projects(repo_root)`**: manuscript workspaces; `projects/` wins over `projects_in_progress/` on name collision
...
Both `generate_architecture_viz.py` and `generate_manuscript_metrics.py` resolve the Layer-1 repo via `_locate_repo_root()` (works from `projects_in_progress/template/` symlinks), then delegate to `src/template/`.
