...
- **Project source**: 90%+ coverage for `projects_in_progress/template/src/`
...
```bash
# Quick run
PYTHONPATH=. uv run pytest projects_in_progress/template/tests/ -v --tb=short

# With coverage
PYTHONPATH=. uv run pytest projects_in_progress/template/tests/ -v --tb=short \
  --cov=projects_in_progress/template/src --cov-report=term-missing

# Single test file
PYTHONPATH=. uv run pytest projects_in_progress/template/tests/test_meta.py -v
```

## Adding New Tests

1. Create `test_<module>.py` in `projects_in_progress/template/tests/`
