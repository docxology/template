...
```bash
# Run tests
PYTHONPATH=. uv run pytest projects_in_progress/template/tests/ -v

# Generate figures + metrics + rendered manuscript
PYTHONPATH=. uv run python projects_in_progress/template/scripts/generate_architecture_viz.py
PYTHONPATH=. uv run python projects_in_progress/template/scripts/generate_manuscript_metrics.py
