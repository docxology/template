...
Usage::
    ...
    uv run python projects/template_code_project/scripts/00_preflight.py
"""
...
def _puppeteer_cache_has_chrome() -> bool:
    ...
    for shell_dir in cache_root.glob("chrome-headless-shell*"):
        if shell_dir.is_dir():
            return True
    # Some Puppeteer versions nest under chrome-headless-shell/<platform>-<ver>/
...
def main() -> int:
    if not _project_manuscript_has_mermaid():
        # No Mermaid → no Chrome requirement.
        return 0
    ...
    sys.stderr.write(
        "\n".join(
            [
                ...
                "",
                "Full detail: projects/template_code_project/docs/troubleshooting.md",
