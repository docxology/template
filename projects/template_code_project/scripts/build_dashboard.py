#!/usr/bin/env python3
"""Build the interactive simulation dashboard for template_code_project.
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=float,
        nargs="+",
        default=[float(v[0]) if isinstance(v, (list, tuple)) else float(v)
                 for v in cfg.get("quadratic_A", [[1.0]])],
    ...
    p.add_argument(
        "--landscape-x-min", type=float, default=-3.0,
        help="x range (lower) for the 1-D objective landscape panel",
    )
    p.add_argument(
        "--landscape-x-max", type=float, default=5.0,
...
def _build_dashboard(args: argparse.Namespace, payload: dict) -> InteractiveDashboard:
    ...
    d.add_note(
        "Stable step bound: 2/λ_max(A). Step sizes ≥ this bound are expected to diverge."
    )
    d.add_note(
        "Closed-form minimum: x* = A^{-1} b; f(x*) is reported in the hyperparameters block."
    )
    ...
    d.add_panel(
        Panel(
            ...
            description=(
                "Stable α (< 2/λ_max(A)) → monotone descent to f(x*). "
                "α ≥ stable bound → divergence."
            ),
            traces=traj_traces,
            layout={
    ...
    d.add_panel(
        Panel(
            ...
            description=(
                "Stable region: distance ≪ tol; divergent region: distance "
                "grows without bound (clipped on log-y)."
            ),
            traces=[
                {
                    "type": "scatter",
                    ...
                    "x": sw["alphas"],
                    "y": [
                        max(d, 1e-15) for d in sw["final_dist"]
                    ],
                    "line": {"color": "#a78bfa"},
                    "marker": {"size": 5},
                },
...
""",
    ...
    d.add_panel(
        Panel(
            ...
            description=(
                "Bar chart of A's eigenvalues with the stable-step "
                "bound 2/λ_max marked."
            ),
            traces=[
                {
                    "type": "bar",
