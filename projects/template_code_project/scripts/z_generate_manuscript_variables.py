...
def count_output_artifacts() -> dict[str, int]:
    ...
    Each category counts only the file types it claims to enumerate, so the
    reproducibility table reports a meaningful number rather than including
    bookkeeping files (e.g., `figures/figure_registry.json`) under
...
def substitute_manuscript_files(variables: dict[str, str]) -> Path:
    ...
    # Exclude developer-facing documentation (AGENTS.md, README.md, SYNTAX.md)
    # which intentionally contains literal `{{TOKEN}}` examples and
    # `\ref{eq:...}` snippets that would otherwise pollute the integrity scan
    # over `output/manuscript/`.
    _DOC_ONLY = {"AGENTS.md", "README.md", "SYNTAX.md"}
    md_files = sorted(
        f for f in MANUSCRIPT_DIR.glob("*.md") if f.name not in _DOC_ONLY
    )
