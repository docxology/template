...
The calculus of indications begins with a single primitive: the act of
\textbf{distinction}. To distinguish is to create a boundary that
separates two regions---an inside and an outside. This act is
represented by the \textbf{mark} or \textbf{cross}:
...
\textbf{Verification Method}: Each theorem's LHS and RHS are reduced to
canonical form; equality confirms the theorem.
...
The calculus provides formal tools for studying: - Distinction as
primitive cognitive act - Negation as boundary crossing - Self-reference
as oscillation - Attention as juxtaposition
...
C3 & Generation &
\(\langle\langle a \rangle a \rangle = \langle\ \rangle\) \\
C4 & Integration & \(\langle\ \rangle a = \langle\ \rangle\) (in
enclosure) \\
C5 & Occultation & \(\langle\langle a \rangle\rangle a = a\) \\
C6 & Iteration & \(aa = a\) \\
C7 & Extension &
