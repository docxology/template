...
(Last refreshed count: **725** on 2026-08-08 UTC — point-in-time; re-derive with the command above, the literal drifts as the tree changes.)
...
**Exemplar `pytest --collect-only` totals** (derived live in each project's declared environment; coverage snapshot last updated 2026-08-08; coverage values use the shared `release` test profile in each project's environment):
...
| `template_pools_rules_tools` | 261 | 94.91 % |
| `template_prose_project` | 134 | 99.58 % |
| `template_redacted_report` | 143 | 97.30 % |
| `template_registered_report` | 37 | 96.61 % |
| `template_search_project` | 315 | 97.44 % |
| `template_sia` | 68 | 99.69 % |
