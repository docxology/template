...
**Generated from live repo state on 2026-07-13 (UTC).** Volatile literals are re-derived on every run: tracked `infrastructure/` Python-file count via `git ls-files infrastructure | grep .py` (**687**), project-scope + publishing test collection via `pytest --collect-only` (**411** / **669**), the public exemplar roster, and the importable module list. The per-exemplar test/coverage snapshot table is a measured snapshot with source-commit and source-hash provenance in
...
(Last refreshed count: **687** on 2026-07-13 UTC — point-in-time; re-derive with the command above, the literal drifts as the tree changes.)
...
Result: **411** project-scope infrastructure tests collected and **669** publishing tests collected. Full behavioral gates still live in CI and in the verification commands listed by the relevant `AGENTS.md` files.

**Exemplar `pytest --collect-only` totals** (derived live in each project's declared environment; coverage snapshot last updated 2026-07-11; `template_active_inference` coverage preserved from its 2026-06-05 project-local gate run — see note below):

| Project | Tests collected | `src/` line+branch coverage |
|---------|-----------------|----------------------------|
| `template_active_inference` | 710 | 93.55 % |
| `template_autopoiesis` | 506 | 96.41 % |
...
| `template_literature_meta_analysis` | 886 | 96.77 % |
| `template_madlib` | 148 | 93.96 % |
| `template_methods_paper` | 90 | 99.01 % |
| `template_newspaper` | 139 | 99.81 % |
| `template_pitch_deck` | 112 | 97.70 % |
| `template_pools_rules_tools` | 261 | 95.52 % |
| `template_prose_project` | 120 | 100.00 % |
| `template_redacted_report` | 111 | 98.83 % |
| `template_registered_report` | 34 | 96.37 % |
| `template_search_project` | 303 | 95.13 % |
| `template_sia` | 67 | 97.16 % |
| `template_storybook` | 15 | 93.92 % |
| `template_template` | 139 | 99.37 % |
| `template_textbook` | 180 | 96.73 % |
