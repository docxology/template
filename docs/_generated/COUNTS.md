...
(Last refreshed count: **751** on 2026-08-10 UTC — point-in-time; re-derive with the command above, the literal drifts as the tree changes.)
...
Result: **598** project-scope infrastructure tests collected and **820** publishing tests collected. Full behavioral gates still live in CI and in the verification commands listed by the relevant `AGENTS.md` files.

**Exemplar `pytest --collect-only` totals** (derived live in each project's declared environment; coverage snapshot last updated 2026-08-15; coverage values use the shared `release` test profile in each project's environment):

| Project | Tests collected | `src/` line+branch coverage |
|---------|-----------------|----------------------------|
| `template_active_inference` | 801 | 92.58 % |
| `template_advanced_literature_review` | 93 | 91.96 % |
| `template_autopoiesis` | 528 | 97.03 % |
| `template_autoresearch_project` | 374 | 96.32 % |
...
| `template_pitch_deck` | 142 | 98.57 % |
| `template_pools_rules_tools` | 285 | 94.72 % |
| `template_prose_project` | 161 | 95.87 % |
| `template_redacted_report` | 154 | 97.03 % |
| `template_registered_report` | 44 | 94.35 % |
| `template_search_project` | 321 | 96.45 % |
| `template_sia` | 77 | 94.39 % |
| `template_storybook` | 25 | 93.91 % |
| `template_template` | 147 | 97.66 % |
| `template_textbook` | 246 | 96.25 % |
