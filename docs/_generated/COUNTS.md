...
**Generated from live repo state on 2026-07-20 (UTC).** Volatile literals are re-derived on every run: tracked `infrastructure/` Python-file count via `git ls-files infrastructure | grep .py` (**710**), project-scope + publishing test collection via `pytest --collect-only` (**466** / **717**), the public exemplar roster, and the importable module list. The per-exemplar test/coverage snapshot table is a measured snapshot with source-commit and source-hash provenance in
...
(Last refreshed count: **710** on 2026-07-20 UTC — point-in-time; re-derive with the command above, the literal drifts as the tree changes.)
