...
| **Type Checking (MyPy)** | Catch type errors | `uv run python -m infrastructure.project.public_scope source-paths \| xargs uv run mypy` | ⚠️ Optional |
| **Linting & Formatting (Ruff)** | Enforce style & fix common issues | `uv run python -m infrastructure.project.public_scope source-paths \| xargs uvx ruff check --fix` | ⚠️ Optional |
...
|------|-------|-----------|-------|
| `ruff` | public CI source paths | `--fix` | Auto-fixes formatting & lint issues (CI scope) |
| `mypy` | public CI source paths | (pyproject.toml config) | Uses project type hints (CI scope) |
