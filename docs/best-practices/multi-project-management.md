...
- The simplified private sidecar requires `working/` and `archive/`; optional `ongoing/` (long-lived projects with no publication target) and legacy `active/` folders are still supported when present.
...
for project in */; do
    cd "$project"
    uv run python scripts/runner/execute_pipeline.py --project "$project" --core-only || echo "$project failed"
    cd ..
done
```
...
for project in */; do
    ...
    # Test
    uv run python scripts/runner/execute_pipeline.py --project "$project" --core-only || echo "Failed: $project"
