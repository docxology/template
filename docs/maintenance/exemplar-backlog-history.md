...
| `TEST-DISCOVERY-PERF-MED-1` | partial | [`test-performance-evidence.json`](test-performance-evidence.json), schema `template-test-performance-v1`, revision-bound 204-test pipeline-smoke comparison | `scripts/maintenance/benchmark_tests.py --target pipeline-smoke --profile quick --parallel-workers 2 --minimum-improvement 30` must pass on a clean checkout; dirty/staged/untracked source changes invalidate the evidence and subprocess diagnostics remain local-only |
