...
| **Current implementation** | `mypy` (community) with a progressive-strictness config and a zero-error gate. Base config sets `disallow_untyped_defs = false`; selected modules opt into `disallow_untyped_defs = true`, and no public package is hidden with `ignore_errors`. Runs over the public CI source set returned by `uv run python -m infrastructure.project.public_scope source-paths` (do not hard-code a file count — it drifts as the tree grows). |
...
| | |
| --- | --- |
| **Contract** | Infrastructure matrix testing across Python 3.10–3.14 on Ubuntu plus a Python 3.14 macOS smoke; coverage gates; linting; security scanning; doc-tree integrity checks. |
