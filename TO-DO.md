...
| `TEST-ISOLATION-SYSPATH-1` | open | Medium | Root and infra `conftest.py` sys.path loaders | Local migration complete as of 2026-09-21 (91e13c290, 414b8496c, 163613937 + finish pass): all 25 exemplars package-nested, zero `tests/__init__.py` shims, zero runnable `from src.`/`import src` sites (pinned scripts retargeted to unique packages; `template_template` roster pin refreshed to 25), `_PKG_ALIAS` loaders documented against the alias-wrapping code, madlib/literature pythonpaths fixed, advanced→literature symlink rebase done (unique-package imports + `_bootstrap` registration). Remaining: per-exemplar pytest ×25, the two-tree single-process acceptance below, regression manifest gate, full infra suite, exemplar matrix + provenance refresh (canonical release-profile coverage re-measured for advanced 91.96 %, madlib 99.11 %, literature 95.92 %, experiment_tree 91.85 %), and hosted py3.10 green per c8634497. | isolation restructure record | `uv run pytest projects/templates/template_autopoiesis/tests projects/templates/template_sia/tests --collect-only -q` | two exemplar trees collected in one process must not resolve another tree's modules |
