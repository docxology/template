...
| `EXECUTABLE-BUNDLE-MAJ-2` | open | Medium | Bundle payload composition, `infrastructure/` availability contract | The bundle Dockerfile now builds (2026-08-22: `template-bundle-verify:2026-08-22` built from `ubuntu:24.04` + Python 3.12 after the deadsnakes fix in `dockerfile_gen.py`), but the payload intentionally omits `infrastructure/`, so the compose `tests`/`render`/`verify` services cannot run standalone (verified: `pytest tests` in a `--network none` container fails collecting `src.benchmark_support` and `scripts.generate_api_docs` with `ModuleNotFoundError: No module named 'infrastructure'`). Decide the contract: either ship a self-contained payload (vendor `infrastructure/` into the bundle) or make the compose services fail-closed with an explicit unavailable-dependency receipt. | offline-container verification receipt naming the chosen contract | `docker run --rm --network none <bundle-image> uv run --project /workspace pytest /workspace/source/tests -q` | a bundle whose compose services silently import a missing `infrastructure/` must fail with an explicit receipt, not a bare ModuleNotFoundError |
...
| `DOC-NEGCTRL-HARDEN-MED-1` | open | Medium | Advisory RedTeam audit (`scripts/audit/audit_documentation.py`) | Triage the ~70 advisory `gate-negative-control` findings: for each active doc site that claims gate/verifier enforcement without naming a negative control, either add a nearby known-wrong fixture/negative-control sentence or reclassify the claim as advisory. Do not bulk-edit; fix per owning surface. | updated `audit_documentation.py` report with reduced finding count | `uv run python scripts/audit/audit_documentation.py` (gate-negative-control finding count must decrease) | deleting findings from prose without adding controls must not reduce the count |
