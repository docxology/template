...
| `SECRET-SCAN-1` | High | The first staged scanner read worktree bytes, so partial staging could hide an indexed secret or report an unstaged value. | Read exact A/C/M/R blobs from the Git index and run the scanner before every commit. | Real-Git partial-stage controls pass in both directions; findings contain only path, line, and kind. | SHIPPED: index-blob scanner, pre-commit/manual hook, tracked pre-push defense, rotation handoff, and 27 focused passing tests. |
