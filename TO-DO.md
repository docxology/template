...
| `REPRODUCIBLE-PDF-1` | Medium | LaTeX PDF builds embed runner timestamps (`\today`, `/CreationDate`, intermediate `.log`) which vary run-to-run on the same commit. The deterministic pipeline claims byte-for-byte reproducibility but lacks `SOURCE_DATE_EPOCH` propagation to the TeX build chain. | Export `SOURCE_DATE_EPOCH` in the composite CI setup action; add a rendered-output snapshot-diff test that fails on byte drift. | Two `--core-only` runs on the same commit produce byte-identical PDFs. | Deterministic timestamp propagation and snapshot-diff gate only. |
| `NO-MOCK-CLAIM-1` | Medium | The "No mocks or fakes" README claim is lexically true (lexical gate catches `unittest.mock` and `monkeypatch.setattr`) but semantically weak against hand-rolled stubs (`class FakeDB`), in-test fixture constants, and the admitted `pytest-httpserver` exception. | Add a hand-rolled-fake heuristic (class names matching `Fake*`/`Stub*`/`Dummy*`) to the advisory inventory; reword the README claim to "No unit-level mock frameworks; HTTP boundaries use an in-process test server." | `verify_no_mocks.py` advisory run reports hand-rolled fakes; README uses the scoped wording. | Lexical gate accuracy and claim scope only; not a policy change. |
| `TRACKED-OUTPUT-BUDGET-1` | Minor | The generated-artifacts guard has a 50MB blind spot and relies on a path allowlist that may not cover new exemplars. Files below 50MB but non-regenerable can be tracked without flagging. | Make the budget check fail-closed on any tracked binary in `output/` whose source isn't regenerable in a clean checkout, regardless of size. | `check_tracked_generated_artifacts.py` fails on a tracked non-regenerable binary in any `output/` subtree. | Budget-enforcement extension only; allowlist granularity improvement. |
