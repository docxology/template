...
| `TEST-ISOLATION-SYSPATH-1` | open | Medium | Root and infra `conftest.py` sys.path loaders | Root-level pytest now uses importlib collection as groundwork (single-tree suites unaffected), but two exemplar trees still cannot share one process: each exemplar's `tests/` is a package named `tests` (first tree's `tests.conftest` wins plugin registration) and every exemplar imports its source as the same top-level `src` package (source-module cross-resolution via `sys.modules`); remaining work is unique exemplar `tests`/`src` package naming; until then keep one exemplar pytest tree per process as documented in `CLAUDE.md`. | isolation restructure record | `uv run pytest projects/templates/template_autopoiesis/tests projects/templates/template_sia/tests --collect-only -q` | two exemplar trees collected in one process must not resolve another tree's modules |
