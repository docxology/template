...
| `SLOW-PROFILE-1` | open | Medium | pytest slow marker + CI test-infra lane | Exercise the 116 `pytest.mark.slow` infra tests (36 files; real-pandoc/latex rendering, llm review generators) on a scheduled/dispatch CI lane instead of never on CI; PR lanes keep the default deselection. | scheduled slow-lane job + collected count | `uv run pytest tests/infra_tests/ -m slow --collect-only -q` reports >0 collected and the scheduled CI lane runs them green | a broken slow-marked test must fail the scheduled lane while PR CI stays green — silent green on both lanes voids the row |
...
| `CI-WIRING-1` | open | Medium | `tests/integration/` suite + ci.yml | Wire `tests/integration/` (run.sh, secure_run.sh, output copying, execute_pipeline CLI) into CI with a `test-integration` job mirroring the existing test-regression job; the suite is registered but no CI job runs it. | ci.yml job | `grep -n "tests/integration" .github/workflows/ci.yml` is non-empty and the job is green | renaming `run.sh` must fail the suite locally (`uv run pytest tests/integration/ -q`) while pre-wiring CI stayed green |
...
| `CI-SLOWMARK-SHARD-1` | open | Medium | ci.yml test-project matrix | Run the active_inference/formal test-project cells with `-m "not slow"` on push/PR and a full `--include-slow` variant on schedule/dispatch only, mirroring the existing public-matrix-receipt precedent, so PR wall-clock drops by the slow-suite duration. | ci.yml lane change | `grep -n "include-slow" .github/workflows/ci.yml` shows conditional gating rather than a bare flag on the PR path | a slow-marked project-test regression must fail only the scheduled lane, never silently pass PR CI |
| `REHEARSAL-PARALLEL-1` | open | Medium | release-rehearsal.yml + `scripts/maintenance/release_rehearsal.py` | Run the two fresh-checkout rehearsals as a two-cell matrix job (add a `--runs`/shard flag to the rehearsal script if absent) so the ~55-65 min wall time approaches single-run time, each cell uploading its own receipt artifact. | release-rehearsal.yml matrix | `grep -n "matrix:" .github/workflows/release-rehearsal.yml` finds the two-cell strategy and the job stays green | unequal run digests must still fail the job — parallel cells must not weaken the two-run determinism comparison |
...
| `REHEARSAL-SCHEDULE-1` | open | Minor | release-rehearsal.yml triggers | Add a monthly cron trigger to the rehearsal workflow so clean-checkout receipts keep being generated without an operator remembering to dispatch. | release-rehearsal.yml trigger | `grep -c "cron" .github/workflows/release-rehearsal.yml` returns 1 | without the trigger, receipt staleness is undetectable until the next manual dispatch |
| `REHEARSAL-ARTIFACT-1` | open | Minor | release-rehearsal.yml artifact upload | Parameterize the rehearsal receipt artifact name with `run_id` and set explicit `retention-days: 365` so every dispatch is independently addressable and retained as release evidence. | release-rehearsal.yml upload step | the artifact name contains `github.run_id` and retention is explicit in the workflow | two consecutive dispatches must produce two retrievable artifacts — the old overwrite behavior must not recur |
| `REHEARSAL-SUMMARY-1` | open | Minor | release-rehearsal.yml summarize step | Guard the always-running "Summarize receipt" step so a rehearsal that fails before writing a receipt prints one clear message instead of turning `cat` into a confusing second failure. | release-rehearsal.yml summary step | `grep -n "receipt not produced" .github/workflows/release-rehearsal.yml` finds the guard and a receipt-less dry run passes the step | with a receipt present, the summary must still print it — the guard must not hide real receipts |
