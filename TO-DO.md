...
| ID | Status | Size | Dependency | Next action / unblock condition | Proving artifact | Acceptance command | Negative control |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `ERG-ENTRY-DOCS-2026-08-31` | completed | Minor | none | 2026-08-31 agent-ergonomics passes: orientation-ladder status/next-action links added to START_HERE.md (duplicate orientation blocks consolidated), AGENTS.md doc map, docs/documentation-index.md. See REVIEW_LOG_2026-08-31.md. | REVIEW_LOG_2026-08-31.md | `grep -c STATUS.md START_HERE.md AGENTS.md docs/documentation-index.md` | broken relative link or missing section must fail lint_docs |
...
| `DOC-ROOT-SCRATCH-HYGIENE-MIN-1` | open | Minor | Owner confirmation of root scratch files | Root carries untracked transient artifacts (`sidecar_*.py/.out/.jsonl`, `AUDIT_2026-08-30*.md`, `.laneD_results.json`, `.tmp_prune/`, `skillarum-docs/`) that a cold agent sees as if current. Archive under a clearly-named `_archive/` or delete once the owner confirms they are no longer needed; do not link them from entry docs. | clean `git status` for root scratch paths | `git status --porcelain` filtered for `sidecar_` then for `AUDIT_2026` (each count must be 0; a piped grep cannot appear inside a markdown table cell) | archiving must not delete files with in-tree inbound links without updating the links |
...
| `AGENT-ERG-COUNTS-PROVENANCE-MED-1` | open | Medium | `scripts/docgen/counts.py` coverage-provenance refresh | counts.py --check reports stale coverage provenance after each coverage measurement (observed 2026-08-31; cf. commits 1dd67aee8, 15045e302). Fold `uv run python scripts/docgen/counts.py --refresh-coverage-provenance --write` into the documented post-measurement step so --check does not require an ad-hoc fix each time. | refreshed docs/_generated/coverage_snapshot.json | `uv run python scripts/docgen/counts.py --check` | stale provenance must fail --check (current behavior) |
...
| `ERG-SIDECAR-SCRATCH-MIN-1` | open | Minor | Root-level `sidecar_*.{py,json,out,txt,jsonl}` (19 untracked files) | Owner of the 2026-08-30 sidecar-migration work should delete or archive these one-off scratch scripts/logs (several hard-code absolute paths); they are transient artifacts, not documentation. Not removed by the 2026-08-31 fleet lane because the shared dirty tree could not prove no sibling lane was using them. | clean root (no sidecar_* scratch) | `ls sidecar_* 2>/dev/null` returns nothing | a root-level scratch script that no doc references must not persist past its task |
