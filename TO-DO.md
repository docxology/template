...
| `TEST-MODULE-SPLITS-1` | open | Medium | Section-banner conventions in the eight oversized modules | Split the eight >800-line test modules (test_combined_exports 1,347; test_pipeline 1,333; test_counts_doc 1,509; test_slides_renderer_core 1,219; test_web_renderer 1,067; test_output_validator 980; test_artifact_manifest_semantics 994; test_publishing 842) along their existing section banners into per-production-module files; pure moves, no behavior change. | split record with collection-parity check | `uv run pytest tests/infra_tests/ -q --co` | collected test count before/after the split must match exactly |
...
| `REHEARSAL-ANALYSIS-EXIT-1` | partial | Medium | Hosted re-dispatch of the quick-profile public matrix | Root cause found and fixed 2026-09-07 (local probe with the exact all-projects flags): (1) the deterministic exit 1 behind an all-green receipt was the receipt's output-isolation check — every exemplar's declared Stage-01 verifier legitimately regenerates manifest-declared outputs (``artifact_provenance.json`` re-pins ``source_commit`` to the current HEAD), so the before/after digest flipped on any fresh clone at a newer commit; the isolation comparison now excludes each project's ``output/reports/artifact_manifest.json``-declared paths (negative control preserved: undeclared output mutations still fail; see ``test_receipt_rejects_test_generated_output_drift``). (2) A second, independent blocker the probe surfaced: storybook's declared 90% floor fails under the quick profile (88.73% with the 7 slow-marked rendering tests deselected; 93.91% with ``--include-slow``) — tracked as ``STORYBOOK-QUICK-FLOOR-1``. Remaining: merge the isolation fix, resolve ``STORYBOOK-QUICK-FLOOR-1``, re-dispatch the hosted quick-profile matrix, and close ``CLEAN-CHECKOUT-MAJ-1`` with the green receipt. | changelog 2026-09-07 entry + local probe receipt | `uv run python scripts/pipeline/stage_01_test.py --project-only --all-projects --public-projects --profile quick --project-workers 2 --receipt /tmp/rr.json` (exit 0) | a silent exit 1 with all tests passing must not be treated as a green rehearsal |
| `STORYBOOK-QUICK-FLOOR-1` | open | Medium | Owner decision on profile-vs-floor contract | Storybook declares a 90% coverage floor but only satisfies it with its 7 ``@pytest.mark.slow`` ``test_rendering.py`` tests (16.6s of real PIL rendering); the quick profile deselects them and lands at 88.73%, so any quick-profile lane (rehearsal/matrix receipt) fails storybook on the coverage gate while ``--include-slow`` passes at 93.91%. CI project lanes run ``--include-slow``, so only the all-projects receipt lane hits this. Pick one: unmark the 7 tests, a profile-aware floor, or recalibrate the declared floor; do not silently weaken the full-profile gate. | probe receipt lane record + /tmp/sb_quick.log | `uv run python scripts/pipeline/stage_01_test.py --project templates/template_storybook --project-only --profile quick` (exit 0 after fix) | the floor must still hold with ``--include-slow`` (93.91% measured 2026-09-07); lowering the declared floor must not excuse the full-profile suite |
