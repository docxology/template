...
|| `GRAFT-DOCACCURACY-1` | partial | Medium | `infrastructure/validation/repo/_repo_documented_commands.py` | `check_documented_commands` scanned only `README.md` + `docs/*.md`, leaving root `START_HERE.md`, `CLAUDE.md`, and `AGENTS.md` unguarded; scanner extension + tests landed locally, close after drift findings are triaged and the entry docs reconcile. | updated scanner + test_documented_commands.py | `uv run pytest tests/infra_tests -k documented_commands -q` green and a deliberately missing script path in CLAUDE.md must fail the scan | renaming a pipeline script referenced in START_HERE.md must fail the extended scan, not pass silently |
