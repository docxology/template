...
| `AGENT-ERG-ROOT-REPORTS-MED-2` | completed | Medium | Root `_FLEET_REPORT_2026-08-30.md` and `REVIEW_LOG_2026-08-31.md` | 2026-08-31 round-2 agent-ergonomics pass: the two tracked root-level dated lane reports violated the docs/audit/AGENTS.md root-hygiene policy (root Markdown is long-lived docs only). Moved via git mv to docs/audit/ with a dated archival note in docs/audit/AGENTS.md; TO-DO row ERG-ENTRY-DOCS-2026-08-31 evidence link updated to the new path. Verified live this session: no dated-report Markdown remains at repo root. | this row plus docs/audit/AGENTS.md archival note | `git ls-files | grep -E '^(_FLEET_REPORT|REVIEW_LOG)'` (must print nothing) and `ls _FLEET_REPORT_2026-08-30.md REVIEW_LOG_2026-08-31.md 2>/dev/null` (must be empty) | a tracked root-level dated report file must fail this check |
