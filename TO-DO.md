...
| `DOC-ROOT-SCRATCH-HYGIENE-MIN-1` | open | Minor | Owner confirmation of root scratch files | Root carries untracked transient artifacts (`sidecar_*.py/.out/.jsonl`, `AUDIT_2026-08-30*.md`, `.laneD_results.json`, `.tmp_prune/`, `skillarum-docs/`) that a cold agent sees as if current. Archive under a clearly-named `_archive/` or delete once the owner confirms they are no longer needed; do not link them from entry docs. | clean `git status` for root scratch paths | `git status --porcelain | grep -E 'sidecar_|AUDIT_2026' | wc -l` | archiving must not delete files with in-tree inbound links without updating the links |
