...
> **Local safety overlay:** bundled Python and shell fragments are architectural
> examples, not trusted entry points. Do not execute them automatically. Use
> argv-based subprocess APIs, keep credentials out of clone URLs and process
> listings, validate repository/identity inputs, and preserve human approval for
> pushes, deployments, credentials, and destructive lifecycle operations.
