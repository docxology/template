...
### Pattern 6: Learning Through Reviewed Memory

Persist learned preferences in a dedicated, reviewable memory file so subsequent
sessions can retrieve them without silently rewriting governing instructions.
In this template, use the gitignored
`.cursor/hooks/state/continual-learning-memory.json` via
`infrastructure.core.agent_memory`; never append learned rules to `AGENTS.md`,
`CLAUDE.md`, or other tracked instruction files.
...
Guard this pattern with validation because accumulated memory can become
incorrect or contradictory. Treat it as experimental, retain provenance, and
review persisted preferences periodically.
