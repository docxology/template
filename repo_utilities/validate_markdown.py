...
try:
    ...
    import_error = f"❌ Error: Failed to import from infrastructure/markdown_validator.py\n   {e}\n   Ensure infrastructure/markdown_validator.py exists and is properly formatted"
else:
    import_error = None
...
def find_markdown_files(directory: str) -> list:
    ...
    # Sort numerically
    return sorted(md_files, key=lambda x: (
        ''.join(filter(str.isalpha, os.path.basename(x))),
        int(''.join(filter(str.isdigit, os.path.basename(x)))) if any(c.isdigit() for c in os.path.basename(x)) else 0
    ))
