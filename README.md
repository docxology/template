...
| `template-experiment-tree` | `template_experiment_tree` | Use `template_experiment_tree` when the deliverable is a research log that stays honest over months: you want planned, preregistered, and answered experiments in one durable tree, negative results registered (not buried), and a manuscript that regenerates from the tree instead of drifting from it. |
...
| [`template_experiment_tree`](projects/templates/template_experiment_tree/) | Use `template_experiment_tree` when the deliverable is a research log that stays honest over months: you want planned, preregistered, and answered experiments in one durable tree, negative results registered (not buried), and a manuscript that regenerates from the tree instead of drifting from it. |
...
- **Output** is generated under each exemplar's project-local
  `projects/templates/<name>/output/` tree. Final deterministic evidence
  (publication PDFs, figures, analysis data, hydrated manuscripts, and
  release/validation registries) may be tracked for public exemplars;
  checkpoints, logs, telemetry, pipeline snapshots, and LaTeX/slide build
  intermediates are disposable, ignored, and regeneratable.
