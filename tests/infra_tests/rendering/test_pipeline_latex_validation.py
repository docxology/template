"""Tests for the LaTeX package pre-flight validation used by the render pipeline.
...
from infrastructure.rendering._manuscript_source import validate_latex_packages
...
def _make_report(
    *,
    ...
    """Build a real ValidationReport dataclass for testing validate_latex_packages."""
...
# ---------------------------------------------------------------------------
# validate_latex_packages  (real ValidationReport instances — no mocking)
# ---------------------------------------------------------------------------


def test_validate_latex_packages_all_available() -> None:
    """Returns 0 when all required packages are available."""
    result = validate_latex_packages(report=_make_report(all_required=True))
...
def test_validate_latex_packages_missing_required() -> None:
    ...
    result = validate_latex_packages(report=_make_report(all_required=False, missing_req=["multirow", "cleveref"]))
...
def test_validate_latex_packages_optional_missing_still_passes() -> None:
    ...
    result = validate_latex_packages(report=_make_report(all_required=True, missing_opt=["minted"]))
...
def test_validate_latex_packages_empty_report() -> None:
    ...
    result = validate_latex_packages(report=_make_report())
...
def test_validate_latex_packages_os_error_is_non_fatal() -> None:
    ...
    This test calls validate_latex_packages() with no report so it runs the
