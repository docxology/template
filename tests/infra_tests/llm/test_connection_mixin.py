...
class ConnectionTestClient(_ConnectionMixin):
    """Minimal real HTTP client host used to exercise the connection mixin."""
...
class TestGetAvailableModels:
    def test_success(self, httpserver: HTTPServer):
        ...
        client = ConnectionTestClient(httpserver.url_for("").rstrip("/"))
    ...
    def test_connection_error_fallback(self):
        client = ConnectionTestClient("http://127.0.0.1:1", timeout=0.5)
        models = client.get_available_models()
...
class TestGenerateResponseDirect:
    ...
    def test_timeout_with_retry(self, httpserver: HTTPServer):
        ...
        client = ConnectionTestClient(httpserver.url_for("").rstrip("/"), timeout=0.5)
        result = client._generate_response_direct("m", [{"role": "user", "content": "x"}], retries=1)
        assert result == "Recovered"

    def test_connection_error_all_retries(self):
        client = ConnectionTestClient("http://127.0.0.1:1", timeout=0.3)
