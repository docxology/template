...
Covers: normalize_for_comparison, jaccard_similarity, tf_cosine_similarity,
sequence_similarity, plus the composite similarity behavior via the public
...
class TestNormalizeForComparison:
    ...
    def test_lowercase(self):
        assert "hello world" in normalize_for_comparison("Hello World")

    def test_removes_markdown(self):
        result = normalize_for_comparison("## **Bold** _italic_ `code`")
    ...
    def test_normalizes_whitespace(self):
        result = normalize_for_comparison("  too   many    spaces  ")
        assert "  " not in result

    def test_replaces_numbers(self):
        result = normalize_for_comparison("Section 42 has 100 items")
...
class TestJaccardSimilarity:
    ...
    def test_completely_different(self):
        sim = jaccard_similarity("cat dog bird", "car house tree")
        assert sim == 0.0

    def test_partial_overlap(self):
        sim = jaccard_similarity("the cat sat", "the dog sat")
        assert 0.0 < sim < 1.0
...
class TestTfCosineSimilarity:
    """Test tf_cosine_similarity."""

    def test_identical_texts(self):
        sim = tf_cosine_similarity("hello world test", "hello world test")
        assert abs(sim - 1.0) < 0.01

    def test_completely_different(self):
        sim = tf_cosine_similarity("alpha beta gamma", "delta epsilon zeta")
        assert sim == 0.0

    def test_partial_overlap(self):
        sim = tf_cosine_similarity("the quick brown fox", "the slow brown dog")
        assert 0.0 < sim < 1.0

    def test_empty_texts(self):
        assert tf_cosine_similarity("", "hello") == 0.0
        assert tf_cosine_similarity("hello", "") == 0.0


class TestSequenceSimilarity:
    """Test sequence_similarity."""
    ...
    def test_short_texts(self):
        sim = sequence_similarity("ab", "cd")
        assert sim == 0.0

    def test_empty_texts(self):
        assert sequence_similarity("", "hello world test") == 0.0
