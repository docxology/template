...
"""
    ...
    def _make_executor(self, repo_root: Path, project_name: str, *, clean: bool, skip_infra: bool, skip_llm: bool, resume: bool) -> PipelineExecutor:
...
""")
    ...
    def test_execute_full_pipeline_success(self, tmp_path: Path):
        ...
        executor = self._make_executor(repo_root, "test", clean=False, skip_infra=False, skip_llm=False, resume=False)

        results = executor.execute_full_pipeline()

        assert len(results) == 9  # setup, infra, project, analysis, pdf, validate, llm review, llm translations, copy
        assert all(r.success for r in results)
        assert all(r.stage_num == i+1 for i, r in enumerate(results))

        # Verify stage names
        expected_names = [
            "Environment Setup", "Infrastructure Tests", "Project Tests", "Project Analysis",
            "PDF Rendering", "Output Validation", "LLM Scientific Review",
            "LLM Translations", "Copy Outputs"
        ]
    ...
    def test_execute_core_pipeline_success(self, tmp_path: Path):
        ...
        executor = self._make_executor(repo_root, "test", clean=False, skip_infra=False, skip_llm=True, resume=False)
        ...
        expected_names = [
            "Environment Setup", "Infrastructure Tests", "Project Tests", "Project Analysis",
            "PDF Rendering", "Output Validation", "Copy Outputs"
        ]
        assert [r.stage_name for r in results] == expected_names
