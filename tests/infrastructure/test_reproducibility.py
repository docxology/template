...
import infrastructure.reproducibility
...
class TestReproducibilityVerification:
    ...
    def test_verify_reproducibility_environment_changed(self, mock_capture):
        ...
        from infrastructure.reproducibility import ReproducibilityReport
...
class TestEdgeCases:
    ...
    def test_generate_build_manifest_with_exception(self, tmp_path):
        ...
        # Patch stat() to properly handle all calls including from Path.exists()
        call_count = [0]
        original_stat = Path.stat
        
        def mock_stat(self, *, follow_symlinks=True):
            # Accept the follow_symlinks keyword argument
            call_count[0] += 1
        ...
        with patch.object(Path, 'stat', mock_stat):
            # This will test the exception path in the first loop
