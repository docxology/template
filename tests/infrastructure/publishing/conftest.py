...
def zenodo_test_server():
    ...
    server.expect_request("/api/files/bucket123/test.pdf", method="PUT").respond_with_json({
        ...
        "size": 1234,
        "links": {
            "self": f"{server.url_for('')}/api/files/bucket123/test.pdf"
        }
    })
    ...
    server.expect_request("/api/files/bucket123/paper.pdf", method="PUT").respond_with_json({
        ...
        "size": 5678,
        "links": {
            "self": f"{server.url_for('')}/api/files/bucket123/paper.pdf"
        }
    })

    # Mock publish endpoint
    server.expect_request("/api/deposit/depositions/12345/actions/publish", method="POST").respond_with_json({
