...
class TestVerifyFiguresExist:
    ...
    def test_verify_figures_sorted(self, tmp_path):
        ...
        assert result["found_figures"] == ["a_figure.png", "m_figure.png", "z_figure.png"]
...
class TestDiscoverManuscriptFiles:
    ...
    def test_discover_excludes_metadata_files(self, tmp_path):
        ...
        assert len(result) == 1
        assert result[0].name == "01_abstract.md"
        assert not any(f.name in ["preamble.md", "AGENTS.md", "README.md", 
                                  "config.yaml", "config.yaml.example", "references.bib"] 
                      for f in result)
