...
pytest tests/infrastructure/rendering/test_pdf_renderer_full.py

# Run CLI tests only
pytest tests/infrastructure/rendering/ -k "cli"

# Run with LaTeX requirement check
pytest tests/infrastructure/rendering/ --ignore-glob="*latex*"  # Skip if no LaTeX
```
...
- name: Test Rendering Infrastructure
  run: |
    ...
    # Run rendering tests
    uv run pytest tests/infrastructure/rendering/ -v
```
...
pytest tests/infrastructure/rendering/ --cov=infrastructure.rendering --cov-fail-under=90
```
...
pytest tests/infrastructure/rendering/test_pdf_renderer_full.py -v -s

# Debug LaTeX compilation
pytest tests/infrastructure/rendering/ --pdb --tb=long
```
